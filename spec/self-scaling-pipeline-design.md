...
2. **Mandatory independent adversarial passes** — the critique + counterexample sub-agents
   spin up full independent reviewers every run, even for a trivial change
...
(the circularity risk: `changeClassification` consumes `finalized_module_contracts`/obligations that
