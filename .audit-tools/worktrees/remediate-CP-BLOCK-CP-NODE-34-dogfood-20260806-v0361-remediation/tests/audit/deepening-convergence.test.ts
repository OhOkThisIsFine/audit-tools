...
// The validator/backfill helpers accept loosely-shaped JSON (`unknown` /
// generic `T`), because they exist precisely to police untrusted worker
// output. These fixture shapes mirror that loosely-validated payload —
// `lens` is deliberately OPTIONAL on the finding (never required), matching
// the omission this whole suite exercises against the runtime guard.
interface FixtureFinding {
  ...
  lens?: string;
  summary: string;
  affected_files: Array<{ path: string; line_start?: number; line_end?: number }>;
  evidence?: string[];
}

interface FixtureAuditResult {
  task_id: string;
  unit_id: string;
  pass_id: string;
  lens: string;
...
function deepeningTask(id: string, lens = "security"): AuditTask {
  return {
    task_id: id,
...
function findingWithoutLens(): FixtureFinding {
  return {
    id: "SEC-001",
...
function resultFor(
  taskId: string,
  { lens = "security", findings }: { lens?: string; findings?: FixtureFinding[] } = {},
): FixtureAuditResult {
  return {
    task_id: taskId,
    unit_id: "flow:auth:security",
