...
import {
  ...
  type BrokeredCompletion,
  type BrokerDispatchInput,
} from "../../src/shared/repair/brokeredDispatch.js";
import {
  enforceSchemaAtEmit,
  buildWorkerRepairContract,
  resolveEmitConstraint,
} from "../../src/audit/contracts/schemaEnforcedEmit.js";
import {
  WorkerAuditResultsSchema,
  WORKER_SCHEMA_SOURCES,
  renderWorkerJsonSchema,
} from "../../src/audit/contracts/workerSchemas.js";
import type { ResolvedProviderName } from "../../src/shared/types/sessionConfig.js";
...
const noopDeps: FreshSessionProviderDeps = {
  orchestratorName: "audit-code",
  createClaudeCodeProvider: () => ({ name: "claude-code", launch: async () => ({ accepted: true }) }),
  createClaudeWorkerProvider: () => ({ name: "claude-worker", launch: async () => ({ accepted: true }) }),
  createOpenCodeProvider: () => ({ name: "opencode", launch: async () => ({ accepted: true }) }),
  createAgyProvider: () => ({ name: "agy", launch: async () => ({ accepted: true }) }),
};
...
test("F3 inv-2: descriptor is output-constraint-scoped only — no concurrency/agent-nesting field (CP-NODE-23)", () => {
  ...
  for (const [name, config] of [
    ...
    const provider = createFreshSessionProvider(name as ResolvedProviderName, config, noopDeps);
...
test("F3 inv-4: an advertised constraint mode + conforming payload validates with zero repair stages", async () => {
  ...
  for (const mode of ["json_schema_constrained", "forced_tool_call"] as const) {
    let patcherCalls = 0;
    const result = await enforceSchemaAtEmit({
      contractId: "audit_results",
      schema: WorkerAuditResultsSchema,
...
test("F3: capability 'none' + invalid payload degrades via O3, patch routed through the broker", async () => {
  ...
  expect((result.repair.repaired_payload as unknown[]).length).toBe(1);
});
...
test("F3: a refused broker means no LLM touch — O3 falls to the re-dispatch signal", async () => {
  ...
  const refusingBroker: BrokeredRepairDispatch = {
    broker: () => ({
      admitted: 0,
...
test("F3 inv-5 [CP-NODE-26]: capability 'none' degrades through the brokered repair seam", async () => {
  ...
  let awaitCalls = 0;
  let patcherCalls = 0;
  const seamBroker: BrokeredRepairDispatch = {
    broker: ({ slots }) => {
      brokerCalls += 1;
...
test("F3 inv-6 [CP-NODE-27]: enforcement core has no per-backend branching (parametrized over all providers)", async () => {
  ...
  const allProviders: ResolvedProviderName[] = [
    'claude-code',
    'codex',
    'opencode',
    'worker-command',
  ...
  const providerNameSet = new Set<string>(allProviders);
  const branchViolations: string[] = [];

  const isProviderNameLiteral = (node: ts.Node) =>
    (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) &&
    providerNameSet.has(node.text);

  const isProviderNameAccess = (node: ts.Node) => {
...
test("F3 fail-1 [CP-NODE-29]: mis-advertised mode + invalid emit => degrade to repair, never accept", async () => {
  ...
  for (const advertisedMode of [
    ...
    expect((result.repair.repaired_payload as unknown[]).length, `${advertisedMode}: the repaired payload is the salvaged valid worker result`).toBe(1);
  }
});
...
test("F3 fail-4 [CP-NODE-32]: tool-owned identity strip handled by O3 stage-1 coercion, not re-implemented in F3", async () => {
  ...
  expect((result.repair.repaired_payload as Array<Record<string, unknown>>)[0].task_id, "the tool-owned identity is restored by the O3 brokered stage, not by F3").toBe("T1");
});
