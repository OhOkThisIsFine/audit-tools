...
const { cmdAdvanceAudit } = await import("../../src/audit/cli/advanceAuditCommand.js");
const { listBatchResultFiles } = await import("../../src/audit/cli/args.js");
const { cmdValidateResult } = await import("../../src/audit/cli/validateResultCommand.js");
const {
  isCoverageTotalLinesMismatch,
  emitCoverageLineCountFriction,
} = await import("../../src/audit/validation/auditResults.js");
const { frictionCaptureDir } = await import("../../src/shared/index.js");
...
describe("advance-audit rejects --results+--batch-results before destructive cleanup", () => {
  let dir: string;
  ...
  beforeAll(async () => {
    dir = await tmp();
    artifactsDir = join(dir, ".audit-tools", "audit");
    ...
    sentinel = join(artifactsDir, "sentinel.txt");
    await writeFile(sentinel, "keep-me");

    batchDir = join(dir, "batch");
    await mkdir(batchDir, { recursive: true });
  });

  it("throws on conflicting flags", async () => {
  ...
  it("artifacts survive the throw (not swept by cleanup)", async () => {
    expect(await exists(sentinel), "sentinel must survive — cleanup must not run before the flag check").toBe(true);
...
describe("listBatchResultFiles admits only canonical result filenames", () => {
  let dir: string;
  ...
  it("returns only canonical files", async () => {
    const files = await listBatchResultFiles(dir);
    const names = files.map((f) => f.split(/[\\/]/).pop()).sort();
    expect(names).toEqual([packetInline, canonical].sort());
  });

  it("throws when only stray JSON is present", async () => {
...
describe("validate-result grounds against configured root, not cwd", () => {
  ...
  beforeAll(async () => {
    ...
    artifactsDir = join(repoRoot, ".audit-tools", "audit");
    ...
    const { taskResultPath } = await import("../../src/audit/cli/args.js");
    await writeFile(taskResultPath(taskResultsDir, taskId), JSON.stringify(result));
  });

  it("no ungrounded warning when span resolves under configured root", async () => {
    const errs: string[] = [];
    ...
    expect(/ungrounded finding/.test(joined), `expected no ungrounded warning, got:\n${joined}`).toBe(false);
  });
});
...
describe("line-count mismatch policy is single-sourced", () => {
  it("predicate matches only file_coverage total_lines warnings", () => {
    expect(isCoverageTotalLinesMismatch({ severity: "warning", field: "file_coverage[0].total_lines", message: "m", path: "src/a.ts", result_index: 0, task_id: "t" })).toBe(true);
    expect(isCoverageTotalLinesMismatch({ severity: "error", field: "file_coverage[0].total_lines", message: "m", path: "src/a.ts", result_index: 0, task_id: "t" })).toBe(false);
    expect(isCoverageTotalLinesMismatch({ severity: "warning", field: "findings[0].title", message: "m", path: "src/a.ts", result_index: 0, task_id: "t" })).toBe(false);
  });

  it("emitter writes friction for a mismatch and nothing for unrelated issues", async () => {
    const dir = await tmp();
    ...
    expect(afterNoise, "no friction file for unrelated issue").toEqual([]);
    ...
    expect(afterMismatch.length > 0, "a friction artifact must be written for the mismatch").toBeTruthy();
  });
});
