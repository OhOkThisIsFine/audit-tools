...
} from "../../src/audit/cli/dispatch/types.js";
...
function multiPacketTasks(): AuditTask[] {
...
function singlePacketTask(): AuditTask[] {
  return [
    {
      task_id: "t-only",
      unit_id: "unit-only",
...
async function makeArtifactsDir(tasks: AuditTask[]): Promise<{ artifactsDir: string; runDir: string }> {
...
function run(artifactsDir: string, sessionConfig?: SessionConfig): Promise<PrepareDispatchResult> {
...
async function readActiveDispatch(artifactsDir: string): Promise<ActiveDispatchState> {
...
await test("FINDING-009: the packet prompt references the schema file and retains existing constraints", async (t) => {
  ...
  onTestFinished(() => rm(artifactsDir, { recursive: true, force: true }));
  await run(artifactsDir);
  const plan = await readJson<DispatchPlanEntry[]>(join(runDir, "dispatch-plan.json"));
...
function sharedFileTasks(tokenEstimate: number): AuditTask[] {
...
await test("FINDING-018: dispatch plan entries include forbidden_patterns for common stray filenames", async (t) => {
  ...
  const plan = await readJson<DispatchPlanEntry[]>(join(join(artifactsDir, "runs", RUN_ID), "dispatch-plan.json"));
  const entry = plan[0];
