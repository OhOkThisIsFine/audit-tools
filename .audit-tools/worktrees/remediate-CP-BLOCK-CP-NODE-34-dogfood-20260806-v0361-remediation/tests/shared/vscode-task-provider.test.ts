...
import { SubprocessTemplateProvider } from "../../src/shared/providers/subprocessTemplateProvider.js";
import {
...
import type { spawnLoggedCommand } from "../../src/shared/providers/spawnLoggedCommand.js";

// Minimal LaunchFreshSessionInput-shaped object for launch() tests.
function buildInput(): LaunchFreshSessionInput {
  return {
    repoRoot: "/repo",
    runId: "run-1",
...
test("SubprocessTemplateProvider (vscode-task) delegates launch via injected launchCommand", async () => {
  ...
  const spyLaunchCommand: typeof spawnLoggedCommand = (command, args, input) => {
...
test("SubprocessTemplateProvider (vscode-task): launchCommand is invoked with rendered command array", async () => {
  const calls: { command: string; args: string[] }[] = [];
  const capturingLaunch: typeof spawnLoggedCommand = (command, args, input) => {
    calls.push({ command, args });
    return Promise.resolve({
      accepted: true,
      exitCode: 0,
