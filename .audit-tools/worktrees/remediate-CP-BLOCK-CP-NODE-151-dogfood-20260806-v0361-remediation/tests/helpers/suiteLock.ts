...
// ────────────────────────────────────────────────────────────────────────────
// Test fixture helpers (extracted from duplicated patterns in test files)

import { mkdir, rm, writeFile } from "node:fs/promises";
import { join } from "node:path";

/**
 * Cleans up and recreates a test directory. Used in beforeEach hooks to ensure
 * test isolation. This pattern was duplicated across intake-resolver.test.ts,
 * implement-dispatch-cluster.test.ts, and intake-sources-and-digest.test.ts.
 */
...
/**
 * Cleans up a test directory without recreating it. Used in afterEach hooks.
 */
export async function cleanupTestDirOnly(testDir: string): Promise<void> {
  await rm(testDir, { recursive: true, force: true });
}

/**
 * Cleans up the test directory and sets up a repo directory structure with
 * package.json. Used in tests that need a minimal repo setup.
 */
export async function cleanupAndSetupRepo(
  testDir: string,
  repoDir: string,
  artifactsDir: string,
  packageName: string = "test-repo",
): Promise<void> {
  await rm(testDir, { recursive: true, force: true });
