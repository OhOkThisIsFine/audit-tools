...
import { runSlidingWindow } from "../../src/shared/quota/slidingWindow.js";

/** Narrow a settled result to its fulfilled arm, failing the test loudly otherwise. */
function fulfilledValue<T>(result: PromiseSettledResult<T>): T {
  if (result.status !== "fulfilled") {
    throw new Error(`expected a fulfilled result, got ${result.status}`);
  }
  return result.value;
}
...
test("DR-008 slidingWindow: all tasks complete and results array is fully populated", async () => {
  ...
  for (let i = 0; i < total; i++) {
    ...
    expect(fulfilledValue(results[i])).toBe(i * 2);
  }
});
...
test("DR-008 slidingWindow: task rejections do not drop subsequent tasks", async () => {
  ...
  for (let i = 0; i < total; i++) {
    if (i % 2 === 0) {
      ...
      expect(results[i].status).toBe("fulfilled");
      expect(fulfilledValue(results[i])).toBe(i);
    }
  }
});
