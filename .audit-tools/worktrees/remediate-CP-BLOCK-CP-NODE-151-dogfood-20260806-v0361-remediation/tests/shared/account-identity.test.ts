...
import { deriveAccountKey, deriveCredentialIdentity } from "../../src/shared/quota/accountId.js";
import { windowResourceKey } from "../../src/shared/quota/windowConstraints.js";
import { dispatchableSourceId } from "../../src/shared/quota/apiPool.js";
import type { DispatchableSource } from "../../src/shared/types/sessionConfig.js";

const NIM = "https://integrate.api.nvidia.com/v1";

/** Narrow deriveAccountKey's nullable result to a definite string, failing loudly when
 * unattributable (mirrors acquiredToken's narrowing idiom in claim-lease.test.ts). */
function definiteAccountKey(source: DispatchableSource): string {
  const key = deriveAccountKey(source);
  if (key === null) throw new Error("expected a derivable account key");
  return key;
}
...
test("the account key never contains a secret", () => {
  const source = { id: "x", transport: "openai-compatible", endpoint: NIM, api_key_env: "NVIDIA_API_KEY" };
  const key = deriveAccountKey(source);
...
test("an unattributable source yields null so the caller meters it ALONE", () => {
  ...
  const unattributable = { transport: "openai-compatible", model: "m" };
  expect(deriveAccountKey(unattributable)).toBeNull();
...
test("the JOIN: siblings get DIFFERENT pool ids but the SAME account key", () => {
  ...
  const nano: DispatchableSource = { id: "nim-nano", transport: "openai-compatible", endpoint: NIM, api_key_env: "NVIDIA_API_KEY", model: "nano" };
  const superb: DispatchableSource = { id: "nim-super", transport: "openai-compatible", endpoint: NIM, api_key_env: "NVIDIA_API_KEY", model: "super" };
  ...
  expect(deriveAccountKey(nano)).toBe(deriveAccountKey(superb));
  expect(windowResourceKey("account", "session", nanoId, definiteAccountKey(nano))).toBe(
    windowResourceKey("account", "session", superId, definiteAccountKey(superb)),
  );
});
