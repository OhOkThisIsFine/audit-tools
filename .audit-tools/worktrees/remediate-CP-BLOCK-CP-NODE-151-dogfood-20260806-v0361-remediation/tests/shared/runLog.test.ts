...
import {
  RunLogger,
  type RunLogEvent,
} from "../../src/shared/observability/runLog.js";
...
test("non-serializable event writes minimal fallback marker", async () => {
  await withTempDir(async (dir) => {
    ...
    const circular: RunLogEvent & { self?: RunLogEvent } = {
      kind: "obligation",
      self: undefined,
    };
...
test("RunLogEvent compile-time type constraints", async () => {
  ...
  const _bad1: RunLogEvent = { kind: "executor_end", durtion_ms: 5 };
  void _bad1;
