...
import {
  ...
  mapUsageToSnapshot,
  type ClaudeOAuthQuotaSourceOptions,
} from "../../src/shared/quota/claudeOAuthQuotaSource.js";
import {
  CodexQuotaSource,
  mapCodexUsage,
  type CodexQuotaSourceOptions,
} from "../../src/shared/quota/codexQuotaSource.js";
import {
  CopilotQuotaSource,
  mapCopilotUsage,
  type CopilotQuotaSourceOptions,
} from "../../src/shared/quota/copilotQuotaSource.js";
import {
  AntigravityQuotaSource,
  mapAntigravityUsage,
  type AntigravityQuotaSourceOptions,
} from "../../src/shared/quota/antigravityQuotaSource.js";
import {
...
type RecordingFetch = FetchLike & { calls: Array<{ url: string; init?: Parameters<FetchLike>[1] }> };
...
const status = (code: number): FetchResponseLike => ({ ok: false, status: code, json: async () => ({}) });
...
function claudeSource(
  fetchImpl: FetchLike,
  extra: Partial<ClaudeOAuthQuotaSourceOptions> = {},
): ClaudeOAuthQuotaSource {
...
function codexSource(
  fetchImpl: FetchLike,
  extra: Partial<CodexQuotaSourceOptions> = {},
): CodexQuotaSource {
...
function copilotSource(
  fetchImpl: FetchLike,
  extra: Partial<CopilotQuotaSourceOptions> = {},
): CopilotQuotaSource {
...
function antigravitySource(
  fetchImpl: FetchLike,
  extra: Partial<AntigravityQuotaSourceOptions> = {},
): AntigravityQuotaSource {
...
interface ProviderRow {
  ...
  key: string;
  live: unknown;
  source: (fetchImpl: FetchLike) => QuotaSource;
...
test("inv-6 / fail-5,7: every mapper degrades a malformed payload to null and never throws", () => {
  // Deliberately-untyped garbage, mirroring the runtime shape of an untrusted
  // JSON.parse result (exactly what `fetchJsonOrNull` hands each mapper in
  // production — see `fetchClaudeUsage`'s `body as UsageResponse`). The single
  // cast-from-`unknown` below at each call site mirrors that same boundary cast.
  const garbage: unknown[] = [null, undefined, {}, { rate_limit: null }, { models: null }, { quota_snapshots: null }, 42, "x", []];
  type UsageBody = Parameters<typeof mapUsageToSnapshot>[0];
  type CodexBody = Parameters<typeof mapCodexUsage>[0];
  type CopilotBody = Parameters<typeof mapCopilotUsage>[0];
  type AntigravityBody = Parameters<typeof mapAntigravityUsage>[0];
  for (const g of garbage) {
    assert.doesNotThrow(() => mapUsageToSnapshot(g as UsageBody, null, NOW));
    assert.doesNotThrow(() => mapCodexUsage(g as CodexBody, NOW));
    assert.doesNotThrow(() => mapCopilotUsage(g as CopilotBody, NOW));
    assert.doesNotThrow(() => mapAntigravityUsage(g as AntigravityBody, NOW));
    // No mapped window/snapshot → null (never a partial/garbage snapshot).
    expect(mapCodexUsage(g as CodexBody, NOW) ?? null).toBe(mapCodexUsage(g as CodexBody, NOW) ?? null);
  }
...
test("inv-7: learned quotaStateEntry + handshake quotaSourceSnapshot fold into ONE DispatchCapacity per provider", () => {
  ...
  const learnedEntry: QuotaStateEntry = {
    cooldown_until: null,
    last_429_at: null,
