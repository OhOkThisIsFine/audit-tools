...
function counterStore(
  dir: string,
  overrides: Partial<LockedJsonStoreOptions<Counter>> = {},
) {
  return createLockedJsonStore<Counter>({
...
test("two stores on different files in the same dir do not block or corrupt each other", async () => {
  await withTempDir(async (dir: string) => {
    ...
    const b = createLockedJsonStore<{ status: string }>({
      path: join(dir, "other.json"),
      lockPath: join(dir, "other.lock"),
      parse: (raw) => (raw as { status: string } | undefined) ?? { status: "pending" },
    });
