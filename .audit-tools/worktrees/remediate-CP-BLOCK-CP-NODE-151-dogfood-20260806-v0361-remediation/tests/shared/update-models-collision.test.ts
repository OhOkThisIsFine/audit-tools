...
type ModelEntry = ReturnType<typeof entry>;
type ApiData = Record<string, { models: Record<string, ModelEntry> }>;
...
interface FlattenResult {
  statics: Record<string, ModelStatic>;
  byProvider: Record<string, Record<string, ModelStatic>>;
  collisions: number;
}
function typedFlatten(apiData: ApiData): FlattenResult {
  const result = flatten(apiData);
  return {
...
describe("flatten — single-provider models stay byte-identical", () => {
  ...
  const { statics, byProvider, collisions } = typedFlatten(api);
