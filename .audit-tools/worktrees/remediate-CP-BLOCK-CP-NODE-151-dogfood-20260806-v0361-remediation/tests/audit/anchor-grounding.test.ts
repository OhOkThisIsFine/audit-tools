...
const { verifyFindingAnchor, combineGroundingWithAnchor, isAllowedAnchorCommand, resolveAnchorTimeoutMs, ANCHOR_TIMEOUT_MS } =
  await import("../../src/audit/validation/anchorGrounding.js");
...
function findingWithAnchor(anchor: ExecutableAnchor | null): Finding {
  return {
    id: "F",
    title: "t",
    category: "c",
...
const fixedRunner =
  (outcome: AllowlistedExecOutcome): AllowlistedExecRunner =>
  async () =>
    outcome;
const throwRunner: AllowlistedExecRunner = async () => {
...
test("combineGroundingWithAnchor: the anchor verdict overrides tier-1 correctly", () => {
  const grounded: FindingGrounding = { status: "grounded" };
  const ungrounded: FindingGrounding = { status: "ungrounded", reason: "no quote" };
