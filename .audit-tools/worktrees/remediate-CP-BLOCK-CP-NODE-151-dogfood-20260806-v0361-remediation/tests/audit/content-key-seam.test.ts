...
import type { ContentKeyInput, IdentityKeyInput } from "../../src/shared/contentKey.js";
...
const baseIdentity: IdentityKeyInput = { unit_id: "u1", lens: "security", pass_id: "p1" };
const sig = buildTaskContentSignature({ goal: "audit auth", scope: ["a.ts"] });
const disc = buildResultContentDiscriminator({ source: "base" });

function makeInput(overrides: Partial<ContentKeyInput> = {}): ContentKeyInput {
  return {
    ...baseIdentity,
...
test("(b) varying any of unit_id/lens/pass_id bumps ALL THREE keys", () => {
  ...
  const fields: Array<keyof IdentityKeyInput> = ["unit_id", "lens", "pass_id"];
  for (const field of fields) {
    const mutated: IdentityKeyInput = { ...baseIdentity, [field]: baseIdentity[field] + "X" };
...
test("(f) missing pass_id or lens throws (fail-3)", () => {
  // @ts-expect-error deliberate contract-violation probe: pass_id omitted to exercise the runtime fail-3 guard.
  assert.throws(() => identityKey({ unit_id: "u1", lens: "security" }));
  // @ts-expect-error deliberate contract-violation probe: lens omitted to exercise the runtime fail-3 guard.
...
test("buildResultContentDiscriminator is tool-owned: enum sources + attempt-keyed redispatch (fail-3)", () => {
  ...
  // @ts-expect-error deliberate contract-violation probe: attempt omitted to exercise the runtime fail-3 guard.
  ...
  // @ts-expect-error deliberate contract-violation probe: an out-of-enum source to exercise the runtime guard.
  ...
  // @ts-expect-error deliberate contract-violation probe: task_id omitted to exercise the runtime fail-3 guard.
