...
const { STALE_LOCK_MS } = await import("../../src/shared/quota/fileLock.js");

/** Narrow a `ClaimResult` to its acquired branch (assertion happened above). */
function requireAcquired(result: ClaimResult): Extract<ClaimResult, { acquired: true }> {
  if (!result.acquired) {
    throw new Error(`expected an acquired claim, got: ${JSON.stringify(result)}`);
  }
  return result;
}
...
test("grants a node to exactly one of two concurrent loops (single-grant)", async () => {
  ...
  try {
    ...
    const winners = attempts.filter(
      (r): r is Extract<ClaimResult, { acquired: true }> => r.acquired,
    );
    const losers = attempts.filter(
      (r): r is Extract<ClaimResult, { acquired: false }> => !r.acquired,
    );
...
test("reclaims a claim once its heartbeat passes STALE_LOCK_MS (stale-reclaim clock)", async () => {
  ...
  try {
    ...
    if (!first.acquired) throw new Error("unreachable: claim just succeeded");
    ...
    if (!second.acquired) throw new Error("unreachable: claim just succeeded");
...
test("never clobbers a re-heartbeated live owner (token-survival)", async () => {
  ...
  try {
    ...
    const token = requireAcquired(claim).ownerToken;
