...
const { prepareDispatchArtifacts } = await import("../../src/audit/cli/dispatch.js");
const { packageRoot } = await import("../../src/audit/cli/paths.js");
const { readOwnerTokens, ownerTokensPath } = await import("../../src/audit/cli/ownerTokens.js");
const { ClaimRegistry, taskClaimsPath } = await import("audit-tools/shared");

const RUN_ID = "test-run-owner-tokens";

type DispatchParams = Parameters<typeof prepareDispatchArtifacts>[0];

function task(id: string, priority: AuditTask["priority"] = "medium"): AuditTask {
  return {
    task_id: id,
    unit_id: `unit-${id}`,
    pass_id: `pass:${id}`,
...
async function writePending(artifactsDir: string, tasks: AuditTask[]): Promise<string> {
...
function run(artifactsDir: string, extra: Partial<DispatchParams> = {}) {
