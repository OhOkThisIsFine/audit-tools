...
function makeResult(
  ...
  if (result.findings.length === 0 && result.reviewed_clean === undefined) {
    result.reviewed_clean = true;
  }
  return result;
}
...
const { cmdMergeAndIngest } = await import("../../src/audit/cli/mergeAndIngestCommand.js");
const { DISPATCH_RESULT_MAP_FILENAME } = await import("../../src/audit/cli/dispatch.js");
...
function runScript(args: string[] = [], cwd: string = process.cwd()) {
...
function minimalValidResult(taskId: string): AuditResult {
  return {
    task_id: taskId,
...
function obsManifestTask(taskId: string): AuditTask {
  return {
    task_id: taskId,
    unit_id: "src/utils/helper.ts",
    pass_id: "pass-1",
