...
const { mergeCriticalFlowFallback, MAX_FALLBACK_FLOWS } = await import(
  "../../src/audit/extractors/flows.js"
);

const deterministicManifest = (): CriticalFlowManifest => ({
  flows: [
    {
...
const flow = (over: Partial<CriticalFlow> = {}): CriticalFlow => ({
...
test("skips invalid host flows without throwing", () => {
  const merged = mergeCriticalFlowFallback(
    { flows: [], fallback_required: true },
    {
      flows: [
        flow(),
        // @ts-expect-error deliberately malformed CriticalFlow (missing required
        // fields) — probes mergeCriticalFlowFallback's runtime schema-validation skip path.
        { id: "flow:bad" /* missing required fields */ },
      ],
    },
  );
