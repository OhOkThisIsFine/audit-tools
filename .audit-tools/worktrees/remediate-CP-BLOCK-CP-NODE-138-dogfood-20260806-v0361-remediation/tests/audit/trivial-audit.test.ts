...
describe("autoCompleteTrivialCoverage skips files where required_lenses is already empty", () => {
  it("already-cleared file is not in skipped; file with lenses is excluded", () => {
    ...
    expect(excluded!.completed_lenses).toEqual([]);
    expect(excluded!.required_lenses).toEqual([]);
    expect(excluded!.classification_status).toBe("excluded_trivial");
    expect(excluded!.unit_ids).toEqual([]);
...
describe("autoCompleteTrivialCoverage respects externalAnalyzerResults signals", () => {
  it("external analyzer signal prevents triviality classification", () => {
    const coverage = {
      files: [
        {
          path: "pkg/__init__.py",
    ...
    const skipped = autoCompleteTrivialCoverage(coverage, lineIndex, externalResults);

    // File must NOT be excluded because external analyzer signal overrides triviality
    expect(skipped.length).toBe(0);
    const file = coverage.files[0];
    expect(file.audit_status).toBe("pending");
    expect(file.required_lenses).toEqual(["security"]);
  });

  it("multiple external analyzer signals all prevent triviality", () => {
    const coverage = {
      files: [
        {
          path: "src/empty.py",
    ...
    const lineIndex = { "src/empty.py": 0 };
    const externalResults = [
      {
        tool: "pylint",
        results: [
    ...
    // File has lineCount=0 which is normally trivial, but external signal prevents it
    expect(skipped.length).toBe(0);
