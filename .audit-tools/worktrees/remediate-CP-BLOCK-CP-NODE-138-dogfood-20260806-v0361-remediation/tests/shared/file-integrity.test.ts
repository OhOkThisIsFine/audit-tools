...
import { checkFileIntegrityRecords } from "../../src/shared/fileIntegrity.js";
...
test("checkFileIntegrityRecords: records with no expected hash are skipped entirely", async () => {
  ...
  const buckets = await checkFileIntegrityRecords({
    ...
    getPath: (r: { path: string; hash?: string }) => r.path,
    getExpectedHash: (r: { path: string; hash?: string }) => r.hash,
    resolveAbsolute: (p) => p,
    exists: () => true,
    hash: async () => {
      hashCalled = true;
