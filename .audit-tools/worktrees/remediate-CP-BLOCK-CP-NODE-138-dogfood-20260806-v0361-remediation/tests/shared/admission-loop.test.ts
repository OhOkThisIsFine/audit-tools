...
import {
  ...
  type AdmissionPool,
  type CapabilityFailOpenInfo,
} from "../../src/shared/dispatch/admissionLoop.js";
import {
  DISPATCH_LEASE_TTL_MS,
  ReservationLedger,
} from "../../src/shared/quota/reservationLedger.js";
import { COLD_START_PROBE_BATCH } from "../../src/shared/quota/scheduler.js";
import { STALE_LOCK_MS } from "../../src/shared/quota/fileLock.js";
import type { DispatchCapacityPoolSummary } from "../../src/shared/quota/capacity.js";
import type { WindowBudget } from "../../src/shared/quota/types.js";
import type { DispatchModelTier } from "../../src/shared/types/stepContract.js";

async function freshLedger(): Promise<ReservationLedger> {
  const dir = await mkdtemp(join(tmpdir(), "admission-loop-"));
  return new ReservationLedger(join(dir, "reservations.json"));
}

function pkt(id: string, cost: number, complexity = 0.5): AdmissionCandidate {
...
test("declared in-flight cap limits a pool by COUNT; overflow spills to the next pool", async () => {
  ...
  const counts = res.granted.reduce<Record<string, number>>(
    (m, g) => ((m[g.pool_id] = (m[g.pool_id] ?? 0) + 1), m),
    {},
  );
...
test("unknown pool capacity is unadmittable rather than an always-fit sentinel", async () => {
  const ledger = await freshLedger();
  const unknown = pool("unknown#a/m", { capacityTokens: null });
  const res = await admitBatch({
    packets: [pkt("p1", 100)],
    pools: [unknown],
    ledger,
...
// Independently declared pools remain cheapest-capable-first. Provider/model
// ordering and failover inside a relay pool are broker-owned.
test("the cheapest capable declared pool wins", async () => {
  const ledger = await freshLedger();
  const cheapSlow = pool("cheap#a/m", { costRank: 0 });
  const priceyFast = pool("fast#a/m", { costRank: 2 });
...
async function ledgerAt(clock: () => number) {
...
test("admissionPoolsFromSummaries: a source pool's own context_cap_tokens outranks the wave's resolved window", () => {
  const base: Omit<DispatchCapacityPoolSummary, "pool_id" | "is_conversation_host"> = {
    slots: 1,
    model: null,
    confidence: "low",
    source: "default",
...
test("capability floor: UNKNOWN capability fails OPEN with a recorded note when banded siblings exist", async () => {
  ...
  const failOpens: CapabilityFailOpenInfo[] = [];
...
test("a MODEL-scoped window throttles its own pool without starving its sibling", async () => {
  ...
  const byPool = res.granted.reduce<Record<string, number>>((acc, g) => {
...
test("END-TO-END: two id'd siblings summarized from ONE credential meter as ONE account", async () => {
  ...
  const summary = (poolId: string): DispatchCapacityPoolSummary => ({
    pool_id: poolId,
    account_key: CRED,
    slots: 4,
    model: poolId,
    rank: "standard",
    confidence: "high",
    // An operator-declared source in sources-declared.json surfaces as
    // `explicit_config` — the LimitSource the contract actually defines.
...
test("admissionPoolsFromSummaries: summary with capability_rank: 2 produces AdmissionPool.capabilityScore === 2", () => {
  const summaries: DispatchCapacityPoolSummary[] = [
    {
      pool_id: "test-pool-1",
      model: "gpt-4",
      rank: "deep",
...
test("capability floor: an unavailable best pool stops holding the floor (survivors become capable)", () => {
  const pools: { id: string; rank?: DispatchModelTier; declaredCapabilityRank?: number | null }[] = [
    { id: "strong", rank: "deep", declaredCapabilityRank: 1 },
    { id: "weak", rank: "small", declaredCapabilityRank: 9 },
  ];
  const deepPacket: { id: string; requiredTier?: DispatchModelTier } = { id: "p1", requiredTier: "deep" };
