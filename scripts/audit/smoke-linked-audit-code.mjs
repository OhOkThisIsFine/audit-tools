  ...
  await withTempRepo(async (root) => {
    ...
    const runNextStep = () =>
      runCommand(auditCodeCommand, ["next-step"], {
        ...
        failureHint:
          "Inspect .audit-tools/audit/steps/current-step.json and rerun with AUDIT_CODE_VERBOSE=1 if the linked wrapper fails earlier than expected.",
      });
    ...
    process.stderr.write(`[smoke:linked] elapsed: next-step until dispatch_review — ${Date.now() - stepStart}ms\n`);
    ...
    process.stderr.write(`[smoke:linked] elapsed: ingest synthetic results — ${Date.now() - stepStart}ms\n`);
    ...
    process.stderr.write(`[smoke:linked] elapsed: next-step until present_report — ${Date.now() - stepStart}ms\n`);
    ...
    process.stderr.write(`[smoke:linked] elapsed: rerun after completion (expect a fresh audit step) — ${Date.now() - stepStart}ms\n`);
  });
  ...
  await runCommand(platformCommand("npm"), ["rm", "--global", "audit-tools"], {
    ...
    process.stderr.write(
      "[smoke:linked] warning: could not remove the global link; run " +
        "`npm rm --global audit-tools` manually: " +
