...
// the release valve: it PROPOSES, it never deletes and it gates nothing.
//
// A candidate is a docs/reviews/*.md whose filename date is older than
// --days (default 14) and which is cited by NOTHING outside docs/reviews/ —
...
//
// Consumed by the nightly doc leg (docs/nightly-routine.md, Leg 1): each
// candidate becomes a retirement proposal in the inbox — an owner decision,
// never an auto-delete.
//
//   node scripts/nightly/review-retirement-candidates.mjs [--days N]
...
function main() {
  ...
  for (const review of reviews) {
    ...
    if (outsideText.includes(name) || memoryText.includes(name)) continue;
  ...
  process.stdout.write(
    `${candidates.length} retirement candidate(s) — dated review docs older than ${days}d that nothing outside ` +
      `docs/reviews/ cites. Propose each for retirement (owner decision); git history keeps the content:\n` +
