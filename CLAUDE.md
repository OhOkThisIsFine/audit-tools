...
**Providers** (`src/shared/providers/`, thin per-orchestrator wrapper at `src/audit/providers/`): `claude-code`, `codex`, `opencode`, `openai-compatible`, `subprocess-template`, `vscode-task`, `antigravity`, `agy`, `worker-command` (runs `task.worker_command`; generic subprocess fallback, not an LLM backend), `claude-worker` (a proxied dispatch-worker class *only* — spawns `claude -p` through a declared `proxy` overlay (any OpenAI-compatible proxy; the tool never depends on a specific implementation, and dispatch works with no proxy installed at all) onto a free backend; architecturally distinct from the other 9, not a drop-in list member: `providerFactory.ts` excludes it from auto-resolution and `assertHostProviderName` rejects it as `--host-provider`/`self.provider`, so it can never be the conversation host driving a run). The other 9 are auto-resolved (`src/shared/providers/providerFactory.ts`); all implement `FreshSessionProvider` from shared. `codex` is headless CLI auto-detected like `claude-code`; `agy` is likewise a headless CLI, auto-detected on `PATH` (the legacy `gemini` binary fallback and its env signal were removed at the 2026-07-18 sunset); `antigravity` is agentic-IDE backend routed through a configured command/task template. `openai-compatible` is NOT a CLI — it's a single-shot, API-driven worker (the `llm write` pattern as a provider): it POSTs the node prompt to any OpenAI-compatible `/chat/completions` endpoint (NVIDIA NIM / vLLM / LM Studio / …), applies the returned `{files,result}` into the node's worktree, and writes the result. Endpoint/model/key are operator-supplied in session config (`openai_compatible.{base_url,model,api_key_env}`) — never hardcoded — so it's a portable, always-available background dispatch pool. Both `openai-compatible` and `claude-worker` run through the same in-process rolling engine for headless autonomy, but at different capability classes: `openai-compatible` is single-shot with no tool loop (permanently capped to self-contained packets), while `claude-worker` is the fuller-capability backend — full Read/Edit/Bash agentic workers, the case `spec/unified-dispatch-worker-model.md` calls out as requiring a proxy transport when a node needs file access on a non-Claude backend.
...
- **Token estimates stay local and deterministic.** Never API-call token counting in planning/dispatch. No tokenizer dep — shared `estimateTokensFromBytes` primitive is the standard. Learned RPM/TPM limits authoritative.
