...
Delegate — do not self-assess; the point is a reader who did not author the plan. Route it to the free
offload lane (`node ~/.claude/llm-call.mjs <alias> "<instruction>" <file...>`; the proxy must be up,
and the session-start guard reports when it is not). Pass the plan plus the evidence files from step 2
as arguments; never paste file bodies into the prompt.
