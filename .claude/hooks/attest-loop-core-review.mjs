...
// attestation after a human performs the adversarial review — it binds the
// review to the exact staged tree (`git write-tree`), so any later restage
// invalidates it and forces a re-review.
//
// It enforces attestation existence + freshness + binding MECHANICALLY; review
// QUALITY is the logged, attributable human step recorded in `--checked` — the
...
for (let i = 0; i < argv.length; i++) {
  ...
  if (a === '--reviewed-by') flags.reviewedBy = argv[++i];
    ...
    console.log(
      'usage: attest-loop-core-review.mjs --reviewed-by <id> --checked "<...>" ' +
        '[--verdict clear|concerns] [--override "<reason>"]',
    );
    process.exit(0);
...
const record = {
  schema_version: 'loop-core-review/v1',
  staged_tree: sha,
