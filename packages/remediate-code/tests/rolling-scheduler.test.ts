...
import type { RemediationBlock, RemediationItemState } from "../src/state/types.js";
import type { CapacityPool, ProviderSlot, SessionConfig } from "@audit-tools/shared";
import { runClosePhase } from "../src/phases/close.js";
...
describe("INV-RS-10 / CE-001 / CE-002: tool-owned final gate command list", () => {
  ...
  it("is INDEPENDENT of plan.test_command (commands are fixed tool-owned argv)", () => {
    ...
    // The shared build runs first, then the full build, then check.
    expect(cmds[0].argv).toEqual(["npm", "run", "build", "-w", "@audit-tools/shared"]);
    expect(cmds[1].argv).toEqual(["npm", "run", "build"]);
    expect(cmds[2].argv).toEqual(["npm", "run", "check"]);
  });
  ...
  it("CE-001: single-flight per package — no package_dir's unit suite appears twice", () => {
    const unit = toolOwnedFinalGateCommands(REPO_ROOT).filter((c) => c.layer === "unit");
    const dirs = unit.map((c) => c.package_dir);
    expect(new Set(dirs).size).toBe(dirs.length);
    expect(dirs.sort()).toEqual([
      "packages/audit-code",
      "packages/remediate-code",
      "packages/shared",
    ]);
  });

  it("CE-001: each package builds at most once (build commands are not per-package repeated)", () => {
    const builds = toolOwnedFinalGateCommands(REPO_ROOT).filter((c) => c.layer === "build");
    // Exactly the shared build + the one whole-workspace build — never a
    // per-package `npm run build -w <pkg>` that would rebuild a package twice.
    expect(builds).toHaveLength(2);
    const perPackageBuilds = builds.filter(
      (b) => b.argv.includes("-w") && b.argv.some((a) => a.startsWith("packages/")),
    );
    expect(perPackageBuilds).toHaveLength(0);
  });
...
describe("INV-RS-10: runToolOwnedFinalGate execution + CE-002 residual", () => {
  it("runs every gate command (injected runner) and passes when all green", async () => {
    ...
    // All six commands ran (build, build, check, 3 unit).
  ...
  it("a vacuous plan.test_command can never substitute: the gate runs its OWN commands", async () => {
    ...
    expect(ran.has("npm run build -w @audit-tools/shared")).toBe(true);
...
describe("CE-003: close.ts force-close guard preserves artifacts when an item is blocked", () => {
  ...
  it("a blocked terminal item keeps fullyGreen false → artifacts dir is preserved (not landed green)", async () => {
    ...
    await new (await import("../src/state/store.js")).StateStore(ARTIFACTS_DIR).saveState(st);
...
describe("atomic-replace: wave-batch shim removed", () => {
  it("packages/remediate-code/src/steps/waveScheduler.ts no longer exists", () => {
    expect(existsSync(join(REPO_ROOT, "packages/remediate-code/src/steps/waveScheduler.ts"))).toBe(false);
  });

  it("the wave-batch single-item step kind is gone from the types union", async () => {
    const typesSrc = await readFile(
      join(REPO_ROOT, "packages/remediate-code/src/steps/types.ts"),
      "utf8",
    );
