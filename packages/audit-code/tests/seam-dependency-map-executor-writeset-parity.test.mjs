...
const EXECUTOR_SOURCE_FILES = [
  join(auditCodeRoot, "src", "orchestrator", "intakeExecutors.ts"),
  join(auditCodeRoot, "src", "orchestrator", "structureExecutors.ts"),
  join(auditCodeRoot, "src", "orchestrator", "planningExecutors.ts"),
  join(auditCodeRoot, "src", "orchestrator", "ingestionExecutors.ts"),
  join(auditCodeRoot, "src", "orchestrator", "synthesisExecutors.ts"),
  join(auditCodeRoot, "src", "orchestrator", "graphEnrichmentExecutor.ts"),
  join(auditCodeRoot, "src", "orchestrator", "intentCheckpointExecutor.ts"),
  join(auditCodeRoot, "src", "orchestrator", "autoFixExecutor.ts"),
  join(auditCodeRoot, "src", "orchestrator", "syntaxResolutionExecutor.ts"),
];
