...
test("io/artifacts.ts ArtifactPayloadMap carries phase-block comments (ARC-dd468422)", async () => {
  const src = await readFile(join(pkgRoot, "src/io/artifacts.ts"), "utf8");
...
test("each package's test framework matches the documented intentional split (ARC-843ce274-2)", async () => {
  async function testScript(pkgDir) {
    const pkg = JSON.parse(await readFile(join(repoRoot, pkgDir, "package.json"), "utf8"));
    return pkg.scripts?.test ?? "";
  }

  const auditScript = await testScript("packages/audit-code");
  const remediateScript = await testScript("packages/remediate-code");
  const sharedScript = await testScript("packages/shared");

  // audit-code: node --test (native node:test runner)
  assert.ok(
    auditScript.includes("node") && auditScript.includes("--test"),
    `audit-code must use node --test; got: ${auditScript}`,
  );
  assert.ok(
    !auditScript.includes("vitest"),
    `audit-code must NOT use vitest (intentional split); got: ${auditScript}`,
  );

  // remediate-code: vitest
  assert.ok(
    remediateScript.includes("vitest"),
    `remediate-code must use vitest; got: ${remediateScript}`,
  );

  // shared: node --test (same as audit-code)
  assert.ok(
    sharedScript.includes("node") && sharedScript.includes("--test"),
    `shared must use node --test; got: ${sharedScript}`,
  );
  assert.ok(
    !sharedScript.includes("vitest"),
    `shared must NOT use vitest; got: ${sharedScript}`,
  );
});
