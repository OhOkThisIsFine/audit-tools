...
} = await import("../src/quota/limits.ts");
const { buildCacheablePrompt } = await import("../src/prompts.ts");
const { filterNewProviders, advancePausedState } = await import(
  "../src/rolling/pausedState.ts"
);
...
test("free_form_intent never emitted verbatim: IntentCheckpoint type-level invariant", () => {
  ...
  /** @type {import('../src/types/intentCheckpoint.ts').IntentCheckpoint} */
  const checkpoint = {
