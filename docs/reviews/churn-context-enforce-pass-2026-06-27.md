...
| E3 | Null `selected_executor` (missing obligation→executor mapping) logged as "configuration gap", not hard-rejected (`nextStep.ts`) | ⚠️ PLAUSIBLE | A compile-time check that every `PRIORITY` obligation id has a registry entry would make it impossible. |
