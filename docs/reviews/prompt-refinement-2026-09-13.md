...
2. Prompts 8 and 9 are owner-approved (2026-09-17) AND implemented, each with its pinning test
   ...
   silent.
3. Rows 8, 9, 9b, 10, 11 and 12 are owner-approved (2026-09-17) AND implemented. Rows 13–20 carry
   lane-drafted PROPOSED specs below and await the owner's verdict. Prompts 15
   (`src/audit/cli/dispatch/hostHandoff.ts`) and 20
   (`src/remediate/steps/dispatch/hostHandoff.ts`) are loop-core, so each needs the design gate and
   a review attestation before it commits.
