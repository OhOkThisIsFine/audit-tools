...
The check (`sharedProviderConfirmation.ts`) detects the right *event* — the reachable set differs
...
`provider_confirmation` obligation at `state.ts` is presence-only and is the sole obligation that
...
`provider_confirmation` to unsatisfied does **not** re-prompt. `nextStepHelpers.ts` consumes
...
(`intakeExecutors.ts`) → `buildSharedProviderConfirmation` folds the newly-reachable backend into
...
|---|---|---|
| **There are TWO artifacts** — shared (`.audit-tools/provider-confirmation.json`, hyphen) vs per-tool seam (`<artifactsDir>/provider_confirmation.json`, underscore) | ✅ | `sharedProviderConfirmation.ts` vs `artifacts.ts`; both written by `intakeExecutors.ts` |
| **Dispatch reads only the SHARED one** | ✅ | stated at `src/audit/orchestrator/providerConfirmation.ts` |
| only the shared artifact carries `roster` / `policy` / `dispatch_bias`; only the seam artifact is in the DAG | ✅ | `sharedProviderConfirmation.ts`; `artifacts.ts` |
| `confirmed_provider_pool` is an inert slot | ✅ | zero producers/consumers; `sessionConfig.ts`, stale comment `providerConfirmation.ts`, 2 type-only tests |
| …and it is **NOT** in `DISPATCH_INVENTORY_FIELDS`, so it survives onto `RepoSessionIntent` | ✅ | `sessionConfig.ts` vs `:679-684` — G2 did not strip it |
| `excluded` has **no** dispatch reader | ✅ | only reader is the in-memory Gate-0 renderer `providerConfirmationStep.ts` |
| dispatch reads the artifact via `model_id` + `cost_order` **only** | ✅ | `resolveConfirmedCostPositions`, `costRank.ts` |
...
| `provider_confirmation` is presence-only, the ONLY obligation bypassing `staleOrSatisfied` | ✅ | `state.ts`, `has` at `:20-22`; contrast `file_disposition` `:94-99` |
...
| `buildSourcePools` is the **shared routing chokepoint** | ✅ | audit `hybridDispatch.ts`; remediate `waveScheduling.ts` |
| **most providers have NO `model_id` at confirmation time** | ✅ | `representativeModelId` (`providerConfirmation.ts`) returns a model only for `openai-compatible` + `codex`; `undefined` for claude-code/agy/opencode/worker-command/subprocess-template. `annotateConfirmedPool:399` spreads conditionally (`...(model ? {model_id: model} : {})`) ⇒ no field at all. `DiscoveredProvider` has no `model_id`. `:249-256`: *"a CLI backend's model arrive[s] only at the dispatch handshake"* |
| **THREE distinct keyspaces, do not conflate** — (1) quota-ledger pool identity `provider[#account]/model` (`quotaPoolKey`, `providers/identity.ts`; `account` is load-bearing for the double-grant boundary, spec `:204`); (2) operator EXCLUSION grammar `provider:model` (spec `:206-208`, owner-approved; account is irrelevant to a rule about a backend); (3) dispatch's confirmed-position lookup on the **bare `model_id`** (`costRank.ts`) — the one the gate's CONFIRMED set is built from | ✅ | nothing forces unification; the `apiPool.ts` comment is stale *about (1)* and is not evidence about (2) |
| **`resolveAmbientSources` is blind to undeclared backends** | ✅ | `auditorSources.ts` iterates ONLY `readSourceDeclaration()` (`~/.audit-code/sources-declared.json`); an installed-on-PATH `codex` is discovered by `discoverProviders` (`sharedProviderConfirmation.ts`) and never by this |
| `provider-confirmation.input.json` is **never deleted** | ✅ | zero `unlink`/`rm` in `src/`; consumed at `nextStepHelpers.ts` |
...
| `autonomous_mode` survives onto `RepoSessionIntent`; only its RESOLVER is remediate-local | ✅ | `types/sessionConfig.ts`, validated `validation/sessionConfig.ts`, absent from `DISPATCH_INVENTORY_FIELDS` `:637-650`; resolver `nextStep.ts` ⇒ lifting gives audit a real flag, NOT dead code |
| `provider_confirmation` is `PRIORITY[0]` | ✅ | `nextStep.ts` — an obligation that never clears is a drain livelock, not a no-op |
...
- **The key = `model_id ?? provider-name`.** Not the bare `model_id`: `representativeModelId`
  (`providerConfirmation.ts`) knows a model only for `openai-compatible` and `codex` — for
...
- **REACH-NOW set** = `discoverProviders(effectiveConfig, env)` ∪
  **`gatherDispatchableSources(effectiveConfig, primaryProviderName)`** — the documented **chokepoint**,
  never `resolveAmbientSources`. `apiPool.ts` states the invariant directly: *"The single async
  ...
  source set)."* `nextStepCommand.ts` already calls it. **Re-deriving reach from `resolveAmbientSources`
  ...
  (`collectDispatchableSources`, `apiPool.ts`):
  ...
  - **the legacy `openai_compatible` fold** (`:454-459`) — it is not in the declaration file.
    ...
    `CLI_PROBES` entry, so `discoverProviders` never sees it"* — is **FALSE**. `providerConfirmation.ts`
...
- **DELTA** = `REACH-NOW \ CONFIRMED` (set difference; a FILTER over fresh reach, never additive — spec
  ...
  synthetic `worker-command` entry (`sharedProviderConfirmation.ts`) and `host_model_cost_order`
...
`probeCommandOnPath` → **`spawnSync("where"/"which")`** (`providerPathGuard.ts`); `discoverProviders`
loops 4 `CLI_PROBES` and agy's `configCommand` calls `commandExists` twice more
(`providerConfirmation.ts`) ⇒ ~6 `spawnSync` per call. `deriveAuditState` is **sync, pure, called
from ~20 sites** — three of them inside the drain loop (`advance.ts`) with
...
⇒ **Compute the DELTA once per invocation at `nextStepCommand.ts`**, stash it on the bundle/ctx,
and let the **pure** predicate read the precomputed field. `state.ts` reads the field; it does not
probe. **Also wire the headless path** — `runProviderConfirmationAutoComplete` (`intakeExecutors.ts`)
...
`:862`. `gatherDispatchableSources` is async-in-signature but sync-in-body (`apiPool.ts` →
...
`providerConfirmation.ts` says *"the operator may reorder"*. The module's `:406-410` doc-comment
...
- DELTA non-empty, **autonomous** → **write the fail-closed exclusion into `policy.exclude` and re-write
  ...
  not incidental:** `provider_confirmation` is `PRIORITY[0]` (`nextStep.ts`), so a delta that recomputes
...
- **2a. Consume-and-invalidate.** Unlink `provider-confirmation.input.json` after promotion and/or stamp it
  with a reach-fingerprint, so a stale input cannot satisfy a new delta (`nextStepHelpers.ts`).
...
- **2c. Delta-only prompt — budgeted, not free.** `renderProviderConfirmationPrompt`
  (`cli/providerConfirmationStep.ts`) takes `{providerPool, sourcePools, inputPath, continueCommand}`
  — no delta channel — and `nextStepCommand.ts` rebuilds `suggested` from scratch each time.
...
`ConfirmedDispatchPolicy.exclude?: ResolvedProviderName[]` (`sharedProviderConfirmation.ts`) becomes a
...
`buildSourcePools`'s `excludedProviders` param (`apiPool.ts`), `hybridDispatch.ts`,
`waveScheduling.ts`, `nextStep.ts`, `nextStepHelpers.ts` — spanning `src/shared/quota/`
(loop-core `:45`), `src/audit/orchestrator/` (`:37`), `src/remediate/steps/nextStep.ts` (`:41`),
`src/remediate/steps/dispatch/` (`:40`).

Also fix the stale `apiPool.ts` comment and reconcile the drifted `"?"` vs `"default"` tail (cosmetic
today — `sourceId()` keys only unverified `DroppedSource.id`, `dispatchSourceKey()` keys verified; disjoint
by construction, `auditorSources.ts`).
...
- **Split the PRODUCER, not the write site.** `buildSharedProviderConfirmation` is the single function
  feeding **both** consumers: persist (`intakeExecutors.ts` → `writeSharedProviderConfirmation`)
  and render (`nextStepCommand.ts` → `suggested.provider_pool` → `renderProviderConfirmationPrompt`,
  ...
  DTO → `nextStepCommand.ts`) and a **persist-builder** (projects to `PersistedPoolEntry[]` →
  `intakeExecutors.ts`). `annotateConfirmedPool` (`:345-428`) stays whole — it is the full-DTO producer
  and needs no split.
...
the `index.ts` re-exports keeping default-mode knip green, and the two type-only tests.
...
**Attestation required.** `LOOP_CORE_PATTERNS` (`loopCorePaths.ts`) hits: `src/audit/orchestrator/`
...
`tests/shared/providerConfirmation.test.mjs` (the type-only slot tests),
`tests/audit/provider-confirmation-gate.test.mjs` + `tests/audit/dc2.test.mjs` (the only
...
`spec/dispatch-quota.md:123-128`, `src/shared/index.ts` (knip).
...
(`nextStepCommand.ts` → `supervisor/sessionConfig.ts`) and remediate reads
`<root>/.remediation-artifacts/session-config.json ?? <root>/session-config.json`
(`nextStep.ts`) — genuinely disjoint, so policy on the intent would not transport audit→remediate.
