...
| "Offload lanes: default to NIM/AGY/Codex, expect timeouts, cheapest model that satisfies the schema" | The offload lane + dispatch ladder in the global `CLAUDE.md`, and the `/the offload router` skill that owns the routing mechanics. |
...
| "Custom Skills — split /start-lap into composable skills" | `ship`, `start-lap`, `design-check`, `disambiguate-backlog`, the offload router already exist as separate invocable skills. |
