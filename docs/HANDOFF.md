...
- **branch `a6-zod-single-source`: A6 IN PROGRESS (full zod single-source migration).** Goal:
  ...
  - **DONE (commits `c243c70`, `d65fede`, `875a397`):** shared `Finding`+subtypes & lens/severity/
    confidence vocab → zod (`packages/shared/src/types/finding.ts`, `lens.ts`, index exports the
    `*Schema`s); audit-code worker contracts `AuditTask`/`AuditVerification`/`AuditResult` → zod
    (`src/types.ts`); `src/contracts/workerSchemas.ts` defines STRICT worker projections; the 5
    worker schemas (`lens`/`finding`/`audit_task`/`audit_result`/`audit_results`) are generated by
    `scripts/generate-schemas.mjs` + drift-guarded by `tests/worker-schema-generation.test.mjs`;
    DAT-db770721 guard rewritten (inline lens enums must equal `LensSchema.options`). Corrected a
    real schema↔type drift: `audit_result.followup_tasks` is `string[]` (was wrongly `AuditTask[]`
    in the hand schema). `coverage_matrix`+`repo_manifest`+`unit_manifest` → zod (`src/types.ts`);
    `coverage_matrix.schema.json` deleted, its test → `CoverageMatrixSchema.parse`,
    `classification-status-drift.test.mjs` deleted (drift now structurally impossible).
  - **REMAINING (the bulk):** convert internal artifacts to zod + their contract tests to
    `Schema.parse`, then DELETE each JSON schema file: surface_manifest, graph_bundle (shared
    graph types), critical_flows, flow_coverage, risk_register, audit_plan_metrics,
    runtime_validation_tasks/report, dispatch_quota (shared quota), blind_spot_register,
    intent_checkpoint (shared), scope, analyzer_capability, external_analyzer_results, audit_state,
    step_contract (shared), task_affinity_graph, agent_reflection (shared), audit_findings (shared
    `AuditFindingsReport` — already zod, decide generate-vs-delete), file_disposition,
    audit-code-v1alpha1. The big interleaved `tests/schema-contracts.test.mjs` "strict schema
    contracts" block validates unit/surface/graph/runtime/plan-metrics together — convert as a
    unit. Then DELETE `tests/helpers/jsonSchemaAssert.mjs` + `auditSchemaRegistry.mjs` once no test
    imports them, and the tests `json-schema-assert.test.mjs` / `seam-schema-validation-single-source.test.mjs`
    (validator-self-tests → obsolete). remediate parity: convert its contract types (RemediationState
    etc.) to zod, delete its (all-unconsumed) `schemas/`, and remove the `PUBLIC_CONTRACT_SCHEMA_COMPANIONS`
    "remember to edit the schema" hack in `phases/plan.ts`. Update `package.json` `files:["schemas/**"]`
    if a package ends with no schemas. Update `cli/dispatch/packetPrompt.ts` $ref wording (schemas now
    self-contained/inlined, no cross-file $ref). Finally: backlog A6 → done, memory, /ship all changed pkgs.
  - Pattern to follow (per cluster, green): type→zod in its module (z.infer keeps the type
    identical so consumers don't change), convert/relocate its contract test to `Schema.parse`,
    delete the now-unconsumed JSON file (+ any obsolete TS↔schema drift guard for it).
