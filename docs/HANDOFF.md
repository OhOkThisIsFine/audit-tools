...
- **branch `a6-zod-single-source`: A6 IN PROGRESS (full zod single-source migration).** Goal:
  ...
  - **DONE — types converted to zod (commits `c243c70` `d65fede` `875a397` `73eadc9` `d5ec46b`
    `a029063`):** shared `Finding`+subtypes & lens/severity/confidence vocab (`shared/src/types/
    finding.ts`,`lens.ts`); audit-code worker `AuditTask`/`AuditVerification`/`AuditResult`
    (`src/types.ts`); `coverage_matrix`/`repo_manifest`/`unit_manifest` (`src/types.ts`); shared
    `surfaces`/`risk`/`disposition`/`graph`; audit-code `runtimeValidation`/`externalAnalyzer`;
    audit-code `reviewPlanning` (ReviewPacket + PacketQuality + AuditPlanMetrics). All `*Schema`s
    exported from barrels. **Worker schemas fully done:** `src/contracts/workerSchemas.ts` (strict
    projections), generated by `scripts/generate-schemas.mjs`, drift-guarded by
    ...
    `classification-status-drift.test.mjs` DELETED (drift structurally impossible).
