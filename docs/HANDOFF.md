...
- **v0.50.19 is live**, published from `f4178677`. The lap changed `src/`, so `dist/` differed and
  ...
  straight after a push fails and must be retried once CI is green. Now an open entry. Both releases
  this lap WAITED on the two push-triggered runs first (`gh run watch --exit-status`), and the gate
  then accepted the SHA on its first attempt each time — waiting is the working recipe, not a retry
  loop.
