...
- **The remediate draw's ingest report still states the repair remedy for a work item that was already ACCEPTED (2026-09-17, medium, friction: tool_should_decide).** The audit draw now picks each issue's section from `SUBMISSION_ISSUE_REMEDY` (`src/shared/submission/hostResultOutcomes.ts`), which answers `none` for `duplicate_submission_id`. The remediate renderers — `src/remediate/steps/nextStep.ts` and `src/remediate/steps/dispatch/hostHandoff.ts` — still split on `code !== "submission_missing"`, so they file that code under "Result status requiring attention" and tell the reader to repair the result and write it again. The item is not pending and the republished workload binds no path for it, so the reader searches for a path that is not there. **Property:** both draws choose an ingest issue's section from the one shared remedy map, never from a negation of the missing code. The map already carries the right answer. Queue items 16–20 of [prompt refinement](../reviews/prompt-refinement-2026-09-13.md) touch these two files.

- **A release re-run after a registry-propagation timeout cuts a SECOND release instead of resuming (2026-09-15, medium, friction: tool_should_decide).** `scripts/release-and-publish.mjs` waited 124 s for `audit-tools@0.51.9` (published, run 35033621077 green) and threw on E404 while npm was still propagating; the re-run read the journal, found HEAD's CI green, and forward-bumped to 0.51.10 — so the registry now carries two identical releases, and the shorter wait is what made the retry necessary. **Property:** a journal whose last phase is `await-npm` for version V resumes THAT wait on re-run (never bumps while V is unresolved), and the propagation wait is sized for observed npm propagation (minutes), separately from the `release`-event delivery wait.
