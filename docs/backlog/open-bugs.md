...
- **The charter layer's code implements the superseded delta-miner design; the spec now describes a five-step pipeline that has no code (2026-09-15, high, friction: tool_should_decide).** `spec/conceptual-design-review-design.md` §"The estimator charters" was rewritten with the owner on 2026-09-15 (decision record `docs/reviews/charter-redesign-feedback-2026-09-15.md`; approved host text in `docs/reviews/prompt-refinement-2026-09-13.md` §8, §9, §9b, §10). The code still has the old shape: `CharterSubmissionSchema` (node list, `files` required, no edges), the file-overlap join in `assembleCharters`, `CharterDeltaSubmissionSchema` + `assembleDeltas` + pair-keyed `DELTA_ROUTES` (all `src/shared/decompose/charterExtraction.ts`), `TriangulatedTelosSchema` and the four-value `CharterClarificationAnswerSchema` (`src/shared/types/charter.ts`), and the `charter_delta_current` obligation. **Property:** each lane submits one `GoalGraph`-shaped DAG with local ids, edge provenance and optional `files`; a `charter_comparison_current` obligation consumes tool-proposed candidates (file overlap + provenance cross-refs) and emits n-ary correspondences and seven-dimension typed differences with a split; a `charter_fidelity_current` obligation runs a quote-grounding pre-check and a separate lane's verdict, and only `supported` differences reach a deterministic discrepancy report; routing keys on `(dimension, relation, split)`; no triangulated telos exists anywhere; the clarification answer is `{governs}` / `rewrite_all` / `leave_open`; dependency-map rows and the `charter_register.json` consumers (`charterClarification`, `systemicChallenge`, blast radius, `renderCharterContext`) read the new artifacts. Loop-core + shared contracts: `/design-check` first.
