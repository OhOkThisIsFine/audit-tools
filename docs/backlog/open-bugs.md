...
- **The generated worker prompt omits `evidence`, which a downstream validator requires (2026-08-21, high, friction: tool_should_decide).** The per-item prompt `buildWorkItem` writes (`src/audit/cli/dispatch/hostHandoff.ts`) enumerates the required finding fields and does NOT list `evidence`, and `FindingSchema` marks it optional — so host-handoff ingestion ACCEPTS a finding without it. `validateEvidence` (`src/audit/validation/auditResults.ts`) then errors on any finding whose evidence is missing or empty. A worker that obeys the tool's own prompt therefore produces a submission that is accepted and then fails validation. Two sibling rules are also unstated in the prompt and enforced later: an affected-file line span must fall inside the file, and `line_start <= line_end`. **Property:** the prompt the tool generates states every field the validators that read its output enforce — one source, not two. Audit-side instance of the same class as the remediate-side entry below.

- **An accepted result that later fails validation wedges the run, and no verb un-accepts it (2026-08-21, high).** Ingestion records results in `host-accepted-results.json` + `-ledger.json` BEFORE the audit-results validation runs; when that validation fails, the whole `next-step` exits 1. Acceptance is idempotent and keyed by `work_item_id`, so rewriting a corrected result file at the bound path changes nothing — the accepted SNAPSHOT is what is re-validated, and the run stays wedged. `validate-results` only pre-checks, `ingest-results` re-ingests, `force-synthesis` skips ahead; none drops an accepted entry. Hit twice on 2026-08-21, the second time with all 495 results accepted at once, and recovered only by hand-clearing both accepted files. **Property:** any state a failed gate can poison has a supported path back out; a hand-edit of run state is the signal that the verb is missing.
...
- **`analyzer_consent` has no per-run "defer" answer, and promotion deletes the policy anyway (2026-08-21, low, friction: tool_should_decide).** The consent step accepts only `granted` / `declined`, both durable, so a delegate advancing a run unattended must record a permanent operator decision merely to proceed. Compounding it, completion's recursive delete of the artifacts dir takes
  <!-- doc-citation-exempt: runtime artifact under the gitignored .audit-tools/ tree, not a tracked file -->
  `analyzer-policy.json` with it, so the cross-run durability the step promises is silently undone and consent is re-asked every lap (sub-case of theme T-004 destroy-before-verify, distinct consequence). **Property:** a run-scoped answer exists, and a recorded decision outlives promotion.
