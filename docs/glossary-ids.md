...
| `CE-` | `CE-<NNN>` | **Counterexample** — a judge-accepted adversarial case the contract pipeline must defend against; the paired invariant must make it impossible. **Not globally unique** — this guard only scans `src/**/*.ts`; `spec/backlog-remediation-design.md` runs its own independent `CE-*`/`FC-*` id family for a different design-history purpose, and a bare `CE-NNN` can also appear as an unrelated local counter in source comments (e.g. `stepBoundaryCapture.ts`, `scheduler.ts`, `clauseInterpreter.ts`). Always resolve a `CE-*` id from its own document/file, never assume cross-document uniqueness. |
...
| `INV-RSM-SPLIT` | Block split preserves verification + phase metadata with field-appropriate semantics — `phase_ordinal` carried unchanged onto every sub-block (a barrier), `targeted_commands` partitioned by relevance (file-specific commands follow their file, generic commands ride everywhere; the union always covers, no silent drop), downstream `dependencies` remapped to ALL sub-blocks. Governs both the finding-granularity splitter and the single-finding FILE-partition splitter. | `src/remediate/phases/plan.ts` |
...
| `INV-PLAN-PERSIST-COMPLETE` | Audit planning persists the WHOLE merged dispatch task list (audit tasks + coverage-deduped requeue fold) as `audit_tasks` before dispatch — the task-affinity graph and plan metrics describe exactly the persisted set, never phantom tasks dispatch (`buildPendingAuditTasks`) can't see. | `src/audit/orchestrator/planningExecutors.ts` |
| `INV-PLAN-FROZEN-ESTIMATES` | Every persisted dispatch task carries provider-neutral `token_estimate` / `risk_estimate` frozen at planning time — the authoritative inputs to JIT packetization/routing. | `src/audit/orchestrator/planningExecutors.ts` |
...
| `INV-READY-STEP-CONTINUATION` | Any current-step written with status `ready` whose stop_condition instructs calling next-step again carries the executable continuation command in `allowed_commands` — the host never reconstructs the invocation from prose. | `src/audit/cli/nextStepCommand.ts` |
| `INV-STATE-PURE-AND-REACHABLE` | `deriveAuditState` stays sync + pure, and every `AuditTopLevelStatus` value is reachable from it — a persisted DC-4 dispatch pause with pending work derives a non-actionable `blocked` obligation (the `dispatch_capacity` id, deliberately outside the PRIORITY scan so the resume path stays actionable). | `src/audit/orchestrator/state.ts` |
| `INV-SCC` | Shared config-and-capacity contract — headless-safe provider auto-resolution (never select a provider whose launch mode deterministically fails), structural capacity on all-oversized pending work (`total_slots >= 1`, defined primary), single-encoding + portable (reserved-stem-escaped, length-bounded, injective) run-id filename derivation, rolling halt-on-partial-terminal at level boundaries, live-holder lock heartbeat (a live holder is never classified stale), and atomic worker file application (a rejected launch leaves the worktree byte-identical). | `src/shared/dispatch/unifiedRolling.ts`, `src/shared/quota/capacity.ts`, `src/shared/quota/fileLock.ts`, `src/shared/io/frictionCapture.ts`, `src/shared/friction/triage.ts`, `src/shared/providers/providerFactory.ts`, `src/shared/providers/openAiCompatibleProvider.ts` |
...
| Id | Meaning | Site |
|---|---|---|
| `INV-S03` | Settled dispatch pools — a spilled-then-exhausted pool is never re-offered on re-discovery; the `SettledExclusionSet` only grows within a run and is never mutated. | `src/shared/rolling/pausedState.ts`, `src/shared/dispatch/settledPools.ts` |
...
| `N-R22` | Redesign: ownership-registry / write-scope dispatch node — gates amended files through the ownership registry (`src/remediate/steps/dispatch/marshal.ts`). |
