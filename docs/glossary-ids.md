...
| `INV-QD` | Quota-driven dispatch — quota headroom is the sole throttle; in-flight token accounting is the single authority (no external concurrency cap). `INV-QD-15`: durability — quota/dispatch state files are written atomically (temp + rename) so a lock-free reader never sees a torn file, and an unusable state file never masquerades as cold start (throw; one loud opt-in degrade; lock-held quarantine-and-heal for corrupt bytes). | `src/shared/dispatch/rollingDispatch.ts`, `src/shared/quota/state.ts` |
