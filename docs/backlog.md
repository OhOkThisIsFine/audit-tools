...
- **Node-context clobber tier — SHIPPED 2026-07-23 (all three items); accepted residuals stay here (low).** Mechanism record: `docs/reviews/node-worktree-guard-mechanisms-2026-07-23.md`. (1) The HIGH shared-state clobber: both CLIs now refuse EVERY subcommand from a node-worktree context (caller cwd, wrapper-propagated `AUDIT_TOOLS_CALLER_CWD`, or raw `--root` inside `.audit-tools/worktrees/<name>`) except a tight worker-safe allowlist — deny by default, so future commands are never silently exposed; remediate's `StateStore` + rolling-session writers additionally assert (defense-in-depth for explicit-`--artifacts-dir`/embedded shapes). Owner-token discrimination REFUTED as a mechanism (driver and worker read the same filesystem; a CLI-flag token is a manual flag = bug signal); forced env redirection REFUTED (host spawns workers, tool can't force env). (2) Dist-dependent verify false-reds: accept-time partition (`partitionDistDependentVerifyCommands`) defers commands whose string or named-test-file CONTENT references `dist/` to the central close gate — the backlog's "build-first" candidate was refuted in recon (a per-node worktree has no `dist/` at all; a central build cannot materialize one). (3) The four accreted worker rules now render as a "Standing rules" section in `implementPrompt`. Residuals (each low, on-evidence only): audit-side writers rely on the CLI-level guard alone (no writer asserts — add if a non-CLI clobber shape ever fires); a worker that both `cd`s out of its worktree AND passes explicit targets can still reach state (containment, not authority — the standing-rules prompt is the remaining layer); a degraded review snapshot falls back to the real root with no cwd signal (`rollingAuditDispatch.ts:253`); deferred dist-dependent commands are subsumed by the close gate's full-suite run rather than individually re-run.
