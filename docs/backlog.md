...
- **The operator's Gate-0 `exclude` decision has NO dispatch effect — exclusion was never wired (G3 recon 2026-07-16, tool-should-decide, HIGH).** `buildSharedProviderConfirmation` computes `excluded` (`sharedProviderConfirmation.ts:258-264`) and persists it, but the only reader of `entry.excluded` in the whole tree is the Gate-0 *renderer* (`providerConfirmationStep.ts:48`), fed in-memory. Dispatch reads the artifact ONLY via `resolveConfirmedCostPositions` (`costRank.ts:299`), which touches `model_id` + `cost_order` and nothing else — and `annotateConfirmedPool:394` assigns a `cost_order` to **every** entry including `excluded: true` ones. So an operator who excludes a provider at Gate-0 sees it rendered "excluded" and it still routes. The whole reach half of the artifact (`capability_tier` / `self_spawn_blocked` / `excluded` / `reason` / `roster`) is write-only for dispatch. Found by the second independent adversarial review after BOTH the author and the first adversary asserted the opposite (that `excluded` was inherited across auditors — it isn't; nothing reads it). Endpoint = G3.
- **Remediate reads the Gate-0 confirmation with an EMPTY session config → cost order + λ silently dead (G3 recon 2026-07-16, tool-should-decide, HIGH — unverified live, verify before fixing).** `marshal.ts:410,416` call `readConfirmedCostPositions` / `readConfirmedDispatchBias` with `waveOptions?.sessionConfig ?? {}`. Both route through `readSharedProviderConfirmation:475`, which re-derives `currentProviderRoster(sessionConfig, env)` and returns `reconfirm` on any mismatch → empty positions map (`:505`) + λ=0 (`:551`). Config-derived providers (`openai_compatible`, `sources[]`) are invisible under `{}`, so remediate's roster should differ from audit's whenever one is configured ⇒ every remediate dispatch silently discards the operator's confirmed cost order AND the cost↔speed dial, degrading to price/tier + cost-first. Degrade is by-design "best-effort", so nothing is loud. Suspected already-live on any NIM-configured run. [[enforce-robustness-in-tooling-not-host-discretion]]
