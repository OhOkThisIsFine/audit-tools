...
- **SPEC — backend identity is ONE function, and it is provider-qualified (raise to MEDIUM-HIGH: this is a
  gate BYPASS, not just a collapsed delta).** The reachability delta keys backends on `model_id ?? provider`
  — the bare model string when known — so two backends from DIFFERENT providers advertising the same model
  collapse into one entry and only one receives an exclusion pattern. **The confirmed set uses that same
  model-only identity**, which is the part the earlier framing understated: confirming one provider's model
  can mark a DIFFERENT provider's identically-named model as confirmed, so a backend the operator never
  saw routes as though approved. The gate exists precisely to stop that.
  This was deferred as "not a local key change — the delta key, the confirmed-set key, and the exclusion
  grammar must all move together, or a provider-qualified delta the confirmed side cannot reproduce
  livelocks the first obligation." That constraint is real, and it is also the diagnosis: **there are three
  hand-maintained answers to "which backend is this?" and they are required to agree.** Two of them
  disagreeing is the bug; a third copy would be a third chance to disagree.
  **Property to hold:** one identity function, provider-qualified, consumed by the delta, the confirmed
  set, and the exclusion matcher alike — so the delta and the confirmed side cannot diverge by
  construction, which is exactly what the livelock requires them to do. The cost is real and should be
  accepted knowingly: the confirmed-side record carries no backend-provider field today, so single-sourcing
  the identity forces that field onto it. That is the work, and it is the right work — same defect class as
  the capability-evidence parser, where a hand-maintained enumeration drifted from its source.
  Made likelier at scale by proxy expansion: several sources behind one transport can serve the same model
  string and still collapse.
