...
- `open-bugs.md:146` — A spec row's category prefix is load-bearing enough to manufacture work — and one was false (2026-07-28, low, TRACED — one owner go outstanding).
- `open-bugs.md:162` — ⬇ LIVE (re-dogfood 2026-07-22, medium): a worker self-reported "valid, verified" on a malformed-JSON result file — result validity must be checked mechanically, never trusted from the worker's claim.
- `open-bugs.md:170` — ⬇ LIVE (re-dogfood 2026-07-22, low): a json_schema-required array elicits FILLER entries from weaker models when the true answer is empty.
...
- `open-bugs.md:206` — LEAD (2026-07-22, low): does remediate's node-claim lifecycle share the merge-only-release defect the audit side just fixed?
- `open-bugs.md:214` — Regenerating the price snapshot INVERTS host tier cost order — the refresh is blocked on the service→vendor-id mapping, not merely followed by it (2026-07-24, medium, ATTEMPTED AND REVERTED).
- `open-bugs.md:240` — LEAD (low): NIM roster latency is bimodal — a slow model can read as a DEAD lane.
- `open-bugs.md:259` — ⬇ LIVE (re-dogfood): token_usage stamping asks for a split real harnesses cannot supply (2026-07-21, low).
- `open-bugs.md:265` — LEAD (re-dogfood): systemic-challenge round counter + banked improvements carry across RUNS (2026-07-21, low).
- `open-bugs.md:270` — Review rounds re-derive the same file map every time (inefficient-feeding, 2026-07-19).
- `open-bugs.md:291` — Window-scope validation at the PRODUCER boundary — designed for step 2, deferred with reason (2026-07-19).
- `open-bugs.md:311` — A per-site pinning gate would make "red-green validated" mechanically checkable — UNBUILT on main.
- `open-bugs.md:331` — SPEC — the proxy catalog's freshness rule gates the WRITE but not the READ, and the lane has no operator-runnable refresh.
- `open-bugs.md:358` — Ranked-pool composition — live-wave watch + the absolute-floor question.
- `open-bugs.md:376` — H2+H4 collapse residual pins (2026-07-18, low, from review h2c3).
- `open-bugs.md:384` — Pre-existing back-compat fold survives, now against standing policy (2026-07-18; re-verified at HEAD 2026-07-26 — NOT low).
- `open-bugs.md:386` — "The free model can't handle reasoning work" is a MYTH built from unset request parameters — check `finish_reason` before diagnosing a model (friction: tool-should-decide, medium-high).
- `open-bugs.md:410` — A nightly item is PRESENTED for an answer without its premise being re-checked, so settled subjects keep getting asked (2026-07-25, medium, friction: inefficient-feeding).
- `open-bugs.md:428` — Friction walk (duplicated-guard lap, 2026-07-25):
- `open-bugs.md:439` — Friction walk (smoke-dedup lap, 2026-07-25):
...
- `open-bugs.md:713` — Twelve settled nightly answers were never executed, and the queue reports them as closed (2026-07-28, medium, tool-should-decide).
- `open-bugs.md:733` — Friction walk (nightly-determinations lap, 2026-07-26):
- `open-bugs.md:759` — Friction walk (contract-sweep producer lap, 2026-07-26):
- `open-bugs.md:776` — Friction walk (inline-api_key retirement lap, 2026-07-26):
- `open-bugs.md:788` — Friction walk (touched_files load-gate lap, 2026-07-25):
- `open-bugs.md:802` — Friction walk (fourth backlog-clearance lap, 2026-07-24):
- `open-bugs.md:823` — Friction walk (second backlog-clearance lap, 2026-07-24):
- `open-bugs.md:835` — ▶ ⬇ LIVE-run watch ONLY — the per-node token estimate is WIRED (2026-07-25, loop-core).
- `open-bugs.md:846` — Remediation must never switch the primary checkout off its base branch (2026-07-22, medium; product fix planned).
- `open-bugs.md:869` — "Delegate the rolling loop" dispatcher pattern breaks on notification routing (2026-07-11 live run, tool-should-decide, medium).
- `open-bugs.md:890` — NIM in-process worker: one packet failed with "empty completion (no choices[0].message.content)" (2026-07-11 live run, watch).
- `open-bugs.md:894` — Abandoned HOST-path grants hold reservation leases to the 20-min TTL, walling a fresh grant (2026-07-11 live run, low — backstop works; not a release bug).
- `open-bugs.md:916` — A2b unmatched-quota fallback — two residuals (each low, documented at the code site).
- `open-bugs.md:929` — Design (remove-waves track): dispatch should be gated ONLY by token-budget, rate, and true task-unlocks — the host merge/re-grant barrier is artificial for independent review packets (2026-07-11 live run, owner design statement, forward-track).
- `open-bugs.md:948` — Host fan-out quota gate — residual: AD-HOC host Agent spawns sit outside every ledger (re-verified 2026-07-24, low, [[host-fanout-quota-gate]]).
- `open-bugs.md:957` — Design-review independence — the solo contract branch is pinned by a shared helper, not by a test (2026-07-24, low).
- `open-bugs.md:974` — Untracked-exclusion scope rule — residuals only (each low-severity, documented at the code site).
- `open-bugs.md:1000` — External shared-logic audit V1–V7 residuals
- `open-bugs.md:1017` — Top gate optimization lead — both packaged smokes REBUILD the identical package (measured 2026-07-06).
- `open-bugs.md:1031` — Dispatch admission-control rework — two residuals (env-bound / architectural, not blocking).
- `open-bugs.md:1053` — Quota-aware dispatch — live validation env-bound.
- `open-bugs.md:1068` — SPEC — a ledger-blocked retry must back off, reusing the ONE backoff the project already owns.
- `open-bugs.md:1082` — Friction detection — M-QUOTA escalation chain: remediate-side friction assertion missing; live validation env-bound.
- `open-bugs.md:1101` — Selective-deepening convergence — live validation env-bound.
- `open-bugs.md:1123` — A design-review pass can auto-complete EMPTY, and nothing distinguishes that from a real review finding nothing.
- `open-bugs.md:1131` — `goal_id` is read verbatim off the LLM envelope, so its format is unvalidated (re-verified at HEAD 2026-07-25).
