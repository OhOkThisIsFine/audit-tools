...
- **Remediator must mechanically decompose + boundary-enforce arbitrary multi-goal scope — stop forcing the host to phase by hand (Ethan, 2026-06-24, VERIFIED recurring).** When `/remediate-code` is pointed at a large multi-item input (e.g. the whole backlog), the contract pipeline produces a correct reconciled design but then expects the host to execute all tracks as ONE run; the independent design-critique repeatedly returns *blocking over-scoping* and the host has to manually re-scope to a phase. This keeps happening and shouldn't — it is the tool's core job, not the host's. The remediator must, by construction: (1) break an arbitrary number of goals/changes into well-defined, well-bounded tasks; (2) **strongly define the boundaries between tasks and write boundary tests that mechanically enforce them** (not prose `seam_adjustments` notes — review concern C-002: edit-order DAGs asserted in prose over shared files like `staleness.ts`/`dispatch.ts` are a host-discretion anti-pattern + latent merge-break); (3) separate the bounded tasks into **parallel work units with mechanical scheduling dependencies** (block A blocks-on block B) so the wave scheduler honors ordering without the host remembering; (4) derive phasing itself (foundations → consumers → review/slivers) from the dependency DAG rather than emitting one monolithic run the critique then rejects. Generalizes *enforce-robustness-in-tooling, never host discretion* and the *no monolithic change* / failure-isolation principles. Consumer modules (F1/F3/F4/F5/F6) shipped 0.30.5; the **foundations O1/O2/O3 are also shipped** (merged `cd089066`, content-key seam + append-only idempotent ledger + friction triage + repair seam, with tests — verified 2026-06-25); the prior "foundations remain unshipped" claim was stale handoff drift. The rolling-dispatch same-file merge-serialization fix (a)+(c) shipped 2026-06-25 (file-ownership-disjoint scheduling + cross-node seam-signature guard; (b) no-op-satisfied was already shipped). The full module map, reconciled seams, and verified design invariants (CE-001…006, FC blocking concerns) are the canonical design doc [`backlog-remediation-design.md`](backlog-remediation-design.md).
  ...
  - **Auto-phasing — derivation + persistence + ordinal threading + scheduler barrier SHIPPED 2026-06-27; one sliver remains.** The tool derives the foundations→consumers phase cut itself, PERSISTS it, threads it into every block, and ENFORCES it in the scheduler — no host re-scope at intake, ordering guaranteed end-to-end. (1) **Derivation** (shipped 2026-06-26): pure `derivePhaseCut` (`phaseCut.ts`) — module tier = longest dependency chain (foundations = 0), deterministic, cycle-safe, out-of-scope edges ignored; now also exposes a `module_phase` (name→ordinal) map. (2) **Persistence**: `src/remediate/contractPipeline/phaseCutArtifact.ts` — `ensurePhaseCutArtifact` derives from `finalized_module_contracts` and writes the first-class sidecar `intake/contract/phase_cut.json`, single-sourced for BOTH the critique render and the DAG promotion (idempotent; byte-identical for the same DAG). Deliberately NOT a `CP_ARTIFACT_NAMES` member — it is re-derivable on demand, so it carries no staleness envelope and never perturbs LLM-phase sequencing. `envelopePayload` was lifted into `artifactStore.ts` for single-sourcing. (3) **Ordinal threading**: `promoteImplementationDagToExtractedPlan` stamps each block a `phase_ordinal` (new optional field on `RemediationBlockSchema`) derived MECHANICALLY from the union of its node's obligations — mapped to the owning module by the `OBL-<moduleSlug>-…` id fragment (`phaseOrdinalForObligations`, max ordinal = fail-toward-later; CE-only/unmatched → last phase). Never trusts a worker-carried field (a node merge could drop it). (4) **Scheduler barrier** (INV-PHASE-01, `rollingDependencyLevels`): a block at phase p enters a dispatch level only once every lower-phase block is VERIFIED-complete (resolved, not merely placed) — so a planning pass emits only the lowest unfinished phase, foundations land + per-node verify, the next pass opens the next phase; a lower phase that can never verify (skipped/blocked) dead-ends its consumers. Absent ordinals collapse to phase 0 (no barrier, pre-auto-phasing behaviour). Tests: `phase-cut.test.ts` (module_phase / `phaseOrdinalForObligations` / sidecar persist+idempotence) + `rolling-scheduler.test.ts` (INV-PHASE-01: only-lowest-phase, next-phase-opens-on-verify, same-phase-parallel, skipped-foundation-dead-ends, ordinal-free-collapse). (5) **Per-phase boundary gate — SHIPPED 2026-06-27 (the last sliver; T3 now COMPLETE).** A whole-repo test-suite gate runs AT each phase boundary. `phaseBoundaryToGate(state)` is a PURE, reblock-safe predicate (returns the phase about to dispatch when it is P>0 and at its UNTOUCHED entry — every phase-P block still all-`pending`; null otherwise) so the gate fires exactly once as foundations→consumers crosses into P, never on P's later intra-phase levels, and re-fires only if a coarse re-block reopens the lower phases. `runPhaseBoundaryGate` is interposed in the `implementing` obligation BEFORE `buildImplementDispatchStep` (the single chokepoint across host-subagent / in-process / hybrid drivers, since one `implementing` pass dispatches exactly one phase). It reuses the all-terminal gate verbatim — `runToolOwnedFinalGate` + `applyCoarseReblock` + the shared `final-gate.json` sidecar (INV-RS-09 coarse-reblock + CE-003 bounded auto-terminate), single-sourced via the new `finalGateDisabled(options)` helper — so a red foundations phase is caught and ATTRIBUTED to that phase before consumers build on it (strictly earlier + more attributable than the close gate's whole-run-unattributable red), and a no-human host converges deterministically. `scoped_out` (non-audit-tools target) never blocks. Tests in `rolling-scheduler.test.ts`. (Ethan, 2026-06-27.)
