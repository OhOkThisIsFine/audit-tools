...
- **`.gitignore` `**/friction/` swallowed the SOURCE dir `src/shared/friction/` — FIXED 2026-06-26 (commit on branch), narrowed to `.audit-tools/**/friction/`.** The runtime-capture-dir ignore was unanchored so it matched the source tree; already-tracked files survived but new source files were silently un-addable. This is the same class as the 0.30.13 "visibility-conditional gitignore hardening" item: ignore patterns for runtime artifact dirs must be ANCHORED to `.audit-tools/` (or the artifact root), never bare `**/<name>/`, so they can't shadow same-named source dirs. Audit the rest of `.gitignore` for other unanchored `**/<name>/` rules that could shadow source.
