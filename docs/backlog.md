...
- **Accept-latch + rollback family (2026-07-23 dogfood resume, tool-should-decide, HIGH; full evidence in `docs/reviews/remediate-dogfood-completion-2026-07-23.md`).** Four coupled defects in the accept-failure path, each observed live: (a) a FAILED accept latches the node as "accepted" in the rolling session, so a retry accept is an idempotent no-op and fix-forward must hand-land from the quarantine ref; (b) the failed-accept rollback restored the branch to the session-recorded base, silently DROPPING a sibling's landed commit (re-cherry-picked by hand); (c) the rollback deregisters/empties the node worktree (un-landed work now survives via the v0.34.15 guard, but the deregistration still strands the fix-forward worker in a hollow dir where git falls through to the main repo); (d) the merge after a rolled-back accept mislabeled the node `resolved_no_change` (false-signal family). Fix direction: the accept lifecycle records a FAILED accept as retryable (never latched-accepted), the rollback target is the tip AT ACCEPT START (never the session base), and the disposition after a rollback names the gate failure.
- **Build-free accept verify false-reds dist-importing targeted commands (2026-07-23 dogfood resume, tool-should-decide, medium).** Three accepts in a row failed verify solely on suites that import/spawn `dist/` (linux-cycle-regression, config-error-handling, wrapper e2es) — deterministic false-reds under the by-design build-free gate (CE-001), each hand-validated by rebuild + rerun green, then quarantine-landed. Fix: classify verify commands at PLAN time (the worker already knows and can declare dist-dependence); a dist-dependent command either triggers the single-flight central build before the gate or is refused at plan time — never a run-time accept failure.
- **Worker-prompt rules accreted by the dogfood live in the DRIVER's prompts, not the tool's template (2026-07-23, tool-should-decide, medium).** Four rules were added one live failure at a time: no whole-dir sweeps; no `remediate-code`/`audit-code` CLI against the shared artifacts dir; new id families must be glossary-registered + declared via `amended_files`; dist-importing verify commands must be flagged. All four belong in `implementPrompt`'s generated worker prompt (enforce-in-tooling) so any host's workers get them.
- **A worker's stray CLI invocation against the SHARED artifacts dir clobbered live run state — shared-state mutation must be mechanically impossible from a node context (2026-07-22 dogfood resume, tool-should-decide, HIGH).** Mid-worker, `rolling-session.json` was rewritten to an empty frontier and a premature merge falsely blocked the in-flight node (its result landed seconds later): the evidence fits the node's own worker (an agentic subagent whose module under test IS the state machine) running `remediate-code next-step`/`merge-implement-results` against the real `.audit-tools/remediation` instead of a hermetic fixture dir. Prompt-level "don't do that" is host discretion. Candidate mechanisms: (a) state-mutating CLI commands refuse to run when CWD is inside a node worktree (`.audit-tools/worktrees/*` — cheap, catches the common case); (b) the rolling session/state writers require the driver's owner token (already minted per claim) so a tokenless invocation is rejected; (c) `AUDIT_CODE_STATE_DIR`-style redirection forced ON for worker-spawned processes. The un-landed-work worktree guard (v0.34.15) contains the blast radius but does not prevent the clobber. ⬇ Recovery protocol that worked: park result file → triage-retry the node → next-step rebuilds the session/frontier → restore result → accept-node.
...
- **SHIPPED 2026-07-22 (v0.34.13 — tagged v0.34.12 in an earlier failed CI attempt; `929dfd18` + `f1c50dbc` + `842642a8`) — the implement-dispatch
