...
- **Recurrence #6 (2026-07-18): vitest exited 0 while reporting "3 failed" (low).** Sixth observed instance of the false-green defect; caught only because the summary counts were read rather than the exit code. Reinforces the two standing test-infra fixes (false-green + the vitest ledger recording no pass/fail outcome).
...
- **Release gate: add `check:doc-manifest` to the pre-commit hook (open remainder, medium).** The durable lesson — a lap cannot report green on evidence weaker than what CI runs; end a lap by checking CI on `main` (the per-workflow runs endpoint is the reliable one), and run `npm run verify:release` before any "this is shippable" claim — is homed in `docs/HANDOFF.md` → "Release gate — the durable lesson" + [[lap-green-must-match-ci-evidence]]. Sole open action: `.claude/hooks/pre-commit-gate.mjs` gates only `npm run check`, so consider adding `check:doc-manifest` (~2s, and it is the check that fired on EVERY push) to it. [[enforce-robustness-in-tooling-not-host-discretion]] **Billed once, 2026-07-18** (a new dated plan doc committed fine locally and blocked `verify:release` afterwards).
- **H3 SHIPPED 2026-07-17; residuals CLOSED by the H2+H4 collapse (2026-07-18).** Residual (a)
  (DEMOTABLE lacked `agy`) dissolved — the set is deleted; an attended agy primary now folds as a
  member pool (D4 synthesis). Residual (b) (remediate `commandWorkers:true`) DECIDED as standing
  policy — command-shaped primaries fold as pools on the remediate draw only, threaded through the
  D1 dedup survivor rule. [[auditor-remediator-mirroring-is-common-logic]]
