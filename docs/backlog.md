...
- `open-bugs.md:9` — `writeOpenItems` accepts an item with no `subject_key` and persists it; the refusal lands two steps later in the HANDOFF generator (2026-08-19, low, friction: tool_should_decide).
- `open-bugs.md:19` — `HOST_GATE_DESCRIPTORS` / `HOST_GATE_KINDS` is a write-only registry — completed by P25, read by nothing in production (2026-08-19, low, owner-decided: DELETE).
- `open-bugs.md:27` — Modularity refinement is superlinear on one large component and unpinned at scale (2026-08-19, low).
- `open-bugs.md:33` — The TASK draw's coherence eligibility is still disjunctive and has never been measured for collapse (2026-08-19, medium).
- `open-bugs.md:40` — The HANDOFF empty-queue projection contract is full-suite-only, so the commit gates pass a red against it (2026-08-18, low, friction).
- `open-bugs.md:48` — `runCommand` buffers child output unboundedly (2026-08-13, medium).
- `open-bugs.md:55` — `shell-trap-guard` misses `git stash push <pathspec>` eating uncommitted work (2026-08-12, medium).
- `open-bugs.md:61` — Contract-pipeline fan-out names a mechanism the host may not have (2026-08-08, medium).
- `open-bugs.md:69` — Diff-based re-review loses the verdict it must diff against (2026-08-08, low).
- `open-bugs.md:75` — `free_form_intent` clause splitter shreds prose on bare `;` (2026-08-08, low).
- `open-bugs.md:79` — Answering an intake question at the checkpoint does not clear `open_questions` (2026-08-08, low).
- `open-bugs.md:84` — Sweep the test tree for tests that re-implement their subject (2026-08-08, medium).
- `open-bugs.md:93` — Regex-perf triage tail from the analyzer sweep (2026-08-07, low).
- `open-bugs.md:100` — Vitest worker RPC starvation — the false-RED exit is CLOSED at the gate; the >60s blocking worker is unlocated (recharacterized 2026-08-07; was "full-suite exits 1 while every test passes", 2026-08-06).
- `open-bugs.md:119` — Remediation pause/recovery is not durable (2026-08-03, medium).
- `open-bugs.md:127` — Graph heuristics are promoted to findings without a semantic lead boundary (2026-08-03, medium).
- `open-bugs.md:134` — Phase-boundary gate false abandonment (2026-07-30, HIGH).
- `open-bugs.md:145` — Contract-type coverage is derived from where TESTS live, not from the contract (2026-07-25, low, friction: inefficient-feeding).
- `open-bugs.md:154` — Backlog prose paraphrased an incident in a way that INVERTED its mechanism, costing a wrong implementation (2026-07-24, medium, friction: ambiguous-direction).
- `open-bugs.md:164` — DD-9 + charter slice-staleness — residual only, revisit on live evidence (2026-07-23, low, accepted).
- `open-bugs.md:189` — A spec row's category prefix is load-bearing enough to manufacture work — and one was false (2026-07-28, low, RESOLVED; the open half is the class).
- `open-bugs.md:200` — ⬇ Live-run watch (re-dogfood 2026-07-22, medium): a worker self-reported "valid, verified" on a malformed-JSON result file — result validity must be checked mechanically, never trusted from the worker's claim.
- `open-bugs.md:208` — ⬇ Live-run watch (re-dogfood 2026-07-22, low, medium-difficulty — an ATTEMPTED fix was reverted 2026-07-25): completion cleanup removes the friction dir before the session stop-gate's close-out walk runs against it.
- `open-bugs.md:226` — LEAD (re-dogfood): systemic-challenge round counter + banked improvements carry across RUNS (2026-07-21, low).
- `open-bugs.md:231` — Review rounds re-derive the same file map every time (inefficient-feeding, 2026-07-19).
- `open-bugs.md:252` — A per-site pinning gate would make "red-green validated" mechanically checkable — UNBUILT on main.
- `open-bugs.md:272` — Friction walk (determinations-execution lap, 2026-07-29):
- `open-bugs.md:286` — Friction walk (duplicated-guard lap, 2026-07-25):
- `open-bugs.md:298` — Implementation workers are never given the contract they must satisfy (2026-08-09, high).
- `open-bugs.md:308` — A delegated step prompt can turn its executor into a second driver (2026-07-16, tool-should-decide, medium).
- `open-bugs.md:324` — Self-audit dogfood loop: fixing the tool mid-run invalidates the run (2026-07-16, ambiguous-direction, low-medium).
- `open-bugs.md:340` — A stale-artifact re-extraction `next-step` runs >2min with no progress signal, silently blowing a caller timeout (live dogfood 2026-07-17, inefficient-feeding, low).
- `open-bugs.md:342` — Friction walk (niggle-fix lap, 2026-08-07):
- `open-bugs.md:358` — Friction walk (loop-core `.ts`-conversion tranche lap, 2026-07-28):
- `open-bugs.md:370` — Friction walk (nightly-determinations lap, 2026-07-26):
- `open-bugs.md:382` — Friction walk (contract-sweep producer lap, 2026-07-26):
- `open-bugs.md:394` — Friction walk (touched_files load-gate lap, 2026-07-25):
- `open-bugs.md:400` — Friction walk (fourth backlog-clearance lap, 2026-07-24):
- `open-bugs.md:413` — Friction walk (second backlog-clearance lap, 2026-07-24):
- `open-bugs.md:422` — Untracked-exclusion scope rule — residuals only (each low-severity, documented at the code site).
- `open-bugs.md:448` — External shared-logic audit V1–V7 residuals
- `open-bugs.md:460` — Top gate optimization — the suite-side tail is subprocess wall, not isolation overhead (measured 2026-07-06).
- `open-bugs.md:466` — Selective-deepening convergence — live validation env-bound.
- `open-bugs.md:474` — `goal_id` is read verbatim off the LLM envelope, so its format is unvalidated (re-verified at HEAD 2026-07-25).
- `open-bugs.md:483` — `StepArtifactSchema` is `.strict()` but `writeStepContract` injects `agent_id`.
...
- `open-bugs.md:503` — `ensure` writes opencode.json with unstable key order.
- `open-bugs.md:508` — Two run-id notions; friction record keyed both ways.
- `open-bugs.md:514` — Auditor severity calibration: 0 of 9 self-audit criticals survived mechanism verification (2026-08-06, lead, low).
- `open-bugs.md:519` — `hostInputPause.ts` says analyzer consent lives in session config; it lives in `.audit-tools/audit/analyzer-policy.json` (2026-08-12, nightly, low).
- `open-bugs.md:528` — `writeOpenItems` reads `subject_key` but never computes or requires it; the HANDOFF generator hard-requires it (2026-08-14, nightly, low).
- `open-bugs.md:538` — `check:memory-citations` cannot see a `[[name]]` cross-link, and 4 are already dangling (2026-08-14, nightly, low).
- `open-bugs.md:549` — Steward verification metadata is undeliverable through the host-result envelope (hit 2026-08-18).
- `open-bugs.md:559` — The report renderer emits control characters from finding prose raw (hit 2026-08-18).
- `open-bugs.md:566` — remediate-code step prompts drift from the validators that read their output (2026-08-19, low, friction: tool_should_decide).
- `open-bugs.md:579` — The commit gate's doc-contract leg did not run check:doc-code-citations for a staged docs/backlog/durable-traps.md (2026-08-19, low) — verified NOT a trigger-set gap; the underlying premise dissolves on inspection.
