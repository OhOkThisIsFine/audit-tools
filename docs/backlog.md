...
- `open-bugs.md:27` — Coherence components have no granularity control, so a whole audit collapses into one work block (2026-08-13, high).
- `open-bugs.md:41` — The HANDOFF empty-queue projection contract is full-suite-only, so the commit gates pass a red against it (2026-08-18, low, friction).
- `open-bugs.md:49` — `runCommand` buffers child output unboundedly (2026-08-13, medium).
- `open-bugs.md:56` — `shell-trap-guard` misses `git stash push <pathspec>` eating uncommitted work (2026-08-12, medium).
- `open-bugs.md:62` — Contract-pipeline fan-out names a mechanism the host may not have (2026-08-08, medium).
- `open-bugs.md:70` — Diff-based re-review loses the verdict it must diff against (2026-08-08, low).
- `open-bugs.md:76` — `free_form_intent` clause splitter shreds prose on bare `;` (2026-08-08, low).
- `open-bugs.md:80` — Answering an intake question at the checkpoint does not clear `open_questions` (2026-08-08, low).
- `open-bugs.md:85` — Sweep the test tree for tests that re-implement their subject (2026-08-08, medium).
- `open-bugs.md:94` — Regex-perf triage tail from the analyzer sweep (2026-08-07, low).
- `open-bugs.md:101` — Vitest worker RPC starvation — the false-RED exit is CLOSED at the gate; the >60s blocking worker is unlocated (recharacterized 2026-08-07; was "full-suite exits 1 while every test passes", 2026-08-06).
- `open-bugs.md:120` — Remediation pause/recovery is not durable (2026-08-03, medium).
- `open-bugs.md:128` — Graph heuristics are promoted to findings without a semantic lead boundary (2026-08-03, medium).
- `open-bugs.md:135` — Phase-boundary gate false abandonment (2026-07-30, HIGH).
- `open-bugs.md:146` — Contract-type coverage is derived from where TESTS live, not from the contract (2026-07-25, low, friction: inefficient-feeding).
- `open-bugs.md:155` — Backlog prose paraphrased an incident in a way that INVERTED its mechanism, costing a wrong implementation (2026-07-24, medium, friction: ambiguous-direction).
...
- `open-bugs.md:201` — ⬇ Live-run watch (re-dogfood 2026-07-22, medium): a worker self-reported "valid, verified" on a malformed-JSON result file — result validity must be checked mechanically, never trusted from the worker's claim.
- `open-bugs.md:209` — ⬇ Live-run watch (re-dogfood 2026-07-22, low, medium-difficulty — an ATTEMPTED fix was reverted 2026-07-25): completion cleanup removes the friction dir before the session stop-gate's close-out walk runs against it.
- `open-bugs.md:227` — LEAD (re-dogfood): systemic-challenge round counter + banked improvements carry across RUNS (2026-07-21, low).
- `open-bugs.md:232` — Review rounds re-derive the same file map every time (inefficient-feeding, 2026-07-19).
- `open-bugs.md:253` — A per-site pinning gate would make "red-green validated" mechanically checkable — UNBUILT on main.
- `open-bugs.md:273` — Friction walk (determinations-execution lap, 2026-07-29):
- `open-bugs.md:287` — Friction walk (duplicated-guard lap, 2026-07-25):
- `open-bugs.md:299` — Implementation workers are never given the contract they must satisfy (2026-08-09, high).
- `open-bugs.md:309` — A delegated step prompt can turn its executor into a second driver (2026-07-16, tool-should-decide, medium).
- `open-bugs.md:325` — Self-audit dogfood loop: fixing the tool mid-run invalidates the run (2026-07-16, ambiguous-direction, low-medium).
- `open-bugs.md:341` — A stale-artifact re-extraction `next-step` runs >2min with no progress signal, silently blowing a caller timeout (live dogfood 2026-07-17, inefficient-feeding, low).
- `open-bugs.md:343` — Friction walk (niggle-fix lap, 2026-08-07):
- `open-bugs.md:359` — Friction walk (loop-core `.ts`-conversion tranche lap, 2026-07-28):
- `open-bugs.md:371` — Friction walk (nightly-determinations lap, 2026-07-26):
- `open-bugs.md:383` — Friction walk (contract-sweep producer lap, 2026-07-26):
- `open-bugs.md:395` — Friction walk (touched_files load-gate lap, 2026-07-25):
- `open-bugs.md:401` — Friction walk (fourth backlog-clearance lap, 2026-07-24):
- `open-bugs.md:414` — Friction walk (second backlog-clearance lap, 2026-07-24):
- `open-bugs.md:423` — Untracked-exclusion scope rule — residuals only (each low-severity, documented at the code site).
...
- `open-bugs.md:475` — `goal_id` is read verbatim off the LLM envelope, so its format is unvalidated (re-verified at HEAD 2026-07-25).
- `open-bugs.md:484` — `StepArtifactSchema` is `.strict()` but `writeStepContract` injects `agent_id`.
- `open-bugs.md:490` — systemic_challenge findings ids are adversary-invented and round-colliding.
- `open-bugs.md:497` — The systemic_challenge loop has no ceiling — its only exit is a dry signal the host may have to fabricate.
- `open-bugs.md:504` — `ensure` writes opencode.json with unstable key order.
- `open-bugs.md:509` — Two run-id notions; friction record keyed both ways.
- `open-bugs.md:515` — Auditor severity calibration: 0 of 9 self-audit criticals survived mechanism verification (2026-08-06, lead, low).
- `open-bugs.md:520` — `hostInputPause.ts` says analyzer consent lives in session config; it lives in `.audit-tools/audit/analyzer-policy.json` (2026-08-12, nightly, low).
- `open-bugs.md:529` — `writeOpenItems` reads `subject_key` but never computes or requires it; the HANDOFF generator hard-requires it (2026-08-14, nightly, low).
