...
- **Machine-global `~/.audit-code` state leaks into EVERY e2e test — with a proxy declared + live, 10 files / 31 tests time out on a clean HEAD (upgraded 2026-07-17 from the smoke:linked-audit-code entry, tool-should-decide, MEDIUM-HIGH).** Proven by bisection this lap: `audit-code-wrapper` "explain a resolved task id" times out at 120s on clean `main` with `~/.audit-code/sources-declared.json` present (live repair-proxy declared) and passes instantly with it set aside — same for the whole failing set (`audit-code-completion`, four `next-step*` files, `quota-command`, plus the three known flakes). Every confirmation-absent CLI invocation in a temp-repo e2e resolves the BOX's proxy declaration → populate + claude-worker pools → live-proxy-dependent behavior and per-invocation network cost (measured 5.6s populate with probes; a 10-min freshness TTL now bounds it, shipped this lap, but does not remove the dependence). The original smoke:linked-audit-code quota-ledger case is the same class. Property to hold: no test/smoke outcome may depend on the box's live `~/.audit-code` (declaration, catalog cache, quota ledger) — add an env override for the state dir (`resolveProxyCatalogPath`, `readSourceDeclaration`, quota-state dir all honor it) and have the e2e harnesses set it to a per-test temp dir. Until then: full-suite verification on a box with a declared proxy requires setting `sources-declared.json` aside (restore after!), and CI is unaffected (fresh clones have no declaration).
