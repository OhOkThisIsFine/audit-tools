...
- **Make the loop cheaper — SELF-SCALING pipeline** (design of record: [`self-scaling-pipeline-design.md`](../spec/self-scaling-pipeline-design.md)). One pipeline self-scales via two dials on a shared intake risk/complexity signal: (A) **adversarial depth** — critique/counterexample scale from an inline light self-check (low) to full independent sub-agents (high), floor = LIGHT never zero (remediation re-finds audit errors); (B) **granularity / round-trips** — degenerate phases collapse by structure, round-trip count = f(complexity), optimistic-start + escalate-on-evidence. Signal computed from intake-available data only (affected_files + configurable path-risk patterns + intent; never a pipeline output — the circularity), fail toward MORE scrutiny when uncertain. Whole-repo green gate is never scaled. **Open:** **slice 4b** (granularity collapse: round-trip count = f(complexity); collapse coherent authoring phases into fewer round-trips for low-tier work; keep fine-grained for medium/high). _(slices 1 degenerate-phase-collapse + 2 shared signal + **3 adversarial-depth dial** + **4a escalate-on-evidence / optimistic-start shipped**: `decompositionRiskEvidence` + an in-band intercept atop `buildNextContractPipelineStep` raises the persisted intake risk signal once decomposition reveals the real shape (>1 module ⇒ ≥medium; risk-subsystem file_scope ⇒ high), tightening the depth dial on the same+every later next-step, idempotent/convergent via `escalateRiskSignal`; tests `tests/remediate/escalate-on-evidence.test.ts` + `intake-risk-signal.test.ts`. 4b un-collapses on a 4a escalation. 3a `adversarialDepthForTier` drives critique/critic light-vs-full from the risk tier (0.30.23); 3b the lean fast path is no longer a zero-scrutiny fork — `runLeanLightReviewGate` runs a mechanically-gated light adversarial review over the approved findings before the lean plan, clear⇒proceed / concern⇒`escalateRiskSignal`+full pipeline (0.30.24, `interpretLeanLightReviewVerdict` fail-safe toward escalation). Floor = light, never zero, on both routes.)_
- **Data-loss on a GENUINE fail-loud commit-refusal.** When the fail-loud path correctly fires on a real generated-artifact-under-scope, the commit-refusal drops the worktree WITHOUT quarantining the worker's uncommitted source edits (the verified implementation is destroyed — node branch 0 commits ahead, worktree gone). On any commit refusal, preserve the worker's real edits (quarantine the dirty worktree / stash a patch) BEFORE removal, exactly like the verify-fail/merge-fail paths call `quarantineFailedNodeCommit`. Generalizes [[enforce-robustness-in-tooling-not-host-discretion]] — a guard that destroys good work is worse than the bug it guards.
