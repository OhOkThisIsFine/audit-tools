...
- **Gemini CLI (agy) not auto-detected when neither `agy` nor `gemini` on PATH — no fallback registration (2026-07-13 audit-gate review).** If neither CLI binary is available, agy provider is invisible. User expectation: Gemini CLI should appear in the pool. [[gemini-cli-not-auto-detected]]
