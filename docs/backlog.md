...
- `open-bugs.md:19` — The nightly inbox wipes doc-citation-exempt markers on every render, so its stale-path citation reds recur (2026-08-19, low, owner decision pending).
- `open-bugs.md:27` — `HOST_GATE_DESCRIPTORS` / `HOST_GATE_KINDS` is a write-only registry — completed by P25, read by nothing in production (2026-08-19, low, owner decision pending).
- `open-bugs.md:35` — Coherence components have no granularity control, so a whole audit collapses into one work block (2026-08-13, high).
- `open-bugs.md:49` — The HANDOFF empty-queue projection contract is full-suite-only, so the commit gates pass a red against it (2026-08-18, low, friction).
- `open-bugs.md:57` — `runCommand` buffers child output unboundedly (2026-08-13, medium).
- `open-bugs.md:64` — `shell-trap-guard` misses `git stash push <pathspec>` eating uncommitted work (2026-08-12, medium).
- `open-bugs.md:70` — Contract-pipeline fan-out names a mechanism the host may not have (2026-08-08, medium).
- `open-bugs.md:78` — Diff-based re-review loses the verdict it must diff against (2026-08-08, low).
- `open-bugs.md:84` — `free_form_intent` clause splitter shreds prose on bare `;` (2026-08-08, low).
- `open-bugs.md:88` — Answering an intake question at the checkpoint does not clear `open_questions` (2026-08-08, low).
- `open-bugs.md:93` — Sweep the test tree for tests that re-implement their subject (2026-08-08, medium).
- `open-bugs.md:102` — Regex-perf triage tail from the analyzer sweep (2026-08-07, low).
- `open-bugs.md:109` — Vitest worker RPC starvation — the false-RED exit is CLOSED at the gate; the >60s blocking worker is unlocated (recharacterized 2026-08-07; was "full-suite exits 1 while every test passes", 2026-08-06).
- `open-bugs.md:128` — Remediation pause/recovery is not durable (2026-08-03, medium).
- `open-bugs.md:136` — Graph heuristics are promoted to findings without a semantic lead boundary (2026-08-03, medium).
- `open-bugs.md:143` — Phase-boundary gate false abandonment (2026-07-30, HIGH).
- `open-bugs.md:154` — Contract-type coverage is derived from where TESTS live, not from the contract (2026-07-25, low, friction: inefficient-feeding).
- `open-bugs.md:163` — Backlog prose paraphrased an incident in a way that INVERTED its mechanism, costing a wrong implementation (2026-07-24, medium, friction: ambiguous-direction).
...
- `open-bugs.md:209` — ⬇ Live-run watch (re-dogfood 2026-07-22, medium): a worker self-reported "valid, verified" on a malformed-JSON result file — result validity must be checked mechanically, never trusted from the worker's claim.
- `open-bugs.md:217` — ⬇ Live-run watch (re-dogfood 2026-07-22, low, medium-difficulty — an ATTEMPTED fix was reverted 2026-07-25): completion cleanup removes the friction dir before the session stop-gate's close-out walk runs against it.
- `open-bugs.md:235` — LEAD (re-dogfood): systemic-challenge round counter + banked improvements carry across RUNS (2026-07-21, low).
- `open-bugs.md:240` — Review rounds re-derive the same file map every time (inefficient-feeding, 2026-07-19).
- `open-bugs.md:261` — A per-site pinning gate would make "red-green validated" mechanically checkable — UNBUILT on main.
- `open-bugs.md:281` — Friction walk (determinations-execution lap, 2026-07-29):
- `open-bugs.md:295` — Friction walk (duplicated-guard lap, 2026-07-25):
- `open-bugs.md:307` — Implementation workers are never given the contract they must satisfy (2026-08-09, high).
- `open-bugs.md:317` — A delegated step prompt can turn its executor into a second driver (2026-07-16, tool-should-decide, medium).
- `open-bugs.md:333` — Self-audit dogfood loop: fixing the tool mid-run invalidates the run (2026-07-16, ambiguous-direction, low-medium).
- `open-bugs.md:349` — A stale-artifact re-extraction `next-step` runs >2min with no progress signal, silently blowing a caller timeout (live dogfood 2026-07-17, inefficient-feeding, low).
- `open-bugs.md:351` — Friction walk (niggle-fix lap, 2026-08-07):
- `open-bugs.md:367` — Friction walk (loop-core `.ts`-conversion tranche lap, 2026-07-28):
- `open-bugs.md:379` — Friction walk (queue-closeout + first `.ts`-conversion lap, 2026-07-28):
- `open-bugs.md:394` — Friction walk (nightly-determinations lap, 2026-07-26):
- `open-bugs.md:406` — Friction walk (contract-sweep producer lap, 2026-07-26):
- `open-bugs.md:418` — Friction walk (touched_files load-gate lap, 2026-07-25):
- `open-bugs.md:424` — Friction walk (fourth backlog-clearance lap, 2026-07-24):
- `open-bugs.md:437` — Friction walk (second backlog-clearance lap, 2026-07-24):
- `open-bugs.md:446` — Untracked-exclusion scope rule — residuals only (each low-severity, documented at the code site).
- `open-bugs.md:472` — External shared-logic audit V1–V7 residuals
- `open-bugs.md:484` — Top gate optimization — the suite-side tail is subprocess wall, not isolation overhead (measured 2026-07-06).
- `open-bugs.md:490` — Selective-deepening convergence — live validation env-bound.
- `open-bugs.md:498` — `goal_id` is read verbatim off the LLM envelope, so its format is unvalidated (re-verified at HEAD 2026-07-25).
- `open-bugs.md:507` — `StepArtifactSchema` is `.strict()` but `writeStepContract` injects `agent_id`.
- `open-bugs.md:513` — systemic_challenge findings ids are adversary-invented and round-colliding.
- `open-bugs.md:520` — The systemic_challenge loop has no ceiling — its only exit is a dry signal the host may have to fabricate.
- `open-bugs.md:527` — `ensure` writes opencode.json with unstable key order.
- `open-bugs.md:532` — Two run-id notions; friction record keyed both ways.
- `open-bugs.md:538` — Auditor severity calibration: 0 of 9 self-audit criticals survived mechanism verification (2026-08-06, lead, low).
- `open-bugs.md:543` — `hostInputPause.ts` says analyzer consent lives in session config; it lives in `.audit-tools/audit/analyzer-policy.json` (2026-08-12, nightly, low).
- `open-bugs.md:552` — `writeOpenItems` reads `subject_key` but never computes or requires it; the HANDOFF generator hard-requires it (2026-08-14, nightly, low).
- `open-bugs.md:562` — `check:memory-citations` cannot see a `[[name]]` cross-link, and 4 are already dangling (2026-08-14, nightly, low).
- `open-bugs.md:573` — Steward verification metadata is undeliverable through the host-result envelope (hit 2026-08-18).
- `open-bugs.md:583` — The report renderer emits control characters from finding prose raw (hit 2026-08-18).
