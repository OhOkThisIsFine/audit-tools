...
> entry has one.
- **⛔ `smoke:packaged-audit-code` is RED — `next-step -> blocked` at iteration 0; blocks `verify:release` (found 2026-07-16, HIGH — gates the G-series release).** Pre-existing and NOT from the assembly lift: **proved** by `git stash` → `git checkout 26076e34` → `npm run build` → `npm run smoke:packaged-audit-code` → identical `advanceToDispatchReady: unexpected step kind 'blocked' (iteration 0)`. Repro: `npm run smoke:packaged-audit-code` (also `AUDIT_CODE_VERBOSE=1`). Known so far: a manual `audit-code next-step` on a fresh temp repo returns `provider_confirmation` (Gate-0), NOT `blocked` — so the packaged path diverges from the dev path; the smoke's own `provider_confirmation` handler (`scripts/audit/smoke-packaged-audit-code.mjs:114`) is never reached because iteration 0 is ALREADY blocked; the smoke's `disableNarrative` writes only `synthesis`, so it is not tripping G2's `validateRepoSessionIntent` dispatch-field rejection; the run logs `npm warn install-scripts 1 package had install scripts blocked` (the npm-12 trap — see Durable traps) which may or may not be causal. **Property to hold: a `verify:checks` step that fails must say WHY.** ⚠ **Diagnosability sub-bug (fix first, it is cheap):** `advanceToDispatchReady` throws bare `unexpected step kind 'blocked'` and prints NOTHING of the blocked step's `summary` / `stop_condition` / handoff — so the gate that is supposed to catch a broken packaged CLI tells you nothing about what broke. Print the step's reason before throwing.
