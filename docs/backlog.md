...
- **repair-proxy liveness probe fails on a cold just-started proxy — lane dropped for the invocation with no retry (claude-worker dogfood 2026-07-16, tool-should-decide, low).** First `next-step` after `npm start` on the proxy: `GET /registry` probe failed (first registry build fan-probes all 4 backends, slow), lane dropped with the correct reason; the SAME probe passed seconds later. Fail-open + reason is working as designed; friction is only that a cold proxy costs one wasted invocation. Consider one bounded retry-after-delay in the probe, or a longer probe timeout.
- **`smoke:linked-audit-code` fails on ambient quota state — blocked "Provider quota wall" at iteration 4 on a clean HEAD (2026-07-16, tool-should-decide, low).** Not in any gate (`verify:checks` runs only the packaged smokes), so it gates nothing — but it is unusable as a local check on a box whose live quota ledger is at a wall: the linked smoke resolves the ambient host provider and inherits the box's real quota state. Verified identical on clean `main`, so not a regression. Property to hold: a smoke's outcome must not depend on the box's live quota ledger — pin the smoke to a quota-less provider (worker-command via `--auditor`) or isolate its quota state dir.
...
- **claude-worker lane dogfood feedback gaps — three named classes, 0/119 packets succeeded while the transport itself is proven (2026-07-16, HIGH — this is the lane's remaining work).** Full record: `docs/reviews/claude-worker-lane-dogfood-2026-07-16.md`. (a) **Per-backend packet sizing:** packets are sized against the HOST window; groq rejected every request 413 (packet + harness overhead > its per-request cap) surfacing as the CLI's misleading "Request too large (max 32MB)". Property: a packet admitted to a pool must fit THAT pool's context/request caps (per-tier re-fit exists for host ranks — extend to source pools). (b) **Backend rate signal → pool pacing:** NIM free tier returned 307×429 vs 34×200 under 12 concurrent agentic workers; the worker's own API 429s never reach the pool ledger, so the pool is hammered instead of cooled/demoted (the three-tier quota classification exists for single-shot NIM — the agentic claude-worker path has no equivalent). (c) **Populate-time model verification:** `nim/moonshotai/kimi-k2.6` 404s on every live call yet ranked into top-K off registry capability data; a model that 404s should be demoted/dropped (availability analog of `declared_cost_drift`). Registry rows are LEADS, not reach. [[external-audit-catalogs-are-leads]]
...
- **Durable: CI runs the release gate on every push, but nothing was watching it (found 2026-07-16, medium).** `main` was RED for ~a dozen laps across three independent causes (doc-manifest stray; an env-dependent test; the packaged smoke) while every lap reported "green" — because the local pre-commit hook gates only `npm run check` (typecheck) and every lap verified with `npm run build && npm run check` + a targeted/full **vitest** run, none of which include `verify:checks`. Nothing had released since the G-series began, so the one gate that would have caught it never ran. **Property to hold: a lap cannot report green on evidence weaker than what CI runs.** Cheapest fixes, in order: (a) end a lap by reading `gh run list --branch main --limit 3` — one command, catches all three; (b) run `npm run verify:release` before any "this is shippable" claim; (c) consider adding `check:doc-manifest` to the pre-commit hook (it is ~2s and it is the one that fired on EVERY push). [[enforce-robustness-in-tooling-not-host-discretion]]
- **The in-process provider-class set is forked AND drifted — audit 4 entries, remediate 7, discovered when both missed `claude-worker` (dogfood 2026-07-16, medium).** `IN_PROCESS_AUDIT_PROVIDERS` (`src/audit/cli/hybridDispatch.ts`) vs `IN_PROCESS_DISPATCH_PROVIDERS` (`src/remediate/steps/nextStep.ts`): remediate additionally classifies `subprocess-template`/`worker-command`/`agy` as in-process; audit routes those to host review. A NEW worker kind must be hand-added to BOTH (claude-worker was added to neither → the whole lane confirmable-but-undrivable; fixed for both same-day). Whether audit's narrower set is policy or drift is undetermined — deciding that is the unification lap's first question. A THIRD near-identical set (`IN_PROCESS_DISPATCH_PROVIDERS` in `src/audit/cli/rollingAuditDispatch.ts:103`, 4 entries) gates a DIFFERENT predicate (is the PRIMARY `sessionConfig.provider` a headless in-process backend) and correctly omits claude-worker (source-pool-only, never primary) — but three same-named/near-named sets with subtly different semantics is exactly how the next worker kind gets missed again. Property to hold: one shared classifier (or one shared base set + explicit per-draw policy) with the primary-provider predicate distinctly named, so a new worker kind routes in both draws by construction. Same family as the emit-wrapper/quota-contract fork below. [[auditor-remediator-mirroring-is-common-logic]] [[silent-fail-closed-on-one-draw]]
- **Dispatch EMIT wrapper + the two quota contracts are still forked (the assembly half SHIPPED 2026-07-16; this is the remainder, medium).** Assembly is now single-sourced (`src/shared/quota/hostPool.ts`, both draws call it; both local preambles deleted). **Still open:** the emit wrapper `finalizeDispatchQuota` (`src/audit/cli/dispatch/quotaPool.ts`) ≡ `buildDispatchQuota` (`src/remediate/steps/dispatch/waveScheduling.ts`) — same `computeDispatchAdmission` shape, character-for-character — and the two contracts `DispatchQuota` (`src/audit/quota/index.ts:102`, literally declared *"Auditor-only type (not in shared)"*) vs `RemediationDispatchQuota` (`src/remediate/steps/types.ts`): **12 shared fields**; audit adds `host_model_roster` + `tier_budgets`, remediate adds `phase` + `estimated_wave_tokens`. One contract with two optional extensions, maintained as two — the kept-in-parity pattern the principle rejects (`waveScheduling.ts` even says *"Mirrors audit's finalizeDispatchQuota"*). **Property to hold: ONE shared emit + ONE shared contract with an optional per-mode extension.** Endpoint: `docs/reviews/dispatch-fork-assessment-2026-07-16.md`. [[auditor-remediator-mirroring-is-common-logic]] [[dissolve-auditor-remediator-distinction]]
- **Undisclosed-at-authoring behavior changes from the 2026-07-16 assembly lift — all judged benign, recorded so they are not rediscovered as mysteries (low).** Surfaced by independent adversarial review, not by the author. (a) **`REMEDIATE_CODE_HOST_MODEL` now works** — remediate's old preamble read no env host-model hint; the shared core takes an `envPrefix` (the `quota/hostLimits.ts` pattern), so remediate gained the var. Nothing referenced it before (grepped src/tests/docs/spec) → no config or test depended on it being ignored. (b) **`block_quota.host_model` is now trimmed/cleaned** for remediate — old code used raw `??`, the shared core routes through `resolveHostModel`'s `clean()`, so `""` / `"  "` now resolve to `null` instead of passing through. (c) **`audit-code quota`'s `capacity_preview.pools` order changed** — roster order → most-capable-first (the shared core sorts). Diagnostic only, no consumer. (d) **Remediate production now reads the real ambient environment** at its three config-load sites (the null short-circuit previously made it immune) — that IS the regression fix, but it is a new hermeticity surface: a remediate test reaching `decideNextStep`/`contractPipeline` without `options.sessionConfig` but with a `session-config.json` on disk now resolves machine-dependent `sources[]`. Suite is green on this box; audit already carries the identical exposure, so it is symmetric. (e) **Latent crash removed:** audit's old `buildDispatchPool` crashed on `hostModelRoster: []` (empty roster → `buildHostModelPools` returns `[]` → `probeBudget(pools[0]!)` on `undefined`); the shared core normalizes empty → null → one scalar pool.
