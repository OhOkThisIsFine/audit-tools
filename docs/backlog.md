...
- **`deriveAccountKey` namespaces the explicit-account key on the TRANSPORT, splitting one vendor credential into two budget partitions (2026-07-19, medium, verified by READING — not yet by execution).** `src/shared/quota/accountId.ts:68` takes the `if (source.account)` branch first and splices `source.transport` in unconditionally: `` `${source.transport}#${source.account}` ``. So a proxied lane (`transport: claude-worker`, `service: nim`) and a direct lane (`transport: openai-compatible`, `service: nim`) both declaring `account: "work"` yield `claude-worker#work` and `openai-compatible#work` — **two account partitions for one real credential at one vendor**, double-booking the budget that `accountKey` exists to protect. The spec is explicit that account is only meaningful *within* a service (`spec/backend-identity-axes.md:46,56-59`), so the namespace must be service-keyed. Confirmed independently twice (classification pass + an offload-lane verification pass) but **not reproduced by execution**, which is why it did not ship with stage 1. ⚠ Note the asymmetry that makes it easy to misread: the *derived* branch (`deriveCredentialIdentity`) is endpoint+credential-keyed and carries no transport at all, so it is already immune — only the explicit-`account` branch is defective. Distinct from the `deriveLocalAccountId` item below, which was reviewed and found CORRECT as transport-shaped; do not fix them together.
  **Settling test (run this first — drop the fix if it does not reproduce):** build pools for `{transport:"claude-worker", service:"nim", account:"work"}` and `{transport:"openai-compatible", service:"nim", account:"work"}` via `buildSourcePool` and assert `accountKey` equality.
  **Property to hold:** one vendor credential ⇒ exactly one account partition, however the lane is reached. Record: [`identity-migration-stage1-plan-2026-07-19.md`](reviews/identity-migration-stage1-plan-2026-07-19.md) §4.
