...
- **`accept-node` merge SILENTLY DROPS a new file a worker created outside the node's declared `touched_files` write-scope → lands a BROKEN commit on the base → cascades false quarantines on sibling nodes (discovered 0.30.13-successor run, 2026-06-26).** Repro: CP-BLOCK-N-FRICTION-FOUNDATION's worker created a NEW shared file `src/shared/friction/frictionRecord.ts` and committed the `captureFrictionEvent.ts`/`triage.ts` imports of it; accept-node's write-scope-filtered commit kept the import edits but dropped the new file (it was both outside the declared write-scope AND — root cause — matched the over-broad `**/friction/` .gitignore so `git add` skipped it). Result: base commit `b8632dd5` imported a module that didn't exist → `npm run check` red on the base → the next two accept-nodes (GROUNDING-RELOCATE, B1-ENVELOPE) verified against the red base and were quarantined as "failed" though they were green in isolation. Two failure modes to fix in the tool: (1) **a node that ADDS a new file must include it in the commit** — either auto-detect new untracked files in the worktree under the node's directory scope and add them (`git add -A` within scope, or `-f` past ignores when the path is a tracked source dir), or fail loudly with "worker created an out-of-scope/ignored file X — widen write-scope or fix ignore" rather than committing a half-change; (2) **accept-node must verify the MERGED base is green before declaring the node merged** — a node whose own worktree was green but whose merge breaks the base build must roll back / quarantine ITSELF, not poison the base for siblings (green-at-every-commit must hold for the base after each merge, not just in the isolated worktree). Generalizes [[worktree-tests-miss-integration-guards]]. (Ethan, 2026-06-26.)
