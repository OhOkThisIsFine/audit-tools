...
- **Make the loop cheaper — SELF-SCALING pipeline** (design of record: [`self-scaling-pipeline-design.md`](../spec/self-scaling-pipeline-design.md)). One pipeline self-scales via two dials on a shared intake risk/complexity signal: (A) **adversarial depth** — critique/counterexample scale from an inline light self-check (low) to full independent sub-agents (high), floor = LIGHT never zero (remediation re-finds audit errors); (B) **granularity / round-trips** — degenerate phases collapse by structure, round-trip count = f(complexity), optimistic-start + escalate-on-evidence. Signal computed from intake-available data only (affected_files + configurable path-risk patterns + intent; never a pipeline output — the circularity), fail toward MORE scrutiny when uncertain. Whole-repo green gate is never scaled. **Open:** **slice 3b** (soften the structured-audit skip-path to a *mechanically-gated* light review — one bounded light-adversarial step + verdict artifact + resume intercept over the approved findings; verdict-clear ⇒ lean plan, concern ⇒ escalate to full pipeline; ONE atomic replace of the current `nextStep.ts` ~2572 skip-fork) and **slice 4** (granularity dial + escalate-on-evidence). _(slices 1 degenerate-phase-collapse + 2 shared signal shipped — `riskSignal.ts`; **slice 3a adversarial-depth dial on the contract pipeline shipped 0.30.23** — `adversarialDepthForTier` drives critique/critic light-vs-full from the risk tier, floor = light never zero, the slice-2 signal's first behavioral consumer.)_
