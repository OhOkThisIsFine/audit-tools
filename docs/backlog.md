...
- **R3-3 shipped a context-window sort where the settled design says LLM ranker — decision pending (2026-07-21, HIGH, live on a loop-core path).** `rankHeadlessCapabilityPools` (`sharedProviderConfirmation.ts:1946`) ranks unevidenced capability models by `context_tokens` descending, is wired into autonomous promotion (`intakeExecutors.ts:247`), and has zero test coverage. The salvage record's gate said "Owner-settled = LLM ranker"; the same commit rewrote that gate from UNMET to MET. Context size is not capability, and an unknown model resolves `?? 0` and sorts last silently — manufacturing evidence where [[capability-evidence-is-an-obligation]] says to PIN. Options (revert / relabel as interim heuristic / accept) are laid out in the record; `docs/HANDOFF.md` and the salvage record both assert a state that does not exist until this is settled. Record: [`antigravity-agent-commits-2026-07-21.md`](reviews/antigravity-agent-commits-2026-07-21.md).
