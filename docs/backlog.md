...
- **Conversation-first `/audit-code` dispatches HOST-ONLY — resolved claude-worker/proxy pools never fan into the wave (live self-audit dogfood 2026-07-17, HIGH, tool-should-decide).** A fresh conversation-first run reached the `dispatch_review` wave with `resolveAmbientSources` returning **7 claude-worker pools** (proxy warm, 6ms probe) — yet the wave's `capacity_pools` was **host-only** (`claude-code/*`): 1 packet admitted, 78 `cap_reached` on the single host pool, `estimated_wave_tokens` 253k. Root: a bare `{}` `.audit-tools/audit/session-config.json` + unset `AUDIT_CODE_ROLLING_ENGINE` does not arm the in-process rolling-dispatch hybrid (needs rolling_engine AND an explicit in-process backend provider), so the resolved source pools are never folded in. **This REFUTES memory [[repair-proxy-dispatch-unblocked-probe-fix]]'s claim that dispatch is "already pool-availability-driven" for the default path** — the probe fix was necessary but not sufficient; the conversation-first host fan-out to the proxy pools is the UNBUILT follow-on (HANDOFF ▶ IMMEDIATE NEXT step 2 / diagnosis fork B). `packet_too_large` routing (the run's stated validation target) could not be exercised because no small-context pools resolve into a conversation-first wave. Property to hold: when source pools resolve, a conversation-first host run must fan out to them (or the operator must be TOLD host-only and why), never silently dispatch host-only. [[repair-proxy-dispatch-unblocked-probe-fix]] [[conversation-first-subagent-dispatch-first-class]]
