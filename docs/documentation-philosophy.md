...
| **`docs/backlog.md`** | A router + generated seek index into the split backlog files — open work, durable traps, future directions live in `docs/backlog/{open-bugs,minor-bugs,forward-tracks,deferred,durable-traps}.md`. | A status log. Remove an entry once it ships — record the durable contract/rationale in a concept doc, `CLAUDE.md`, or memory, never "where the code is today." |
