...
### 2. Contained file splits — ✅ DONE (2026-06-01)
All three done as pure moves, each verified by the full audit suite (543):
- **`cli/envelope.ts`** ✅ — `buildEnvelope`/`emitEnvelope`/`buildManualReviewBlocker`/`shouldRunInlineExecutor`/`buildBlockedAuditState`; the `ADVANCE_AUDIT_CONTRACT_VERSION` const moved with them.
- **`cli/reviewRun.ts`** ✅ — `activeReviewRunFromTask`/`loadCurrentActiveReviewRun`/`writeHandoffOnly`/`ensureSemanticReviewRun` (imports the two blocked-state helpers from `cli/envelope.ts`).
- **`graph.ts`** ✅ — split into [`graphRoutes.ts`](packages/audit-code/src/extractors/graphRoutes.ts) (registered+conventional+framework routes), [`graphSuites.ts`](packages/audit-code/src/extractors/graphSuites.ts) (json-schema + schema-contract-test + bounded-suite; co-located since they share `MAX_BOUNDED_SUITE_EDGE_FILES`+`isJsonSchemaPath`), and [`graphTestSources.ts`](packages/audit-code/src/extractors/graphTestSources.ts). The shared predicates (`isJsonSchemaPath`/`isPytestConftestPath`/`resolveSpecifier`/`resolveReferenceLiteral`) + `SOURCE_EXTENSIONS`/`STRING_LITERAL_PATTERN` were hoisted into `graphPathUtils.ts` first, exactly as the gotcha note advised. graph.ts 1602 → 561 lines.
  - *Update (✅ done):* the "duplicate manifest-path predicate behavior reconciliation" between `reviewPackets` and `graphManifestEdges` is now resolved. The two divergent `normalizeGraphPath` functions (reviewPackets/scope lowercased the **whole** path; graphPathUtils preserves case) were a latent correctness bug — on a case-sensitive filesystem the wholesale lowercasing collapsed distinct files differing only by case into one packet/cluster identity, and leaked lowercased paths into output. Unified on the **case-preserving** [`graphPathUtils.normalizeGraphPath`](packages/audit-code/src/extractors/graphPathUtils.ts) (reviewPackets re-exports it for scope.ts), and hoisted **one** canonical set of manifest predicates (`isPackageManifestPath`/`isTypescriptProjectConfigPath`/`isGo*`/`isCargoManifestPath`/`isMavenPomPath`/`isPyproject*`/`isPnpm*`) into graphPathUtils — graphManifestEdges and reviewPackets both import them; the filename match stays case-insensitive. Regression test: [graph-path-utils.test.mjs](packages/audit-code/tests/graph-path-utils.test.mjs). Full audit suite 545 green.
...
**Net: cli.ts 4072 → 1728 lines (-58%) across the sprint.** cli.ts is now a thin command dispatcher (`main`/`runCli`) plus the smaller handlers; each large handler/pipeline lives in its own `src/cli/*` module. The `reviewPackets`/`graphManifestEdges` case-sensitivity reconciliation that was deferred here is now **✅ done** — see the ✅ update under section 2. The roadmap is fully closed.
