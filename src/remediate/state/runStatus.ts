 ...
 * and the run-status invariant test asserts against this array. Nothing outside
 * this module may re-enumerate them.
 *
 ...
 * literal written inside the test. Nothing joined them, so the load gate
 * and the type could drift apart with no red build — and the assertion
 ...
 *
 * `documenting` is deliberately absent: the document phase was dissolved, and
 * planning transitions DIRECTLY to implementing. Adding it back is a red — see
 * the run-status invariant test, which reads this array rather than restating
 * it.
 */
