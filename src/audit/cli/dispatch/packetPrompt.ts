...
export function buildPacketPrompt(params: {
  ...
  return [
    ...
    "  reviewed_clean  REQUIRED and must be true when findings is []; omit it otherwise.",
    "",
    "Affirming a clean result (this is a hard validation rule, not advice):",
    "  An empty findings array is only accepted when you also set reviewed_clean: true.",
    "  That flag means: you READ the assigned scope and it is genuinely clean.",
    "  If your review did NOT complete - you ran out of output budget, a file could not be",
    "  read, the task was unclear, or you are otherwise unsure - do NOT submit an empty",
    "  result with the flag. Submit findings you did reach, or report the failure. An",
    "  unaffirmed empty result is rejected precisely because a failed review and a clean",
    "  one otherwise look identical, and a broken lane would be read as a weak one.",
    "  Setting reviewed_clean: true alongside one or more findings is also rejected.",
    "",
    "Lens verification tasks:",
    "  tasks tagged lens_verification must use findings: [] with reviewed_clean: true, and include verification:",
