...
import { collectGraphEdges } from "./reviewPacketGraphEdges.js";
import { normalizeGraphPath } from "../extractors/graphPathUtils.js";

/**
 * Continuity scoring (context-efficiency access-memory track, increment 2b).
 *
 ...
 * used to bias later packet composition toward files already in play — the
 * known god-file-re-read waste hotspot.
 *
 ...
 * structural neighbours too (direct + structural continuity). The derived packet
 ...
 * the `access_memory.json` integer counters are — and the packet-mass rounding
 * below absorbs ULP differences well before they could flip an ordering.)
 *
 ...
 * map as "no bias", so behaviour is identical to pre-2b on the first steps.
 */
