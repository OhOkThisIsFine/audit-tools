 ...
 * tool-created worktree (`<root>/.audit-tools/worktrees/<name>` — remediation
 * implementation worktrees and audit review snapshots) ran a driver lifecycle
 * CLI
 ...
 * root, so the stray invocation rewrote shared run state and falsely blocked an
 * in-flight node. Prompt-level "don't do that" is host
 ...
 *     an explicit worker-safe allowlist (currently only remediation's
 *     result-scoped validators). A NEW command is therefore refused from worker
 *     context until
 ...
 *   • Forced state-dir redirection for workers — the conversation host owns
 *     execution, so the tool cannot force a worker's environment.
 */
 ...
 * working directory to the CLI it spawns. The packaged audit-code wrapper
