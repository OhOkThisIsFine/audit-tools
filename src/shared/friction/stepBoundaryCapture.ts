...
/**
 * CE-005 — the single shared workflow step-boundary chokepoint.
 *
 * Every mechanically observed friction fact flows through `captureStepBoundaryFriction`
 ...
 * workflow already computes each of these facts at its own step boundary — phase
 * re-emit counts, artifact rejection, repair rounds, post-repair re-derive,
 * no-change merge, and the intent-gate lock-across-judge fallback. Routing them
 * all through one chokepoint with ZERO host discretion (INV-O1-1 /
 * OBL-m-friction-inv-1) means a new workflow fact is added by routing
...
/**
 * The mechanically observed step-boundary fact kinds. OPEN/EXTENSIBLE — this union is
 * the documented catalogue, but the chokepoint accepts any string `event_type`
 * so a new workflow fact routes through the SAME emitter without forking a closed
 * enum (CE-005). The named members below are the facts the contract pins:
 *
 ...
 *  - `repair_round`       — one bounded contract-repair stage firing.
...
/**
 * Deterministic map from a workflow step-boundary fact kind to the REAL close-out
 ...
 * every workflow fact is redundant/wasteful re-work or something-the-tool-had-to
 ...
 *    rounds, post-repair re-derives, no-change merges, and the
 ...
 * workflow fact always lands in a real category — never `trap`, never uncovered.
 */
...
export interface StepBoundaryFriction {
  /** Mechanically observed fact kind (one of the named members, or a new string). */
  eventType: StepBoundaryEventType;
  /**
...
/**
 * Route one mechanically observed step-boundary fact through the single chokepoint.
 *
...
export async function captureStepBoundaryFriction(
  ...
  const event: FrictionEvent = {
    ...
    // The aggregation axis: N same-artifact workflow facts collapse to ONE derived
