...
} from "../../src/shared/quota/coverage.js";
import { shouldEmitQuotaNudge, quotaNudgeMarkerName, renderQuotaCoverageNudge } from "../../src/shared/quota/quotaCoverageNudge.js";
import { buildQuotaSource } from "../../src/shared/quota/compositeQuotaSource.js";
...
test("COMPOSED: classifyQuotaCoverage over the composite source's real coversProvider verdicts", () => {
  ...
  const classifyVia = (provider: string) =>
    classifyQuotaCoverage(provider, sourceCoversProvider(composite, provider));
...
test("a proactive source covers exactly its providers; composite is the union", () => {
  ...
  const composite = buildQuotaSource();
  expect(sourceCoversProvider(composite, "claude-code")).toBe(true);
  expect(sourceCoversProvider(composite, "codex")).toBe(true);
  // An as-yet-unsupported host provider is NOT covered.
  expect(sourceCoversProvider(composite, "some-future-ide")).toBe(false);
});
...
function writeQuota(dir: string, pools: Array<{ pool_id: string; quota_coverage: string }>): string {
