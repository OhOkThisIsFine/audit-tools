...
import {
  ...
  type RollingDispatchConfig,
  type ProviderSlot,
} from "../../src/shared/dispatch/rollingDispatch.js";
import { setQuotaStateDir, readQuotaState } from "../../src/shared/quota/state.js";
import { deriveAccountKey } from "../../src/shared/quota/accountId.js";
import {
  HostSessionQuotaSource,
  type HostSessionEscalation,
} from "../../src/shared/quota/hostSessionQuotaSource.js";
import { ReservationLedger } from "../../src/shared/quota/reservationLedger.js";
import type { StampedEngineDecisionRecord } from "../../src/shared/dispatch/dispatchDecisionLog.js";
import type { CapacityPool } from "../../src/shared/quota/capacity.js";
import type { QuotaStateEntry } from "../../src/shared/quota/types.js";
import type { QuotaUsageSnapshot } from "../../src/shared/quota/quotaSource.js";
...
/** The engine treats a packet payload as opaque, so the tests carry a bag. */
type TestPayload = Record<string, unknown>;
type TestPacket = RollingDispatchPacket<TestPayload>;
type TestResult = RollingDispatchResult<TestPayload>;
type TestConfig = RollingDispatchConfig<TestPayload>;

/**
 * The observation-hook payloads are DERIVED from the engine's own config contract
 * rather than restated here — a hook that grows or drops a field surfaces at the
 * recording array, which is the whole point of bringing this file under the gate.
 */
type HookInfo<F> = F extends (info: infer I) => void ? I : never;
/** One arm of the engine's discriminated decision-record union. */
type DecisionOfKind<K extends StampedEngineDecisionRecord["kind"]> = Extract<
  StampedEngineDecisionRecord,
  { kind: K }
>;
type CreditExhaustedInfo = HookInfo<NonNullable<TestConfig["onCreditExhausted"]>>;
type QuotaUnclassifiedInfo = HookInfo<NonNullable<TestConfig["onQuotaUnclassified"]>>;
type ModelUnavailableInfo = HookInfo<NonNullable<TestConfig["onModelUnavailable"]>>;
type PacketTooLargeInfo = HookInfo<NonNullable<TestConfig["onPacketTooLarge"]>>;
type CostDriftInfo = HookInfo<NonNullable<TestConfig["onCostDrift"]>>;
...
function makePacket(
  id: string,
  {
    ...
    complexity = 0.5,
    payload = {},
  }: { estimatedTokens?: number; complexity?: number; payload?: TestPayload } = {},
): TestPacket {
...
const settleOnePass = (): Promise<void> =>
  new Promise((resolve) => setTimeout(resolve, ENGINE_REPASS_SETTLE_MS));

function makePool(id: string, overrides: Partial<CapacityPool> = {}): CapacityPool {
...
/**
 * A live quota snapshot carrying EVERY required field of the real contract.
 *
 * The `.mjs` fixtures supplied only the two fields selection reads
 * (`remaining_pct` / `tokens_remaining`) and silently omitted `reset_at`,
 * `requests_remaining`, `captured_at` and `source`. Every default here is the
 * runtime-equivalent of the omission it replaces — `null` where the field was
 * `undefined` (both take the `??` / `!= null` branch), and a real ISO instant /
 * source name for the two required strings no consumer branches on.
 */
function usageSnapshot(overrides: Partial<QuotaUsageSnapshot> = {}): QuotaUsageSnapshot {
  return {
    remaining_pct: null,
    reset_at: null,
    ...
    source: "rolling-dispatch-test",
    ...overrides,
  };
}

/**
 * A learned quota-state entry carrying every required field. Same story as
 * {@link usageSnapshot}: the fixtures carried `cooldown_until` alone and left
 * `updated_at` / `last_429_at` off the real contract.
 */
function stateEntry(overrides: Partial<QuotaStateEntry> = {}): QuotaStateEntry {
  return {
...
function unlimitedSession(): SessionConfig {
  return { quota: {} };
}
...
function enabledSession(): SessionConfig {
  return { quota: { safety_margin: 1.0 } };
}

/**
 * Narrow `selectProvider`'s nullable return, failing the test loudly otherwise.
 * Every call site already asserts non-null through `expect` first; this restates
 * the same fact to the typechecker (and stays a REAL runtime check, so it can
 * never silently paper over a null the assertion above stopped catching).
 */
function assertSlot(slot: ProviderSlot | null): asserts slot is ProviderSlot {
  if (slot === null) throw new Error("expected selectProvider to return a slot, got null");
}

/** Narrow `getTerminal()`'s nullable return the same way. */
function assertTerminal<T>(terminal: T | null): asserts terminal is T {
  if (terminal === null) throw new Error("expected a partial-completion terminal, got null");
}

async function setupTmpQuotaDir(): Promise<string> {
  ...
  const dispatcher = createRollingDispatcher<TestPayload>(
    {
      ...
      dispatchPacket: async (packet) => {
        ...
        await new Promise<void>((resolve) => { resolvers[packet.id] = resolve; });
...
test("createRollingDispatcher — a pool's own concurrencyCap ceilings its in-flight COUNT (no global option)", async () => {
  ...
  let active = 0;
  let peak = 0;
  const resolvers: Record<string, (() => void) | undefined> = {};
  const dispatcher = createRollingDispatcher<TestPayload>({
...
test("createRollingDispatcher — packets enqueued mid-run are dispatched before run() resolves", async () => {
  ...
  const dispatcher = createRollingDispatcher<TestPayload>(
    {
      ...
      dispatchPacket: async (packet) => {
        ...
        if (packet.id === "p1") {
          ...
          await new Promise<void>((resolve) => { firstResolver = resolve; });
        }
        return { packet, outcome: "success" };
      },
    },
  ...
  // Now release p1
  if (!firstResolver) throw new Error("p1 was never dispatched, so it cannot be released");
  firstResolver();
  const results = await runPromise;
...
test("createRollingDispatcher — rate_limited result re-queues the packet and dispatches it to a surviving pool (INV-QD-07)", async () => {
  ...
  const attemptsByPool: Record<string, number> = { "pool-a": 0, "pool-b": 0 };
...
test("createRollingDispatcher — the capability floor RELAXES to surviving pools when the best-band pool exhausts (zero-spill fix)", async () => {
  await setupTmpQuotaDir();
  const attemptsByPool: Record<string, number> = { strong: 0, weak: 0 };
  ...
  const p: TestPacket = { ...makePacket("p1"), requiredTier: "deep" };
...
test("createRollingDispatcher — fires onQuotaUnclassified once per pool with poolId + verbatim text", async () => {
  await setupTmpQuotaDir();
  const harvested: QuotaUnclassifiedInfo[] = [];
...
test("createRollingDispatcher — when the only pool repeatedly quota_unclassifies, the run resolves via a RETRYABLE quota_paused strand (bounded, never a livelock, pool never excluded)", async () => {
  ...
  const results = await Promise.race([
    dispatcher.run(),
    new Promise<TestResult[]>((_, reject) => setTimeout(() => reject(new Error("dispatcher.run() did not resolve — livelock")), 5000)),
  ]);
...
test("createRollingDispatcher — a rate_limited result with a session-limit reset records a pool pause + parses reset_at (does NOT permanently exhaust)", async () => {
  ...
  if (resetAt === undefined) throw new Error("a paused pool must carry a reset instant");
...
test("createRollingDispatcher — all pools paused → quota_paused terminal with stranded ids + earliest_reset_at (NOT empty_pool, NOT blocked)", async () => {
  ...
  if (typeof terminal.earliest_reset_at !== "string") {
    throw new Error("a quota_paused terminal must carry earliest_reset_at");
  }
...
test("createRollingDispatcher — host-session source wired via recordRateLimit/isPacketEscalated strands a same-packet account wall BEFORE all pools exhaust", async () => {
  ...
  const escalations: HostSessionEscalation[] = [];
...
test("selectProvider — the DEFAULT capability floor tracks the exhausted set (standalone zero-spill)", async () => {
  ...
  // finding, 2026-07-23).
  const packet: TestPacket = { ...makePacket("p1", { complexity: 1.0 }), requiredTier: "deep" };
...
test("selectProvider — spills off a quota-degraded pool to a healthy peer (INV-QD-14)", async () => {
  ...
  const degraded = makePool("degraded-pool", {
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.05, tokens_remaining: 100 }),
  });
  const healthy = makePool("healthy-pool", {
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.95, tokens_remaining: 1_000_000 }),
  });
  const tracker = new InFlightTokenTracker();
...
test("selectProvider — spills off a pool in active cooldown to a peer with headroom (INV-QD-14)", async () => {
  ...
  const healthy = makePool("healthy-pool");
  const tracker = new InFlightTokenTracker();
  const liveEntries: Record<string, QuotaStateEntry> = { "cooling-pool": stateEntry({ cooldown_until: future }) };
...
test("selectProvider — a cooldown learned mid-run (live entry) is observed even when the pool's frozen snapshot is stale-healthy (bug 4)", async () => {
  ...
  const throttled = makePool("throttled-pool", {
    quotaStateEntry: stateEntry({ cooldown_until: null, consecutive_429_count: 0 }),
  });
  const healthy = makePool("healthy-pool");
  const tracker = new InFlightTokenTracker();
  // The cooldown exists ONLY in the live record, keyed by pool id.
  const liveEntries: Record<string, QuotaStateEntry> = { "throttled-pool": stateEntry({ cooldown_until: future }) };
...
test("selectProvider — falls back to the pool's construction snapshot cooldown when the live record is unavailable (transient-read window)", async () => {
  ...
  const packet = makePacket("p1", { complexity: 0.5 });
  const future = new Date(Date.now() + 60 * 60 * 1000).toISOString();
  const cooling = makePool("cooling-pool", { quotaStateEntry: stateEntry({ cooldown_until: future }) });
  const healthy = makePool("healthy-pool");
  const tracker = new InFlightTokenTracker();
...
test("selectProvider — all pools degraded still yields a slot; capability order applies within the fallback group (INV-QD-14)", async () => {
  ...
  const deep = makePool("deep-pool", {
    rank: "deep",
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.04, tokens_remaining: 100 }),
  });
...
test("selectProvider — a near-wall pool that can't fund the packet's tokens spills to a healthy peer of LOWER capability (INV-QD-14, token-native, reachable at requestedConcurrency:1)", async () => {
  ...
  const healthy = makePool("healthy-standard", {
    rank: "standard",
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.9, tokens_remaining: 1_000_000 }),
  });
  const tracker = new InFlightTokenTracker();
...
test("selectProvider — a low remaining_pct pool that STILL holds enough tokens is NOT degraded (the retired percentage cliff is gone)", async () => {
  ...
  const lowPctButFunded = makePool("lowpct-deep", {
    rank: "deep",
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.04, tokens_remaining: 50_000 }),
  });
  const healthy = makePool("healthy-standard", {
    rank: "standard",
    quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.9, tokens_remaining: 1_000_000 }),
...
test("selectProvider — among healthy pools, capability still decides; health never overrides rank within a group (INV-QD-14)", async () => {
  await setupTmpQuotaDir();
  const packet = makePacket("p1", { complexity: 1.0 });
  const deep = makePool("deep-pool", { rank: "deep", quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.9 }) });
  const small = makePool("small-pool", { rank: "small", quotaSourceSnapshot: usageSnapshot({ remaining_pct: 0.9 }) });
  const tracker = new InFlightTokenTracker();
  const slot = selectProvider(packet, [small, deep], tracker, {}, enabledSession());
  expect(slot !== null).toBeTruthy();
  assertSlot(slot);
...
test("selectProvider — a cooldown learned on one same-account source gates its sibling (Bug 3 account fold)", async () => {
  ...
  const tracker = new InFlightTokenTracker();
  const liveEntries: Record<string, QuotaStateEntry> = { "nim-nano": stateEntry({ cooldown_until: future }) };
  const slot = selectProvider(
    packet,
...
test("createRollingDispatcher — packet_too_large with a SECOND pool re-queues and succeeds there; hook fires with the pair", async () => {
  ...
  const hookCalls: PacketTooLargeInfo[] = [];
...
test("createRollingDispatcher — model_unavailable excludes the pool for the run and fires the hook once", async () => {
  ...
  const hookCalls: ModelUnavailableInfo[] = [];
...
test("selectProvider — a floor-carrying packet is never bound to a pool below its capability floor (F4)", async () => {
  ...
  const packet: TestPacket = { ...makePacket("p1", { complexity: 0.2 }), requiredTier: "deep" };
...
test("createRollingDispatcher — a deep packet whose only above-floor pool PAUSES strands retryably (quota_paused) instead of spinning until the reset (pause wall)", async () => {
  await setupTmpQuotaDir();
  const records: StampedEngineDecisionRecord[] = [];
  const seen: string[] = [];
  const dispatcher = createRollingDispatcher<TestPayload>({
    confirmedPools: [
      // Two SCORED pools → terciles put `strong` in band 0, `weak` in band 1.
...
test("createRollingDispatcher — a REUSED dispatcher does not inherit a prior run's wall reset: a later permanent strand classifies empty_pool (lifecycle hygiene)", async () => {
  ...
  const run1Terminal = dispatcher.getTerminal();
  assertTerminal(run1Terminal);
  expect(run1Terminal.reason).toBe("quota_paused");
...
test("decision log — a ledger block records engine_blocked with the constraint outcomes, then the forced backstop admit records forced:true", async () => {
  ...
  const admitRecord = records[1];
  if (admitRecord.kind !== "engine_admitted") {
    throw new Error(`expected an engine_admitted record, got ${admitRecord.kind}`);
  }
  expect(admitRecord.lease_id).not.toBe(null);
...
test("decision log — identical re-blocks across passes are transition-deduped; the transition and the forced backstop still record (host-review F1/F3)", async () => {
  ...
  const dispatcher = createRollingDispatcher<TestPayload>({
    ...
    dispatchPacket: (packet) =>
      packet.id === "huge"
        ? Promise.resolve<TestResult>({ packet, outcome: "success" })
        : new Promise<TestResult>((resolve) => resolvers.set(packet.id, () => resolve({ packet, outcome: "success" }))),
    reservationLedger: ledger,
    resolvePoolConstraints: (poolId, tokens) => ({
    ...
    onAdmissionDecision: (r) => {
      ...
      const release = next === undefined ? undefined : resolvers.get(next);
      if (next && release) {
        release();
  ...
  const hugeBlocked = records.filter(
    (r): r is DecisionOfKind<"engine_blocked"> =>
      r.kind === "engine_blocked" && r.packet_id === "huge",
  );
  ...
  const hugeAdmits = records.filter(
    (r): r is DecisionOfKind<"engine_admitted"> =>
      r.kind === "engine_admitted" && r.packet_id === "huge",
  );
