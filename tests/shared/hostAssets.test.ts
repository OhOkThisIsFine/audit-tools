...
import { renderHostAsset, type HostAssetKind } from "../../src/shared/hostAssets.js";
...
test("renderHostAsset markdown kinds do not escape body characters", () => {
  for (const kind of ["vscode-agent", "codex-recipe", "antigravity-guide"] as const) {
    const out = renderHostAsset(kind, { promptBody: CANONICAL_BODY, toolName: "tool" });
...
test("renderHostAsset throws on an unknown kind", () => {
  assert.throws(
    // @ts-expect-error — intentional invalid kind for runtime error assertion
