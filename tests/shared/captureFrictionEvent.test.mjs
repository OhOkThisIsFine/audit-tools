...
test("captureFrictionEvent appends an event to the per-run record", async () => {
  ...
  try {
    ...
    assert.deepEqual(raw.frictions[0], { id: "e1", note: "first" });
  } finally {
...
test("captureFrictionEvent de-dups by event id across re-entrant passes", async () => {
  ...
  try {
    ...
    assert.equal(raw.frictions.length, 2, "the duplicate id must be dropped");
    assert.deepEqual(
      raw.frictions.map((f) => f.id),
      ["e1", "e2"],
    );
    // First-write-wins on the de-duped id.
    assert.equal(raw.frictions[0].note, "first");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test("captureFrictionEvent paths are OS-agnostic + run-id sanitized", async () => {
  ...
  try {
    ...
    assert.equal(raw.frictions.length, 1);
    assert.ok(
      path.startsWith(join(dir, "friction")),
      "record must stay under <artifactsDir>/friction",
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
