...
import type { CapturedFrictionItem } from "../../src/shared/friction/frictionRecord.js";

const { emitBlindDispatchFrictionIfBlind } = await import(
  "../../src/shared/friction/blindDispatchFriction.js"
);
const { frictionCapturePath } = await import("../../src/shared/io/frictionCapture.js");

async function readFrictions(dir: string, runId: string): Promise<CapturedFrictionItem[]> {
  const raw: { frictions: CapturedFrictionItem[] } = JSON.parse(
    await readFile(frictionCapturePath(dir, runId), "utf8"),
  );
    ...
    await emitBlindDispatchFrictionIfBlind({
      ...
      schedule: {
        ...
        host_concurrency_limit: {
          ...
          active_subagents: 4,
          description: "test host concurrency cap",
        },
      },
      itemCount: 10,
      waveKind: "review",
