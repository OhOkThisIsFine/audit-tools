...
import type { FetchLike, FetchResponseLike } from "../../src/shared/quota/httpQuotaSource.js";
...
interface ClaudeOAuthBlockFixture {
  accessToken: string;
  expiresAt: number;
  refreshToken?: string;
}

function writeCreds(creds: unknown): string {
...
function validCreds(overrides: Partial<ClaudeOAuthBlockFixture> = {}): { claudeAiOauth: ClaudeOAuthBlockFixture } {
...
function recordingFetch(
  response: FetchResponseLike | ((url: string, init: FetchInit) => FetchResponseLike | Promise<FetchResponseLike>),
...
/** The refresh-grant POST body shape sent by `runRefreshGrant`. */
interface RefreshGrantBody {
  grant_type: string;
...
function routedFetch(handlers: {
  onUsage: (init: FetchInit) => FetchResponseLike | Promise<FetchResponseLike>;
  onToken: (body: RefreshGrantBody) => FetchResponseLike | Promise<FetchResponseLike>;
}): FetchLike & { calls: Array<{ url: string; init: FetchInit }> } {
  const calls: Array<{ url: string; init: FetchInit }> = [];
  const fn = async (url: string, init: FetchInit = {}): Promise<FetchResponseLike> => {
    calls.push({ url, init });
    if (url === TOKEN_URL) return handlers.onToken(JSON.parse(init.body ?? "{}"));
    if (url === USAGE_URL) return handlers.onUsage(init);
    throw new Error(`unexpected url ${url}`);
  };
  return Object.assign(fn, { calls });
}

interface PersistedClaudeOAuthBlock {
  accessToken: string;
  refreshToken: string;
  expiresAt: number;
  scopes?: string[];
...
interface PersistedCreds {
  claudeAiOauth: PersistedClaudeOAuthBlock;
  organizationUuid?: string;
}

function readCreds(p: string): PersistedCreds {
...
test("an expired token is refreshed, the rotated creds are persisted, and /usage uses the new token", async () => {
  ...
  let usageAuth: string | undefined = undefined;
...
test("window scope: a model-scoped limit whose display_name is EMPTY still scopes to the model", () => {
  ...
  const snap = mapUsageToSnapshot(body, "claude-fable-5", NOW);
...
test("window scope: a model-scoped limit is NOT deduped away by an account window of the same name", () => {
  ...
  const snap = mapUsageToSnapshot(body, "claude-sonnet-5", NOW);
  const windows = snap?.windows ?? [];
  const account = windows.filter((w) => w.scope === "account");
  const model = windows.filter((w) => w.scope === "model");
...
test("window scope: a limit with scope.model PRESENT but unnameable is still model-scoped", () => {
  ...
  const snap = mapUsageToSnapshot(body, "claude-sonnet-5", NOW);
  const windows = snap?.windows ?? [];
  const scoped = windows.find((w) => w.label === "opaque_cap");
  ...
  expect(windows.filter((w) => w.scope === "account").map((w) => w.label)).not.toContain("opaque_cap");
});
