...
import type { SharedProviderConfirmation } from "../../src/shared/providers/sharedProviderConfirmation.js";
...
import { buildCapabilityFloorCapable } from "../../src/shared/dispatch/admissionLoop.js";
import type { AdmissionPool, AdmissionCandidate } from "../../src/shared/dispatch/admissionLoop.js";
import { tierRank } from "../../src/shared/dispatch/tierRank.js";
import type { QuotaSource } from "../../src/shared/quota/quotaSource.js";
import type { SessionConfig, DispatchableSource } from "../../src/shared/types/sessionConfig.js";
...
describe("additive schema: a confirmation written before capability_rank existed", () => {
  ...
  async function writeLegacy(root: string): Promise<void> {
    await writeFile(
...
const RT_CONFIG: SessionConfig = {
  openai_compatible: { base_url: "http://nim.local/v1", model: "model-alpha" },
};
const RT_SOURCES: DispatchableSource[] = [
  {
    id: "gamma-src",
    transport: "openai-compatible",
    model: "model-gamma",
...
const RT_INPUT: ProviderConfirmationInput = {
...
function buildRoundTrip(): SharedProviderConfirmation {
...
describe("round-trip: capability_rank survives persist → read, keyed by model_id", () => {
  ...
  test("a pool with no model_id contributes nothing (unjoinable ⇒ unrankable)", async () => {
    await withTempRoot(async (root) => {
      ...
      expect(raw.provider_pool.map((e: { name: string }) => e.name)).toContain("worker-command");
  ...
  test("the reach brand is STILL not persisted — the new field opened no leak", async () => {
    await withTempRoot(async (root) => {
      ...
      const alpha = raw.provider_pool.find((e: { name: string }) => e.name === "openai-compatible");
...
describe("sign convention: capability_order is most-capable-first ⇒ LOWER rank", () => {
  const SIGN_SOURCES: DispatchableSource[] = [
    {
      id: "strong-src",
      transport: "openai-compatible",
      model: "model-strong",
  ...
  test("the FIRST-listed model gets the LOWER rank and wins a deep-tier packet", async () => {
    await withTempRoot(async (root) => {
      ...
      const admissionPools: AdmissionPool[] = pools.map((p) => ({
        poolId: p.id,
        resourceKey: p.id,
      ...
      const deepPacket: AdmissionCandidate = { id: "hard", cost: 1000, complexity: 1, requiredTier: "deep" };
      ...
      const smallPacket: AdmissionCandidate = { id: "easy", cost: 1000, complexity: 0.1, requiredTier: "small" };
...
describe("the operator's answer survives the input PARSER", () => {
  ...
  test("a non-string-array capability_order degrades to absent, never throws", () => {
    for (const bad of [42, "model-a", [1, 2], [{}], null]) {
      ...
      if (parsed === null) throw new Error("unreachable: asserted not null above");
  ...
  test("END TO END from the input FILE on disk to a ranked pool", async () => {
    await withTempRoot(async (root) => {
      ...
      if (fromDisk === null) throw new Error("expected the Gate-0 input to round-trip from disk");
...
describe("resolveUnevidencedCapabilityPools: the capability delta", () => {
  ...
  const src = (model: string | null, extra: Partial<DispatchableSource> = {}): DispatchableSource => ({
  ...
  async function writeConfirmation(
    root: string,
    overrides: Partial<SharedProviderConfirmation> = {},
  ): Promise<void> {
...
describe("R3-3 additive schema: a confirmation written before capability_order_llm_ranked existed", () => {
  ...
  test("it still parses: capability_order survives, capability_order_llm_ranked is simply absent", async () => {
    await withTempRoot(async (root) => {
      ...
      if (read === null) throw new Error("unreachable: asserted not null above");
...
describe("R3-3 round-trip: capability_order_llm_ranked survives write → read", () => {
  test("the LLM-ranked set persists (sorted, as a SET, not a positional ordering) and round-trips", async () => {
    await withTempRoot(async (root) => {
      ...
      expect(back, "the confirmation must round-trip").not.toBeNull();
      if (back === null) throw new Error("unreachable: asserted not null above");
      expect(
