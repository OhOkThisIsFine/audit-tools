...
} from "../../src/shared/quota/state.js";
import {
  windowSlopeKey,
...
// stops that in TypeScript, which is exactly the discipline this file's conversion
// from .mjs to .ts turns on: every slope key below is built through the real
// constructor, never a hand-typed string.
const SESSION = windowSlopeKey("account", "session");
const WEEKLY = windowSlopeKey("account", "weekly");
const ACCOUNT_DEFAULT = windowSlopeKey("account", "default");
const MODEL_SESSION = windowSlopeKey("model", "session");

/**
 * A window reading as it could appear in a `dispatch-quota.json` written before
 * the `scope` field existed — read back raw with no schema parse (see
 * quotaSnapshotWindowPctMap's scope-less-window skip contract), so `scope` may be
 * absent even though the current {@link QuotaWindow} contract requires it.
 */
type PreScopeQuotaWindow = Omit<QuotaWindow, "scope"> & { scope?: QuotaWindowScope };
type PreScopeQuotaUsageSnapshot = Omit<QuotaUsageSnapshot, "windows"> & {
  windows?: PreScopeQuotaWindow[];
};
...
test("COR-d528d2cd: 'rate_limited' outcome correctly increments 429 count and sets cooldown", async () => {
  await withTempStateDir(async () => {
    ...
    const state = await readQuotaState();
    const entry = state.entries[KEY];
    expect((entry.consecutive_429_count ?? 0) > 0, "rate_limited must increment consecutive_429_count").toBeTruthy();
...
test("quotaSnapshotWindowPctMap: single-window snapshot falls back to an account-scoped 'default' window, carrying reset_at", () => {
  ...
  expect(map.get(ACCOUNT_DEFAULT)).toEqual({ remainingPct: 0.6, resetAt: "2026-07-11T10:00:00.000Z" });
});
...
test("quotaSnapshotWindowPctMap: multi-window snapshot keys by (scope,label), each with its own reset_at", () => {
  ...
  expect(map.has(ACCOUNT_DEFAULT)).toBe(false);
});
...
test("quotaSnapshotWindowPctMap: a scope-less window is SKIPPED, not keyed as 'undefined:'", () => {
  ...
  const rawSnapshot: PreScopeQuotaUsageSnapshot = {
    remaining_pct: 0.6,
    reset_at: null,
    requests_remaining: null,
    tokens_remaining: null,
  ...
  const map = quotaSnapshotWindowPctMap(rawSnapshot as QuotaUsageSnapshot);
  expect([...map.keys()]).toEqual([WEEKLY]);
...
test("foldSlopeObservationFromSnapshots does not throw on a pre-scope persisted snapshot", async () => {
  ...
  await withTempStateDir(async () => {
    const preScope = (pct: number): PreScopeQuotaUsageSnapshot => ({
      remaining_pct: pct,
      reset_at: null,
      requests_remaining: null,
      tokens_remaining: null,
    ...
    const folded = await foldSlopeObservationFromSnapshots(
      "prov/model",
      preScope(0.5) as QuotaUsageSnapshot,
      preScope(0.4) as QuotaUsageSnapshot,
      1500,
    );
...
test("quotaSnapshotWindowPctMap: same label at two scopes stays TWO entries (no blended slope)", () => {
  ...
  expect(map.get(SESSION)!.remainingPct).toBe(0.6);
  expect(map.get(MODEL_SESSION)!.remainingPct).toBe(0.2);
});

test("foldSlopeObservationFromPctMaps: zero/negative delta is rejected for both an increase and no change", async () => {
  await withTempStateDir(async () => {
    const priorIncrease = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.4, resetAt: null }]]);
    const currentIncrease = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.5, resetAt: null }]]); // went UP
    ...
    const priorSame = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.4, resetAt: null }]]);
    const currentSame = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.4, resetAt: null }]]); // unchanged
...
test("foldSlopeObservationFromSnapshots (C1 shared-core wrapper): folds a real PRE/POST snapshot pair end to end", async () => {
  await withTempStateDir(async () => {
    ...
    expect(folded).toEqual([ACCOUNT_DEFAULT]);
    const state = await readQuotaState();
    // 1500 tokens / (0.5-0.4)*100 = 10 percent → 150 tokens/pct.
    expect(Math.abs(state.entries["prov/model"].tokens_per_pct![ACCOUNT_DEFAULT] - 150) < 1e-6).toBe(true);
  });
});
...
test("foldSlopeObservationFromPctMaps (C2): a swallowed write failure is never reported as folded", async () => {
  ...
  try {
    ...
    const prior = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.5, resetAt: null }]]);
    const current = new Map([[ACCOUNT_DEFAULT, { remainingPct: 0.4, resetAt: null }]]);
...
test("INV-QD-15: a legacy `buckets` blob is dropped on the next v2 write, not carried forever", async () => {
  await withTempStateDir(async () => {
    ...
    // `buckets` predates the current v2 QuotaStateEntry contract (the bucket
    // learner was retired), so a v1 entry carrying it is not modeled by
    // QuotaStateEntry — read back raw with no schema parse, same as the
    // pre-scope window case above.
    await writeFile(
      getQuotaStatePath(),
      JSON.stringify({
        version: 1,
    ...
    type LegacyV1Entry = QuotaStateEntry & { buckets?: unknown };
    expect(((await readQuotaState()).entries[KEY] as LegacyV1Entry).buckets).toBeDefined();
