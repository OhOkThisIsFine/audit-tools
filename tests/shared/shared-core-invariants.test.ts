...
import { readFileSync, existsSync, type WriteStream } from "node:fs";
...
import * as sharedIndex from "../../src/shared/index.js";
...
import { spawnLoggedCommand } from "../../src/shared/providers/spawnLoggedCommand.js";
...
test("INV-shared-core-02: CAPABILITY_TIER_MAP is not exported from shared", () => {
  // CAPABILITY_TIER_MAP was an internal map; it must not be exported.
  expect((sharedIndex as Record<string, unknown>)["CAPABILITY_TIER_MAP"], "CAPABILITY_TIER_MAP must not be exported from shared (INV-shared-core-02)").toBe(undefined);
});
...
test("INV-shared-core-02: rollingDispatch derives pool rank from pool.rank, not from provider name", async () => {
  ...
  function makePool(id: string, rank: CapacityPool["rank"]): CapacityPool {
    return {
      id,
      // Each pool is an independent backend for this test: it meters alone (the
      // required accountKey partitions account-scoped allowances; distinct = no
      // folding), matching the established fixture style in capacity.test.ts.
      accountKey: id,
  ...
  const highComplexityPacket: RollingDispatchPacket<Record<string, never>> = {
    id: "p1",
    payload: {},
    estimatedTokens: 100,
    complexity: 0.9,
  ...
  const slot: ProviderSlot | null = selectProvider(highComplexityPacket, [smallPool, deepPool], tracker, {}, session);
...
test("INV-shared-core-03: shared providerFactory exports createFreshSessionProvider", () => {
  expect(typeof createFreshSessionProvider, "createFreshSessionProvider must be exported from shared").toBe("function");
});

test("INV-shared-core-03: FreshSessionProviderDeps interface exposes createClaudeCodeProvider and createOpenCodeProvider", () => {
...
test("INV-shared-core-08: validateSessionConfig warns when dangerously_skip_permissions=true", () => {
...
test("INV-shared-core-08: validateSessionConfig does not warn when dangerously_skip_permissions is absent", () => {
  const issues = validateSessionConfig({
    provider: "claude-code",
...
test("INV-shared-core-08: validateSessionConfig does not warn when dangerously_skip_permissions=false", () => {
  const issues = validateSessionConfig({
...
test("INV-shared-core-12: CODEX env var resolves auto provider to codex (in-session)", () => {
...
test("INV-shared-core-12: ANTIGRAVITY env var resolves auto provider to antigravity when a template is configured", () => {
  const resolved = resolveFreshSessionProviderName(
    "auto",
...
test("INV-shared-core-12: TERM_PROGRAM=antigravity also resolves to antigravity with a template", () => {
  const resolved = resolveFreshSessionProviderName(
    "auto",
    { antigravity: { command_template: ["ag", "--run"] } },
...
test("INV-shared-core-12: providerFactory.ts has no TODO(verify) comments for Codex/Antigravity signals", () => {
  const factoryPath = resolve(__dirname, "../../src/shared/providers/providerFactory.ts");
...
test("INV-shared-core-13: RunLogger exports the full structured logging contract", () => {
  ...
  for (const kind of ["obligation", "executor_start", "executor_end", "artifact_write",
                       "scope", "outcome", "provider_launch", "step", "state", "error"] as const) {
    assert.doesNotThrow(
      () => disabled.event({ kind }),
...
test("INV-shared-core-14: createFreshSessionProvider emits provider_launch RunLogger event on auto-resolution", async () => {
  ...
  try {
    ...
    const deps: FreshSessionProviderDeps = {
      ...
      // PATH, so on a box with `agy` installed it reaches a branch whose stub is
      // missing and dies with `deps.createX is not a function`. That made the
      // test pass in CI (no CLIs) and fail locally — an ambient-PATH hermeticity leak,
      // not a product defect. (Codex is constructed directly by the factory, not
      // via deps, so it needs no stub here.)
      createAgyProvider: (_config) => ({ name: "agy", launch: async () => ({ accepted: false }) }),
      runLogger,
    };
...
test("INV-shared-core-14: FreshSessionProviderDeps.runLogger is optional (no logger = no throw)", () => {
...
test("INV-shared-core-11: spawnLoggedCommand spawns the command unwrapped", async () => {
  ...
  function fakeWS() {
    // `SpawnLoggedCommandOptions.createWriteStream` is `typeof createWriteStream`
    // (Node's overloaded signature); a zero-arg function is still assignable —
    // fewer params is allowed by function-type compatibility — so only the
    // returned fake stream object needs the cast (it does not implement the
    // full WriteStream class), mirroring the established double style in
    // tests/remediate/providers.test.ts.
    const s = new EventEmitter() as EventEmitter & { write: unknown; end: unknown };
    s.write = (_c: unknown, cb?: () => void) => { if (typeof cb === "function") cb(); return true; };
    s.end = (cb?: () => void) => { if (typeof cb === "function") cb(); };
    return s as unknown as WriteStream;
  }

  // `typeof spawn` is overloaded across the `(command, options)` and
  // `(command, args, options)` forms; a single-signature double is assignable
  // to it only when its middle parameter accepts either shape — the same
  // `SpawnDouble` alias used in tests/remediate/providers.test.ts.
  type SpawnDouble = (
    command: string,
  ...
  const spawnDouble: SpawnDouble = (command, args) => {
    calls.push({ command, args: (args as readonly string[]) ?? [] });
