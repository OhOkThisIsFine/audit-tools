...
describe('session-start-guards: stale agent worktrees are reaped', () => {
  let base;
  let repo;
  let pass; // the ONE prune pass every assertion below reads
  const wt = (name) => join(base, name);
   ...
   */
  function backdate(path, hours = 7) {
  ...
  const isListed = (path) => listedPaths().includes(resolve(path).replace(/\\/g, '/').toLowerCase());
