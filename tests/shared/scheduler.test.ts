...
import { scheduleWave, TOKEN_BUDGET_COLD_START_SLOTS } from "../../src/shared/quota/scheduler.js";
import type { QuotaConfig, SessionConfig } from "../../src/shared/types/sessionConfig.js";
import type { HostConcurrencyLimit } from "../../src/shared/quota/types.js";

// A minimal session-config that keeps quota enabled.
function baseSessionConfig(overrides: Partial<QuotaConfig> = {}): SessionConfig {
  return {
...
// Minimal HostConcurrencyLimit fixture — active_subagents is the field under
// test; source/description are required by the contract but immaterial here.
function hostLimit(activeSubagents: number): HostConcurrencyLimit {
  return { active_subagents: activeSubagents, source: "cli_flags", description: "test host limit" };
}
...
test("host_concurrency cap: a tighter host ceiling overrides a looser quota cap and binds last", () => {
  ...
  const schedule = scheduleWave({
    ...
    quotaStateEntry: null,
    hostConcurrencyLimit: hostLimit(2),
  });
...
test("host_concurrency cap: a looser host ceiling does NOT override a tighter quota cap", () => {
  ...
  const schedule = scheduleWave({
    ...
    requestedConcurrency: 20,
    quotaStateEntry: null,
    hostConcurrencyLimit: hostLimit(10),
  });
...
test("cooldown cap: an active cooldown throttles to one slot and short-circuits cap logic", () => {
  ...
  const schedule = scheduleWave({
    ...
    hostModel: "test/model",
    requestedConcurrency: 20,
    quotaStateEntry: { updated_at: new Date().toISOString(), cooldown_until: future, last_429_at: future },
    hostConcurrencyLimit: null,
  });
...
test("cold-start concurrency clamp binds an in-process (backend) pool identically to a host pool — no selfPacing exemption", () => {
  const base = {
    providerName: "openai-compatible" as const, // in-process backend provider (NIM)
    sessionConfig: { quota: { safety_margin: 1.0 } },
    hostModel: null,
    requestedConcurrency: 10,
    estimatedSlotTokens: new Array(10).fill(1000),
    quotaSourceSnapshot: {
      ...
      source: "test",
      windows: [
        { label: "session", scope: "account" as const, remaining_pct: null, tokens_remaining: 100_000, reset_at: null },
        { label: "weekly", scope: "account" as const, remaining_pct: 0.5, tokens_remaining: null, reset_at: null },
      ],
    },
  };
...
test("window scope guard: fires on the COOLDOWN path, where budget derivation is skipped", () => {
  const future = new Date(Date.now() + 5 * 60_000).toISOString();
  // Deliberately off-contract (the window carries no `scope`) — this simulates
  // data crossing a boundary the type system doesn't police (e.g. a stale
  // persisted snapshot), which is exactly what `assertWindowScopes` guards
  // against at runtime. Routed through JSON.parse so the value arrives as `any`
  // the same way it would from a real deserialization boundary, rather than
  // masking the missing field with a type assertion.
  const scopelessRaw = {
    remaining_pct: 0.5,
    reset_at: null,
    requests_remaining: null,
  ...
  const scopeless = JSON.parse(JSON.stringify(scopelessRaw));
  expect(() =>
    scheduleWave({
      providerName: "claude-code",
      sessionConfig: { quota: { safety_margin: 1.0 } },
