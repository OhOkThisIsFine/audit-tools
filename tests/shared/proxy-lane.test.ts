...
function cache(
  {
    endpoint = PROXY,
    sources = [EXPANDED],
  }: { endpoint?: string; sources?: DispatchableSource[] } = {},
): string {
...
describe("verifySourceReach — claude-worker", () => {
  it("requires endpoint and model", () => {
    expect(
      (verifySourceReach({ transport: "claude-worker", model: "m" }, deps({})) as {
        verified: false;
        reason: string;
      }).reason,
    ).toContain("endpoint");
    expect(
      (verifySourceReach({ transport: "claude-worker", endpoint: PROXY }, deps({})) as {
        verified: false;
        reason: string;
      })
        .reason,
  ...
  it("api_key_env must be set when declared", () => {
    const source: DispatchableSource = { transport: "claude-worker", endpoint: PROXY, model: "m", api_key_env: "UNSET_VAR" };
    const result = verifySourceReach(source, deps({ env: {} })); // UNSET_VAR not in env
    expect(result.verified).toBe(false);
    expect((result as { verified: false; reason: string }).reason).toContain("UNSET_VAR");
  });

  it("reach IS the proxy liveness probe", () => {
    const source: DispatchableSource = { transport: "claude-worker", endpoint: `${PROXY}/`, model: "m" };
    ...
    expect((down as { verified: false; reason: string }).reason).toContain("liveness");
  });
});
