...
import { OpenCodeQuotaSource } from "../../src/shared/quota/openCodeQuotaSource.js";
...
/**
 * Mirrors the private `OpenCodeAuthEntry` shape the module reads from auth.json —
 * plus `refresh`, which real opencode auth.json entries carry but the module never
 * reads; kept here only to mirror the on-disk shape realistically.
 */
interface AuthFixtureEntry {
  type?: string;
  access?: string;
  key?: string;
  expires?: number;
...
type AuthFixture = Record<string, AuthFixtureEntry | undefined>;

function authFixture(overrides: Partial<AuthFixture> = {}): AuthFixture {
  return {
...
function writeAuth(auth: AuthFixture): string {
...
type RoutingFetch = FetchLike & { calls: FetchCall[] };

// Routes by URL so we can assert the broker hit the RIGHT underlying endpoint.
function routingFetch(): RoutingFetch {
...
function source(auth: AuthFixture, fetchImpl?: FetchLike): OpenCodeQuotaSource {
