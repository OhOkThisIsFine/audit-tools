...
} from "../../src/shared/repair/emitValidateRepair.js";
import { frictionCapturePath } from "../../src/shared/io/frictionCapture.js";
import { stepBoundaryEventId } from "../../src/shared/friction/stepBoundaryCapture.js";
...
const repairId = (discriminator: string) =>
...
interface MakeContractOverrides {
  recoverableSingle?: boolean;
}

function makeContract(overrides: MakeContractOverrides = {}): RepairContract {
  return {
    ...
    validate(payload: unknown) {
      ...
      payload.forEach((el: Record<string, unknown>, i: number) => {
    ...
    coercion: {
      coerce(payload: unknown) {
        ...
        const next = payload.map((el: Record<string, unknown>, i: number) => {
          const copy = { ...el };
...
async function frictionIds(dir: string, runId: string): Promise<string[]> {
  try {
    const raw = JSON.parse(await readFile(frictionCapturePath(dir, runId), "utf8"));
    return raw.frictions.map((f: { id: string }) => f.id);
  } catch {
    return [];
  }
}
...
test("coercion alone clears OPTIONAL errors: status coerced, NO LLM call", async () => {
  ...
  try {
    ...
    // Identity preserved.
    expect((out.repaired_payload as Record<string, unknown>[])[0].unit_id).toBe("u1");
    expect(!("opt" in (out.repaired_payload as Record<string, unknown>[])[0]), "invalid optional dropped").toBeTruthy();
...
test("recoverable per-element identity is backfilled (single element)", async () => {
  ...
  try {
    ...
    expect(out.status).toBe("coerced");
    expect((out.repaired_payload as Record<string, unknown>[])[0].unit_id).toBe("recovered-unit");
    const ids = await frictionIds(dir, "r1");
...
test("multi-element missing unit_id escalates to unrepairable, never homogenized", async () => {
  ...
  try {
    ...
    expect((out.repaired_payload as Record<string, unknown>[])[1].unit_id).not.toBe("u1");
  ...
  try {
    const out = await runEmitValidateRepair({
      ...
      patcher: async (payload, errors) => {
        ...
        return (payload as Record<string, unknown>[]).map((el) => ({ ...el, unit_id: "patched-unit" }));
      },
    });
    ...
    expect((out.repaired_payload as Record<string, unknown>[])[0].unit_id).toBe("patched-unit");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
