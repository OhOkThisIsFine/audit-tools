...
import { discoverProviders, queryProviderQuota } from "../../src/shared/providers/providerConfirmation.js";
import type { DiscoveredProvider } from "../../src/shared/providers/providerConfirmation.js";
import type { FreshSessionProvider } from "../../src/shared/providers/types.js";
...
test("queryProviderQuota returns null when queryLimits is not implemented", async () => {
  const syntheticDiscovered: DiscoveredProvider = {
    name: "worker-command",
    capabilityTier: "unknown",
    detected: true,
  };
  const providerWithoutQueryLimits: FreshSessionProvider = {
    name: "worker-command",
...
test("queryProviderQuota returns null when queryLimits rejects", async () => {
  ...
  const providerThatThrows: FreshSessionProvider = {
    name: "codex",
    launch: async () => ({ accepted: false }),
...
test("queryProviderQuota returns the limits when queryLimits succeeds", async () => {
  ...
  const providerWithLimits: FreshSessionProvider = {
    name: "claude-code",
    launch: async () => ({ accepted: false }),
...
test("OBS-9a9091ad: queryProviderQuota invokes the injected log on a swallowed query error", async () => {
  ...
  const logged: { providerName: string; error: unknown }[] = [];
