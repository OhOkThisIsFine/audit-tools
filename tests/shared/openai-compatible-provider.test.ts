...
function parseRequestBody(init?: RequestInit): CapturedRequestBody {
...
function emptyCapturedRequestBody(): CapturedRequestBody {
  return {};
}

const tmpCtxDirs: string[] = [];
afterEach(() => {
...
function makeCtx(promptText?: string) {
...
test("launch inlines current contents of prompt-referenced files", async () => {
  ...
  let captured = emptyCapturedRequestBody();
...
test("defect-1 sub-3: referenced-file byte cap is operator-configurable (read-heavy audit tuning)", async () => {
  ...
  let capturedSmall = emptyCapturedRequestBody();
  await new OpenAiCompatibleProvider(
    { ...minimalConfig, referenced_files_total_byte_cap: 10 },
    { fetchFn: async (u, i) => { const r = await capture(u, i); capturedSmall = parseRequestBody(i); return r; } },
  ).launch(input);
  ...
  let capturedBig = emptyCapturedRequestBody();
  await new OpenAiCompatibleProvider(
    { ...minimalConfig, referenced_files_total_byte_cap: 100_000 },
    { fetchFn: async (u, i) => { const r = await capture(u, i); capturedBig = parseRequestBody(i); return r; } },
  ).launch(input);
...
test("a configured endpoint beats the codex last-resort; without it codex wins", () => {
  ...
  const onlyCodex = (c: string) => c === "codex";
...
for (const status of [400, 422]) {
  test(`HTTP ${status} on the response_format request degrades to a no-format retry that succeeds`, async () => {
    ...
    const fetchFn = async (_url: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (call === 1) {
        return new Response("no json mode", { status });
      }
      return jsonResponse({
...
function captureBodyFetch(
  onBody: (body: CapturedRequestBody) => void,
  response?: Response,
) {
  const fn = async (_url: RequestInfo | URL, init?: RequestInit) => {
    onBody(parseRequestBody(init));
    return (
...
test("CP-NODE-10: outputSchema is attached via response_format json_schema + guided_json/nvext.guided_json", async () => {
  ...
  if (captured.response_format?.json_schema === undefined) {
    throw new Error("expected a captured json_schema response format");
  }
...
for (const status of [400, 422]) {
  test(`CP-NODE-10: HTTP ${status} on the json_schema request degrades to a json_object retry`, async () => {
    ...
    const fetchFn = async (_url: RequestInfo | URL, init?: RequestInit) => {
      ...
      bodies.push(parseRequestBody(init));
      if (call === 1) {
        return new Response("no guided decoding", { status });
      }
      return jsonResponse({
        choices: [{ message: { content: JSON.stringify({ files: [], result: { ok: true } }) } }],
      });
    ...
    if (bodies[0].response_format === undefined) {
      throw new Error("expected a captured schema response format");
    }
...
test("CP-NODE-10: with guided_json wanted but json_object disabled, a schema-form rejection degrades straight to none", async () => {
  ...
  const fetchFn = async (_url: RequestInfo | URL, init?: RequestInit) => {
    ...
    bodies.push(parseRequestBody(init));
    if (call === 1) {
      return new Response("no schema", { status: 400 });
    }
    return jsonResponse({
      choices: [{ message: { content: JSON.stringify({ files: [], result: {} }) } }],
    });
...
test("createFreshSessionProvider constructs the openai-compatible provider", () => {
  const unavailableProvider = (name: string): FreshSessionProvider => ({
    name,
    launch: async () => ({ accepted: false }),
  });
  const deps = {
    orchestratorName: "test",
    createClaudeCodeProvider: () => unavailableProvider("claude-code"),
    createClaudeWorkerProvider: () => unavailableProvider("claude-worker"),
    createOpenCodeProvider: () => unavailableProvider("opencode"),
    createAgyProvider: () => unavailableProvider("agy"),
  };
...
const { extractReportedCostUsd, extractObservedUsage } = await import(
  "../../src/shared/providers/openAiCompatibleProvider.js"
);
...
function fakeFetchWithCost(content: unknown, cost: unknown) {
  const fn = async (): Promise<Response> =>
    jsonResponse({
...
function normalizeHeadersPreservingCase(
...
function recordingFetch(content: string) {
  ...
  const fn = async (url: RequestInfo | URL, init?: RequestInit) => {
    calls.push({
      url,
      init: { headers: normalizeHeadersPreservingCase(init?.headers) },
    });
