...
} from "../../src/shared/friction/frictionRecord.js";
...
const {
  ...
  FRICTION_CATEGORIES,
} = await import("../../src/shared/friction/triage.js");
const { captureStepBoundaryFriction, stepBoundaryFrictionCategory } = await import(
  "../../src/shared/friction/stepBoundaryCapture.js"
);
...
const evt = (
  ...
  artifact: string,
  extra: Pick<CapturedFrictionItem, "tokens"> = {},
): CapturedFrictionItem => ({
  id,
  note: `note-${id}`,
...
test("decideFrictionTriage pre-populates a category from N same-artifact mechanical events", async () => {
  ...
  try {
    ...
    if (!record.open_observations) throw new Error("expected derived observations");
...
test("decideFrictionTriage: re-derive is idempotent and PRESERVES host-authored observations", async () => {
  ...
  try {
    ...
    const record = await readRecord(dir, "run-1");
    if (!record.open_observations) throw new Error("expected persisted observations");
