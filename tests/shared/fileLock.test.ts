...
import { rm, writeFile, utimes, stat, readFile } from "node:fs/promises";
...
/**
 * Read back a {@link RunLogger}'s NDJSON file as parsed event records. The log
 * events acquireLock emits (`lock_path`, `timeout_ms`) carry fields outside the
 * declared RunLogEvent contract — fileLock.ts itself casts them `as never` on
 * write — so this reads the real file a real RunLogger wrote (matching the
 * pattern in shared-core-invariants.test.ts's INV-shared-core-14 tests) rather
 * than faking a logger double: RunLogger is a class with private fields, so no
 * plain object literal can satisfy `logger?: RunLogger` structurally.
 */
async function readLoggedEvents(logPath: string): Promise<Record<string, unknown>[]> {
    ...
    try {
      const err = await assert.rejects(
        ...
        (e) => {
          ...
          expect(thrown.name, "error.name should be FileLockTimeoutError").toBe("FileLockTimeoutError");
          return true;
        },
      );
...
test("stale-lock removal emits a structured log event with kind=step and note=stale_lock_removed", async () => {
  await withTempDir(async (dir: string) => {
    ...
    const staleLockEvents = events.filter(
...
test("acquireLock does not call stat on every retry cycle", async () => {
  await withTempDir(async (dir: string) => {
    ...
    try {
      ...
      if ((err as Error).name === "FileLockTimeoutError") {
        timedOut = true;
      } else {
        throw err;
      }
...
test("acquireLock uses exponential backoff — far fewer wakeups than a fixed 50ms poll", async () => {
  await withTempDir(async (dir: string) => {
    ...
    let sleepCalls = 0;
    const fakeSetTimeout = ((fn: (...args: unknown[]) => void, ms?: number, ...rest: unknown[]) => {
      sleepCalls++;
