...
import { buildSourcePools } from "../../src/shared/quota/apiPool.js";
import { PROVIDER_CONFIRMATION_INPUT_VERSION } from "../../src/shared/types/providerConfirmation.js";
import type { SessionConfig, ResolvedProviderName } from "../../src/shared/types/sessionConfig.js";
...
const backend = (
  ...
  endpoint?: string,
  service?: string,
): ExcludableBackend => ({ transport, model, endpoint, service });
...
describe("the operator's explicit decision is persisted reach-free", () => {
  ...
  test("a `transport:provider/model` rule marks the pool entry whose model it names", () => {
    ...
    // not.
    const config: SessionConfig = { openai_compatible: { model: "model-a", base_url: "https://x.invalid" } };
    ...
    const entryOf = (c: typeof hit) => c.provider_pool.find((e) => e.name === "openai-compatible");
...
const MULTI_SOURCE_CONFIG: SessionConfig = {
  sources: [
    {
      id: "nim-a",
...
async function buildExcluding(patterns: string[] | undefined) {
  return await buildSourcePools({
    sessionConfig: MULTI_SOURCE_CONFIG,
    primaryProviderName: "claude-code",
    quotaSource: STUB_QUOTA,
    quotaEntries: {},
    // No confirmed capability ranks in scope for this test — the floor fails
    // open on `null` (the explicit "no confirmation in scope" answer), never
    // silently omitted (the field is required on the real contract).
    capabilityRanks: null,
    ...(patterns ? { excludedBackends: resolveDispatchExclusion({ exclude: patterns }, {}) } : {}),
  });
}

async function poolIdsExcluding(patterns: string[] | undefined) {
...
describe("readConfirmedDispatchPolicy — the artifact round-trip", () => {
  async function rootWithConfirmation(exclude: string[], include: ResolvedProviderName[]) {
...
describe("B+D — a rejected confirmation is LOUD, never silent", () => {
  const rejectedFor = async (confirmation: Parameters<typeof writeSharedProviderConfirmation>[1]) => {
    const root = await mkdtemp(join(tmpdir(), "reject-"));
    await writeSharedProviderConfirmation(root, confirmation);
    const written: string[] = [];
    const original = process.stderr.write;
    process.stderr.write = ((chunk: unknown) => { written.push(String(chunk)); return true; }) as typeof process.stderr.write;
    try {
  ...
  test("a schema_version drift warns, naming the version and the consequence", async () => {
    ...
    } as unknown as Parameters<typeof writeSharedProviderConfirmation>[1]);
...
describe("the capacity guard: a policy that zeroes ALL reach must not be silent", () => {
  ...
  test("excludedBy names the pattern responsible, and never disagrees with excludes", async () => {
    ...
    const nimA: ExcludableBackend = { transport: "openai-compatible", model: "model-a" };
    const nimB: ExcludableBackend = { transport: "openai-compatible", model: "model-b" };
    const ocA: ExcludableBackend = { transport: "opencode", model: "model-b" };
...
const submissionExcluding = (exclude: unknown[]) => ({
...
describe("unmatchedExclusionPatterns — the non-blocking zero-match signal", () => {
  const BACKENDS: ExcludableBackend[] = [
