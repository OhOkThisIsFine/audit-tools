...
//               a proxy-setup example was tracked in ZERO rows and nothing
//               caught it. That file has since been retired;
...
/**
 * `reconcile`'s destructured default (`constitutionalPaths = []`) infers as
 * `never[]` through allowJs inference of the untyped .mjs export; this restates
 * the real contract so genuine call sites (a real `string[]`) typecheck.
 */
type ReconcileArgs = {
  manifest: ManifestRow[];
  onDisk: string[];
  guidelinesText: string;
  constitutionalPaths?: readonly string[];
};
...
const guidelinesFor = (manifest: ManifestRow[]) =>
...
describe('the live manifest reconciles against the whole tracked tree', () => {
  ...
  it('reach extends past `docs/` — the files the old listing could not see', () => {
    ...
    // Pin only STRUCTURALLY durable files. This list once named the transient
    // proxy-setup example that exposed the hole;
