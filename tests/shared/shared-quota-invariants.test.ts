...
import {
  ...
  type CapacityPool,
  type ComputeDispatchCapacityInput,
...
test("INV-shared-quota-02: mixed pools (one with limit, one without) are independent — no global cap", async () => {
  const poolWithLimit: CapacityPool = {
...
test("INV-shared-quota-03: detectLivelock returns terminal at noProgressLimit", async () => {
  ...
  if (result === null) throw new Error("expected a terminal");
...
test("INV-shared-quota-09: scheduleWave max_concurrent is always >= 1", async () => {
  ...
  const extremeCases: Partial<ScheduleWaveOptions>[] = [
...
test("INV-shared-quota-11: CRIT-name-canonical-tier-field — capacity module rank field uses DispatchModelTier only", async () => {
  ...
  for (const alloc of capacity.pools) {
    const rank = alloc.rank;
    if (rank != null) {
      expect(!NON_CANONICAL.has(rank), `PoolDispatchAllocation.rank "${rank}" must not use a CapabilityTier value — CRIT-name-canonical-tier-field`).toBeTruthy();
      expect(["small", "standard", "deep"].includes(rank), `PoolDispatchAllocation.rank "${rank}" must be a canonical DispatchModelTier value`).toBeTruthy();
    }
  }
});
...
test("INV-shared-quota-14: INV-QD-02 — total_slots >= 1 across pool/quota configurations", async () => {
  ...
  function pool(id: string, overrides: Partial<CapacityPool> = {}): CapacityPool {
    return {
      id,
      accountKey: id,
      providerName: "claude-code",
  ...
  const configs: ComputeDispatchCapacityInput[] = [
    ...
    {
      pools: [pool("a", { quotaSourceSnapshot: {
        remaining_pct: 0.0, reset_at: null, requests_remaining: null,
        tokens_remaining: null, captured_at: "2026-07-28T00:00:00.000Z", source: "test",
      } })],
      sessionConfig: { quota: {} },
