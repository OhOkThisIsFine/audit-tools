...
import { TOKEN_BUDGET_COLD_START_SLOTS } from "../../src/shared/quota/scheduler.js";

function withQuotaFile<T>(contract: unknown, fn: (file: string) => T): T {
...
test("scheduleWave stamps calibrating=true at cold start (percent-only snapshot, no learned slope)", () => {
  ...
  // No real token budget derivable yet → calibrating, and the wave clamps to the
  // cold-start concurrency default (distinct from the host-grant's 1-packet probe,
  // which admissionLoop.ts sizes separately via deriveColdStartAdmissionBatch).
