...
import { applyTemplate, SubprocessTemplateProvider } from "../../src/shared/providers/subprocessTemplateProvider.js";
...
import type { WorkerTaskWithCommand } from "../../src/shared/providers/workerTaskLaunch.js";

// Minimal LaunchFreshSessionInput-shaped object used across tests.
function buildInput(repoRoot = "/repo"): LaunchFreshSessionInput {
  return {
    repoRoot,
    runId: "run-1",
...
function buildTask(workerCommand = ["node", "/repo/worker.js"]): WorkerTaskWithCommand {
...
test("SubprocessTemplateProvider: unknown placeholder routes to RunLogger, not console.warn, when runLogger is provided", async () => {
  ...
  const provider = new SubprocessTemplateProvider(
    ...
    "test-provider",
    async (command: string, args: string[], launchInput: LaunchFreshSessionInput) => {
...
test("SubprocessTemplateProvider: no error thrown when no runLogger and unknown placeholder encountered", async () => {
