...
} from "../../src/shared/providers/proxyCatalog.js";
...
/** Roster / `/model/info` entries: the wire shape is loosely typed upstream too. */
type RosterEntry = Record<string, unknown>;
type ModelInfoEntry = Record<string, unknown>;

/** Parse a mocked `/v1/messages` probe request body (always JSON text in these fixtures). */
function parseProbeBody(body: BodyInit | null | undefined): { model?: string } {
...
function neutralContractFetch(
  { roster = [], modelInfo = null }: { roster?: RosterEntry[]; modelInfo?: ModelInfoEntry[] | null } = {},
  { probeHandler }: { probeHandler?: (options: RequestInit) => Response | Promise<Response> } = {},
): typeof fetch {
  return async (url: RequestInfo | URL, options?: RequestInit): Promise<Response> => {
    ...
    if (urlStr === `${PROXY}/v1/models`) {
      return new Response(JSON.stringify({ data: roster, object: "list" }), { status: 200 });
    }
    ...
    if (urlStr === `${PROXY}/model/info`) {
      ...
      return new Response(JSON.stringify({ data: modelInfo }), { status: 200 });
    }
...
describe("populateProxyCatalog — discovery contract", () => {
  it("discovers roster via /v1/models and enriches via /model/info (array form)", async () => {
    ...
    if (catalog === null) throw new Error("unreachable: asserted not null above");
...
describe("populateProxyCatalog — degrade, never throw", () => {
  ...
  it("unparseable /v1/models (missing data[]) returns written:false", async () => {
    const result = await populateProxyCatalog({
      endpoint: PROXY,
      homeDir: tempHome(),
      fetchImpl: async (url: RequestInfo | URL): Promise<Response> => {
        if (String(url).includes("/v1/models")) {
          // Missing data[]
          return new Response(JSON.stringify({ not_data: "invalid" }), { status: 200 });
        }
...
describe("readProxyCatalog — read-side age rule", () => {
  ...
  function readAged(agedMs: number, { fetchedAt }: { fetchedAt?: string } = {}) {
  ...
  it("a cache inside the max read age is fresh, carrying no reason", () => {
    ...
    if (catalog === null) throw new Error("unreachable: readAged returned null unexpectedly");
  ...
  it("models probed 200 are kept; 404 dropped with reason", async () => {
    ...
    const result = await populateProxyCatalog({
      ...
      fetchImpl: neutralContractFetch(
        ...
        {
          probeHandler: (options: RequestInit): Response => {
            probeCount++;
            const body = parseProbeBody(options.body);
