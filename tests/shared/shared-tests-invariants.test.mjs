...
// ── INV-shared-tests-08: no test roots a fixture tree at its own source dir ───
// A fixture tree written under `tests/**` leaves the working tree dirty after a
// run, so `git add -A` sweeps test residue into a commit — and a dir a LATER run
// deleted stages as an `AD` phantom deletion. `tests/helpers/scratch.ts` exists
// to make that unrepresentable (per-INVOCATION root under os.tmpdir()), but the
// helper only helps the callers that use it: after the migration ONE writer
// survived, anchoring a fixture at the test file's own directory, and was
// invisible because a `.gitignore` stopgap had been hiding the residue. A second
// survivor was found only when this guard first ran.
//
// So the property is enforced here rather than remembered. A repo-root
// resolution (`join(__dirname, "..", "..")`) is untouched — this matches only a
// NEW scratch/fixture dir anchored at the test file's own location.
//
// The pattern below is assembled from fragments deliberately: written out whole,
// this file's own source would match its own guard.
test("INV-shared-tests-08: no test file roots a scratch fixture dir at __dirname (use scratchDir)", () => {
  const TESTS_ROOT = resolve(__dirname, "..");
  const AREAS = ["audit", "shared", "remediate", "helpers"];
  const ANCHOR = "(?:join|resolve)\\(\\s*(?:__dirname|here|import\\.meta\\.dirname)\\s*,\\s*";
  const FIXTURE_NAME = "[`\"'](?:\\.test-|\\.tmp-|\\.scratch)";
  const IN_TREE_FIXTURE = new RegExp(ANCHOR + FIXTURE_NAME);
  ...
  const walk = (dir) => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) {
        walk(full);
      } else if (/\.test\.(mjs|ts)$/.test(e.name) || e.name.endsWith(".ts")) {
        scanned += 1;
        const source = readFileSync(full, "utf8");
        if (IN_TREE_FIXTURE.test(source)) {
          violations.push(full.slice(TESTS_ROOT.length + 1).replace(/\\/g, "/"));
        }
      }
    }
  };
  for (const area of AREAS) {
    const dir = resolve(TESTS_ROOT, area);
    if (existsSync(dir)) walk(dir);
  }

  expect(
    violations,
    `These test files root a fixture dir at their own source location, which leaves ` +
      `in-tree residue a 'git add -A' can commit. Use scratchDir() from ` +
      `tests/helpers/scratch.ts instead: ${violations.join(", ")}`,
  ).toEqual([]);
  // A broken walker must not pass by scanning nothing.
  expect(scanned > 50, `INV-shared-tests-08 must scan the test tree (scanned ${scanned})`).toBeTruthy();
});
