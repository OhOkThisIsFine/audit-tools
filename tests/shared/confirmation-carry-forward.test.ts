...
} from "../../src/shared/providers/sharedProviderConfirmation.js";
import type {
  ConfirmedDispatchPolicy,
  NewlyReachableBackend,
  SharedProviderConfirmation,
} from "../../src/shared/providers/sharedProviderConfirmation.js";
import { PROVIDER_CONFIRMATION_INPUT_VERSION } from "../../src/shared/types/providerConfirmation.js";
import type { ProviderConfirmationInput } from "../../src/shared/types/providerConfirmation.js";
import type {
  DispatchableSource,
  SessionConfig,
...
import { runProviderConfirmationAutoComplete } from "../../src/audit/orchestrator/intakeExecutors.js";
import type { ProviderConfirmationGateState } from "../../src/audit/orchestrator/advanceTypes.js";
...
const NO_CLI = (): boolean => false;

/**
 * The confirmation as it lands on disk, read back with `JSON.parse`. `policy` and the
 * two orderings the tests below read off it are declared present: every call site
 * writes a confirmation through the real writer first, so an absent one is a broken
 * setup rather than a case to branch on.
 */
interface PersistedConfirmation extends Omit<SharedProviderConfirmation, "policy"> {
...
/**
 * The same narrowing for the sites that read the confirmation back through the REAL
 * reader: a `null` read (or one carrying no policy block) is a broken setup, so it is
 * refused outright rather than branched on.
 */
function withPolicy(value: SharedProviderConfirmation | null) {
  if (!value) throw new Error("no provider confirmation was persisted");
  const { policy } = value;
  if (!policy) throw new Error("the persisted provider confirmation carries no policy");
  return { ...value, policy };
}

/** {@link withPolicy}, for the sites that read the confirmed ordering itself. */
function withCapabilityOrder(value: SharedProviderConfirmation | null) {
  const confirmation = withPolicy(value);
  const { capability_order } = confirmation.policy;
  if (!capability_order) {
    throw new Error("the persisted policy carries no capability_order");
  }
  return { ...confirmation, policy: { ...confirmation.policy, capability_order } };
}
...
// not destroy.
const PRIOR_EXCLUDE: string[] = ["agy", "integrate.api.nvidia.com"];
const PRIOR_INPUT: ProviderConfirmationInput = {
...
function buildPrior(
  input: ProviderConfirmationInput = PRIOR_INPUT,
  exclude: string[] = PRIOR_EXCLUDE,
): SharedProviderConfirmation {
 ...
 */
async function promote(root: string, artifactsDir: string | undefined) {
...
async function readPersisted(root: string): Promise<PersistedConfirmation> {
...
describe("HEADLINE: a capability-only submission destroys nothing (real files, real parser)", () => {
  const CAPABILITY_ONLY: ProviderConfirmationInput = {
    schema_version: PROVIDER_CONFIRMATION_INPUT_VERSION,
  ...
  async function runHeadline(root: string) {
  ...
  test("the real PARSER preserves the file's shape into the carried input", async () => {
    await withTempRoot(async (root) => {
      ...
      if (effectiveInput === null) {
        throw new Error("unreachable: a prior confirmation is on disk, so a carry exists");
      }
...
describe("input === null (autonomous re-promotion) carries the prior decision forward", () => {
  ...
  test("carryForwardConfirmationInput(null, prior) synthesizes a full input", () => {
    ...
    if (carried === null) throw new Error("unreachable: asserted not null above");
...
describe("an explicit empty array means DELETE; omission means said-nothing", () => {
  test("the parser PRESERVES an explicit host_models: [] rather than dropping it", () => {
    // Non-null: the object above IS a valid submission, so a null parse is the
    // very defect the assertions below pin.
...
describe("no prior confirmation: the input passes through untouched", () => {
  test("a submission is returned unchanged (same value, nothing invented)", () => {
    const input: ProviderConfirmationInput = {
      schema_version: PROVIDER_CONFIRMATION_INPUT_VERSION,
      capability_order: ["model-alpha"],
    };
...
const NEW_BACKEND: NewlyReachableBackend = {
  key: "brand-new-model",
...
async function writeInputFile(
  artifactsDir: string,
  body: Omit<ProviderConfirmationInput, "schema_version">,
) {
  await writeFile(
    join(artifactsDir, PROVIDER_CONFIRMATION_INPUT_FILENAME),
...
describe("B1 PROVENANCE: an AUTO exclusion is superseded by the next submission; an OPERATOR one is not", () => {
  ...
  test("an OPT-BACK-IN supersedes the placeholder: the backend becomes dispatchable again", async () => {
    await withTempRoot(async (root) => {
      ...
      await runProviderConfirmationAutoComplete(
        {}, root, artifactsDir, CONFIG,
        { newlyReachable: [], unevidencedCapability: [], autonomous: false },
      );
  ...
  test("an LLM-authored submission's include/exclude are STRIPPED: the fail-closed placeholder survives", async () => {
    await withTempRoot(async (root) => {
      ...
      await runProviderConfirmationAutoComplete(
        {}, root, artifactsDir, CONFIG,
        { newlyReachable: [NEW_BACKEND], unevidencedCapability: [], autonomous: true },
      );
      expect((await readPersisted(root)).policy?.auto_exclude).toEqual([
        NEW_BACKEND.exclusion_pattern,
      ]);
...
describe("B2 PROVENANCE: external capability evidence never becomes the operator's answer", () => {
  ...
  const source = (
    id: string,
    model: string,
    extra: Partial<DispatchableSource> = {},
  ): DispatchableSource => ({
    id,
    transport: "openai-compatible",
  ...
  const configWith = (sources: DispatchableSource[]): SessionConfig => ({
  ...
  async function seed(root: string, artifactsDir: string) {
    await mkdir(artifactsDir, { recursive: true });
    await writeInputFile(artifactsDir, {
      host_models: HOST,
...
describe("BL-1 REGRESSION: two successive PARTIAL answers still converge to an empty delta", () => {
  const src = (model: string): DispatchableSource => ({
    id: `${model}-src`,
    transport: "openai-compatible",
  ...
  const configWith = (models: string[]): SessionConfig => ({
    provider: "claude-code",
  ...
  test("a partial answer does not disturb the other confirmed decisions either", async () => {
    await withTempRoot(async (root) => {
      ...
      const persisted = withPolicy(await readSharedProviderConfirmation(root));
...
import { detectDiscardedCapabilityReorder } from "../../src/shared/providers/sharedProviderConfirmation.js";
 ...
 */
function analyzeDiscard(prior: string[], submitted: string[]) {
...
describe("END TO END: a discarded reorder is reported by the real executor", () => {
  ...
  test("a NEW model in the same submission is still placed, exactly as the warning says", async () => {
    await withTempRoot(async (root) => {
      ...
      const persisted = withCapabilityOrder(await readSharedProviderConfirmation(root));
...
describe("renderConfirmationSummary ACCUMULATES: co-occurring outcomes are ALL reported", () => {
  ...
  const gate = (unevidencedCapability: string[] = []): ProviderConfirmationGateState => ({
    newlyReachable: [],
    unevidencedCapability,
    autonomous: true,
  });
  ...
  async function promote(
    ...
    artifactsDir: string,
    body: Omit<ProviderConfirmationInput, "schema_version">,
    gateObj: ProviderConfirmationGateState = gate(),
  ) {
    await writeInputFile(artifactsDir, body);
    return await runProviderConfirmationAutoComplete(
      {},
...
const configWithModels = (models: string[]): SessionConfig => ({
  provider: "claude-code",
  sources: models.map((model): DispatchableSource => ({
    id: `${model}-src`,
    transport: "openai-compatible",
    endpoint: `http://${model}.local/v1`,
    model,
...
describe("R3-3: supersession — an operator submission repositions a previously LLM-ranked id", () => {
  test("repositioning two LLM-ranked ids is HONORED (not discarded) and removes them from capability_order_llm_ranked; a third, un-named, LLM-ranked id is untouched", async () => {
    await withTempRoot(async (root) => {
      const artifactsDir = join(root, ".audit-tools", "audit");
      await mkdir(artifactsDir, { recursive: true });
      const config: SessionConfig = { provider: "claude-code" };
...
// ─────────────────────────────────────────────────────────────────────────────
import { retainAutoExclusions as retainAutoExclusionsFn } from "../../src/shared/providers/sharedProviderConfirmation.js";
...
describe("retainAutoExclusions — an explicit include clears the matching auto-exclusion", () => {
  ...
  test("the model tier of the same transport is cleared too", () => {
    expect(
      retainAutoExclusionsFn(["transport:codex/model-a"], {
        schema_version: PROVIDER_CONFIRMATION_INPUT_VERSION,
        include: ["codex"],
      }),
  ...
  test("a host-axis exclusion is never cleared by a provider opt-in", () => {
    ...
    expect(
      retainAutoExclusionsFn(["host:localhost"], {
        schema_version: PROVIDER_CONFIRMATION_INPUT_VERSION,
        include: ["codex"],
      }),
...
describe("a zero-match operator exclusion is REPORTED, and the promotion still succeeds", () => {
  const CONFIG_ADVISORY: SessionConfig = {
    // `provider: "claude-code"` keeps the primary fold inert, so the executor gathers
  ...
  const ATTENDED_GATE: ProviderConfirmationGateState = {
    newlyReachable: [],
    unevidencedCapability: [],
    autonomous: false,
  };

  async function promoteExcluding(
    root: string,
    artifactsDir: string,
    exclude: string[],
  ) {
