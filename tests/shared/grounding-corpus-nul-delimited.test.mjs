...
const { enumerateTrackedFilePaths } = await import(
  "../../src/shared/validation/findingGrounding.ts"
);
const { enumerateRepoTreePaths } = await import(
...
const nfcSet = (paths) => new Set([...paths].map(nfc));

/** Throwaway repo tracking one plain and one non-ASCII path. */
async function withUnicodeRepo(fn) {
