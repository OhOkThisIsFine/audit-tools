...
const { resolveSessionConfig } = await import(
  "../../src/shared/config/resolveSessionConfig.js"
);
const { ambientAuditorDescriptor } = await import(
  "../../src/shared/types/auditorDescriptor.js"
);

/**
 * Narrow a possibly-undefined `sources[]` after a runtime `toBeDefined()` check,
 * which proves the value at runtime but does not narrow the TYPE.
 */
...
const ambientDeps = (): AmbientSourceDeps => ({
  env: { NIM_KEY: "sk-real" },
  homeDir: "/home/t",
  commandExists: () => false,
  fileReadable: () => false,
