...
test("reset form 'resets 3:30pm' (clock time) → next-future-occurrence pause, auto-resume", async () => {
  ...
  const reopenedPct = probe.snapshot!.remaining_pct;
  expect(reopenedPct != null && reopenedPct > 0 && reopenedPct < 0.1, `near-wall band after a recorded limit; got ${reopenedPct}`).toBeTruthy();
...
test("reset form 'Resets in 2h' (duration) → ~2h pause, auto-resume", async () => {
  ...
  const resumedPct = resumed.snapshot!.remaining_pct;
  expect(resumedPct != null && resumedPct > 0 && resumedPct < 0.1, "near-wall band after a recorded limit").toBeTruthy();
});
...
test("bounded escalation: same packet re-limiting past the bound escalates to a terminal operator surface", async () => {
  ...
  let last: HostSessionLimitEvent | undefined;
  for (let i = 0; i < 4; i++) {
