...
function makePacket(id: string, estimatedTokens: number): RollingDispatchPacket<Payload> {
  return { id, payload: { id }, estimatedTokens, complexity: 0.5 };
}

async function tmpQuotaDir(): Promise<string> {
...
function sharedMeter() {
  ...
  return {
    dispatchPacket(cost: number, delayMs = 15) {
      return async (
...
test("two co-located dispatchers over one ledger never exceed the shared account budget", async () => {
  ...
  const mkDispatcher = (prefix: string, ledger: ReservationLedger) => {
    const d = createRollingDispatcher<Payload>({
