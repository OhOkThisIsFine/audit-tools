...
} from "../../src/shared/quota/compositeQuotaSource.js";
import { createRollingDispatcher } from "../../src/shared/dispatch/rollingDispatch.js";
import type { QuotaProbeResult, QuotaSource } from "../../src/shared/quota/quotaSource.js";
...
interface FakeClock extends NowFn {
  advance(ms: number): void;
  set(ms: number): void;
}

function fakeClock(startMs: number): FakeClock {
  let t = startMs;
  const now = (() => t) as FakeClock;
...
function alwaysHealthySource(name: string): QuotaSource {
  return {
    name,
    async queryCurrentUsage() {
      return null;
    },
    async probeUsage(): Promise<QuotaProbeResult> {
      return {
        snapshot: {
          remaining_pct: 0.9,
          reset_at: null,
...
function singlePool(): CapacityPool {
  return {
    id: KEY,
    accountKey: KEY,
...
test("buildQuotaSource PREPENDS the host-session source before proactive sources", async () => {
  ...
  if (!composite.probeUsage) throw new Error("composite must implement probeUsage");
  const own = await composite.probeUsage(KEY);
...
test("graduated pre-wall: after a recorded limit the band is below CRITICAL (scheduler throttles pre-429)", async () => {
  ...
  expect((probe.snapshot?.remaining_pct ?? 0) > 0, "graduated, not a hard 0").toBeTruthy();
  expect(
...
test("rollingDispatch consults isPacketEscalated: an escalated packet is STRANDED, not re-queued (INV-QD-07 preserved)", async () => {
  ...
  const dispatcher = createRollingDispatcher({
    ...
    isPacketEscalated: (id: string) => hostSession.isEscalated(id),
  });
