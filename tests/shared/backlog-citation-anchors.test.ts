...
function locateEntry({ file, locator }: { file: string; locator: RegExp }) {
...
function sourceFilesMentioning(pattern: RegExp): string[] {
  const hits: string[] = [];
 ...
 */
function citedRange(token: string): string {
  expect(
    /^[^:]+$/.test(token),
