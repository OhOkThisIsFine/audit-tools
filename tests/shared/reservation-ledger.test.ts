...
async function tempLedger(nowRef: { t: number }, ttlMs: number) {
...
function one(resourceKey: string, cost: number, budget: number, poolId: string): AdmitInput {
  return { constraints: [{ resourceKey, budget, cost }], poolId };
}

/** Narrow an AdmitDecision's binding to non-null — only used where the caller
 *  supplied constraints, so binding is always set (see AdmitDecision.binding). */
function bindingOf(d: AdmitDecision): ConstraintOutcome {
  if (!d.binding) throw new Error("expected a binding constraint");
  return d.binding;
}

/** Narrow an AdmitDecision's leaseId to non-null — only used where admission succeeded. */
function leaseIdOf(d: AdmitDecision): string {
  if (!d.leaseId) throw new Error("expected a lease id");
  return d.leaseId;
}
...
test("admits when budget covers cost; outstanding + headroom reflect the lease", async () => {
  ...
  try {
    ...
    expect(leaseIdOf(d)).toBeTruthy();
    expect(bindingOf(d).outstandingBefore).toBe(0);
    expect(bindingOf(d).headroomBefore).toBe(1000);
...
test("second admit sees the first's outstanding lease and blocks when budget is exhausted", async () => {
  ...
  try {
    ...
    expect(bindingOf(second).outstandingBefore).toBe(700);
    expect(bindingOf(second).headroomBefore).toBe(300);
...
test("reconcile frees the reservation and is token-checked", async () => {
  ...
  try {
    ...
    expect(await ledger.reconcile(leaseIdOf(d))).toBe(true);
    expect(await ledger.outstanding(KEY)).toBe(0);
    // Reconciling an already-freed lease is a no-op.
    expect(await ledger.reconcile(leaseIdOf(d))).toBe(false);
...
test("non-finite budget admits optimistically (unbounded start; reactive floor corrects)", async () => {
  ...
  try {
    ...
    expect(d.admitted).toBe(true);
    expect(Number.isFinite(bindingOf(d).headroomBefore)).toBe(false);
  } finally {
    await cleanup();
  }
});
...
test("two ledgers on the same file share outstanding (co-located account budget)", async () => {
  ...
  try {
    ...
    expect(bindingOf(peerAttempt).outstandingBefore).toBe(600);
  } finally {
    await cleanup();
  }
});
...
test("a model-scoped window blocks even when the shared account window has room", async () => {
  ...
  try {
    ...
    expect(bindingOf(d).resourceKey).toBe(MODEL_SESSION);
  } finally {
    await cleanup();
  }
});
...
test("siblings on one account share the account window; each keeps its own model window", async () => {
  ...
  try {
    ...
    expect(second.admitted).toBe(false);
    expect(bindingOf(second).resourceKey).toBe(ACCOUNT_SESSION);
    expect(bindingOf(second).headroomBefore).toBe(40);
...
test("windows meter in their OWN unit — binding is the tightest RATIO, not the smallest headroom", async () => {
  ...
  try {
    ...
    expect(bindingOf(d).resourceKey).toBe(modelTokens);
  } finally {
    await cleanup();
  }
});
...
test("binding does not depend on the order the caller listed constraints in", async () => {
  ...
  try {
    ...
    await ledger.reconcile(leaseIdOf(forward));
    const reversed = await ledger.admit({ poolId: "a", constraints: [loose, tight] });
    expect(bindingOf(forward).resourceKey).toBe(MODEL_SESSION);
    expect(bindingOf(reversed).resourceKey).toBe(MODEL_SESSION);
  } finally {
    await cleanup();
  }
});
...
test("UNEQUAL sibling budgets: the smaller model window binds its own pool only", async () => {
  ...
  try {
    ...
    expect(bindingOf(nanoAgain).resourceKey).toBe(nano);
...
test("an UNCALIBRATED sibling's non-finite budget is unbounded on its own window only", async () => {
  ...
  try {
    ...
    expect(d.admitted).toBe(false);
    expect(bindingOf(d).resourceKey).toBe(account);
  } finally {
    await cleanup();
  }
});
...
test("tied tightness breaks on the key, not on caller order", async () => {
  ...
  try {
    ...
    expect(bindingOf(forward).resourceKey).toBe("aaa::session");
    expect(bindingOf(reversed).resourceKey).toBe("aaa::session");
  } finally {
    await cleanup();
  }
});
