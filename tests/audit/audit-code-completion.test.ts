...
async function callIngestResults(args: string[]) {
  const stdout = await captureConsoleLog(() => withEnvParity(() => cmdIngestResults(args)));
  return JSON.parse(stdout);
}

async function callForceSynthesis(args: string[]) {
...
async function nextStepUntilPresentReport(root: string, extraArgs: string[] = []) {
  const artifactsDir = join(root, ".audit-tools/audit");
