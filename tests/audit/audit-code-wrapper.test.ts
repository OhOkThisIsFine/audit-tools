...
// Explicit file-level timeout, not the 120s default. Most cases here pack a real
// tarball and spawn real subprocesses, taking ~30-100s ALONE — and they are
// `concurrent`, so under a full-suite run they contend with siblings doing the
// same and blow past the default with a ROTATING victim (three different tests
// crossed 120s across three consecutive runs). A test that passes in isolation
// failing only under load teaches a reader to wave at reds, which is the same
// corrosion as a false green. A genuine hang still fails here, just later.
...
function assertMatchesResponseSchema(value: unknown, label: string): void {
...
function spawnWrapper(args: string[], options: WrapperOptions = {}) {
...
function runWrapper(args: string[], options: WrapperOptions = {}): Promise<WrapperOutput> {
...
function assertOpenCodeAuditPermissions(config: any): void {
...
function validAuditResultForTask(task: any, overrides: Record<string, unknown> = {}) {
  return {
...
async function setupMergeFixture(root: string) {
  return setupDispatchFixture(root);
}

async function submitAllPackets(
  root: string,
  runId: string,
  artifactsDir: string,
  plan: any,
...
async function setupSubmitPacketFixture(root: string) {
...
async function assertPacketResultFilesMissing(entries: any[]): Promise<void> {
...
type HostInstallPaths = ReturnType<typeof repoLocalHostInstallPaths>;

function hostGuidance(parsed: InstallResponse, host: string): any {
  const guidance = parsed.host_guidance.find((entry) => entry.host === host);
  expect(guidance, `expected guidance for ${host}`).toBeTruthy();
  return guidance;
}

async function setupRepoLocalHostInstallFixture(root: string) {
...
function assertSharedHostInstallResponse(
