...
test("isolates failures without stopping other tasks", async () => {
  ...
  expect((results[2] as PromiseFulfilledResult<string>).value).toBe("also ok");
});

test("fires onComplete callback for each task", async () => {
  const completed: Array<{ index: number; status: string }> = [];
  const tasks = [
    () => Promise.resolve("a"),
    () => Promise.resolve("b"),
  ];
...
test("handles concurrency greater than task count", async () => {
  ...
  expect((results[1] as PromiseFulfilledResult<number>).value).toBe(2);
});

test("fires onComplete callback for a rejected task", async () => {
  const completed: Array<{ index: number; result: PromiseSettledResult<never> }> = [];
  const tasks = [
  ...
  expect((completed[0].result as PromiseRejectedResult).reason.message).toBe("boom");
});
