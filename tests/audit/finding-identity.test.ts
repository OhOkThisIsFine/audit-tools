...
import { buildAuditReportModel } from "../../src/audit/reporting/synthesis.js";
...
// it is part of the Finding contract itself, but the identity tests below
// deliberately vary it to prove it never influences the derived id.
...
function finding(overrides: Partial<FindingFixture> = {}): FindingFixture {
  return {
    id: "MNT-001",
    title: "A finding",
    category: "smell",
...
function idOf(overrides: Partial<FindingFixture> = {}): string {
...
test("unrelated unit-membership changes leave untouched findings' ids byte-identical", () => {
  ...
  const untouched = (): FindingFixture[] => [
    finding({
      unit_id: "unit-a",
      lens: "correctness",
      category: "",
...
test("anchor tier is stable across volatile fields", () => {
  const anchored = (extra: Partial<FindingFixture> = {}) =>
    idOf({
      affected_files: [
...
test("title normalization strips volatile content", () => {
  ...
  const titleId = (title: string) => idOf({ affected_files: [], category: "", title });
...
test("buildAuditReportModel: colliding per-packet ids become unique and work_blocks round-trip", () => {
  ...
  const report = buildAuditReportModel({ results });
