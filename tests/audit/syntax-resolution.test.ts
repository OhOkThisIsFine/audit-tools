...
async function writeFakeEslint(root: string): Promise<void> {
...
function createBundle(): ArtifactBundle {
  return {
    file_disposition: {
...
function createBundleWithExisting(items: ExternalAnalyzerResultItem[]): ArtifactBundle {
  return {
    file_disposition: {
      files: [{ path: "src/app.js", status: "included" }],
    },
...
test("syntax resolution preserves existing external_analyzer_results and appends new items", async () => {
  await withTempRepo(async (root) => {
    ...
    const preExisting: ExternalAnalyzerResultItem = {
      id: "pre-existing-0",
      category: "maintainability",
      severity: "warning",
      path: "src/app.js",
...
test("syntax resolution deduplicates items with matching path:line_start:rule:summary", async () => {
  await withTempRepo(async (root) => {
    ...
    const duplicate: ExternalAnalyzerResultItem = {
      id: "pre-0",
      category: "maintainability",
      severity: "error",
      path: "src/app.js",
