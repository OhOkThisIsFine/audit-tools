...
const remediateProvidersUrl = new URL(
  "../../src/remediate/providers/index.js",
  import.meta.url,
);
const remediateProviders = await import(remediateProvidersUrl.href);

const auditQuota = await import("../../src/audit/quota/index.js");
const remediateQuotaUrl = new URL(
  "../../src/remediate/quota/index.js",
  import.meta.url,
);
const remediateQuota = await import(remediateQuotaUrl.href);

const auditHostLimits = await import("../../src/audit/quota/hostLimits.js");
const remediateHostLimitsUrl = new URL(
  "../../src/remediate/quota/hostLimits.js",
  import.meta.url,
);
const remediateHostLimits = await import(remediateHostLimitsUrl.href);
...
test("audit-code resolveHostActiveSubagentLimit reads its own AUDIT_CODE_HOST_MAX_ACTIVE_SUBAGENTS", () => {
  const result = requirePresent(
    auditHostLimits.resolveHostActiveSubagentLimit({
      sessionConfig: {},
...
function readProviderSource(relFromSeamTest: string): string {
...
test("per-orchestrator delta is ONLY the claude-code skip-permissions default", async () => {
  ...
  type LaunchCommand = NonNullable<Parameters<CreateProvider>[1]>;
  type LaunchCall = {
    command: Parameters<LaunchCommand>[0];
    args: Parameters<LaunchCommand>[1];
    launchInput: Parameters<LaunchCommand>[2];
  };

  async function argvFor(createProvider: CreateProvider): Promise<LaunchCall> {
