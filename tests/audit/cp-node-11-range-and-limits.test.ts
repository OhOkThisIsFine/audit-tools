...
import { WorkerAuditTaskSchema } from "../../src/audit/contracts/workerSchemas.js";
...
} from "../../src/audit/quota/discoveredLimits.js";
...
test("updateDiscoveredLimits persists context_tokens/output_tokens through the cache round-trip", async () => {
  ...
  await withTempQuotaDir(async () => {
    ...
    if (result === null) throw new Error("expected discovered limits");
...
test("mergeDiscoveredLimits fills context_tokens/output_tokens from later sources", () => {
  ...
  if (result === null) throw new Error("expected merged discovered limits");
