...
import type { DiscoveredRateLimits } from "../../src/audit/quota/index.js";
...
const { detectHostActiveSubagentLimit, resolveHostActiveSubagentLimit } = await import("../../src/audit/quota/hostLimits.js");
const { resolveHostModel } = await import("../../src/audit/quota/index.js");
...
test("scheduleWave caps wave size by host active subagent limit", () => {
  const hostConcurrencyLimit: HostConcurrencyLimit = {
    active_subagents: 6,
    source: "environment",
...
test("scheduleWave: a reported host limit never raises waves above a known RPM cap", () => {
  ...
  const schedule = scheduleWave({
    ...
    discoveredLimits: makeDiscoveredLimits({ requests_per_minute: 3, source: "provider_query" }),
  });
  expect(schedule.max_concurrent).toBe(3);
});
...
test("scheduleWave invents NO ceiling for an unconfigured local provider", () => {
  ...
  for (const providerName of ["opencode", "worker-command"] satisfies ResolvedProviderName[]) {
    const schedule = scheduleWave({
      providerName,
      sessionConfig: {},
      hostModel: null,
...
test("scheduleWave dispatches the full wave when discovered limits are generous", () => {
  const schedule = scheduleWave({
    ...
    requestedConcurrency: 22,
    quotaStateEntry: null,
    discoveredLimits: makeDiscoveredLimits({ requests_per_minute: 20, source: "header_extraction" }),
  });
...
test("detectHostActiveSubagentLimit discovers Codex agents.max_threads from config", () => {
  ...
  if (limit === null) throw new Error("expected a non-null discovered limit");
...
test("detectHostActiveSubagentLimit respects AUDIT_CODE_HOST_MAX_ACTIVE_SUBAGENTS env var", () => {
  ...
  if (limit === null) throw new Error("expected a non-null limit from env var");
