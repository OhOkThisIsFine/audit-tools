...
type PythonFromImportTarget = { specifier: string; target: string };
 ...
 */
function makeLookup(paths: string[]): Map<string, string> {
...
describe("resolvePythonFromImportTargets skips star imports and non-identifier names", () => {
  ...
  it("star import is excluded from results", () => {
    ...
    const hasStarEntry = results.some((r: PythonFromImportTarget) => r.specifier.endsWith(".*"));
  ...
  it("non-identifier name is excluded from results", () => {
    ...
    const has123 = results.some((r: PythonFromImportTarget) => r.specifier.includes("123bad"));
...
await test("pythonLogicalLines merges backslash-continued import lines", () => {
  ...
  const targets = edges.map((e: GraphEdge) => e.to);
...
await test("pythonLogicalLines flushes a line with unbalanced close paren rather than leaving it dangling", () => {
  ...
  expect(!targets.some((t: string) => t.includes("os)")), "stray `)` is not part of a resolved module name").toBeTruthy();
});
