...
type CaptureResult = Awaited<ReturnType<typeof captureConsole>>;
...
async function runValidate(root: string): Promise<CaptureResult> {
  const argv = [
    process.execPath,
...
function parseJsonOutput(result: CaptureResult): ValidationOutput {
  try {
