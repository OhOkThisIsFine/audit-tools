...
test("runRollingDispatch: dispatches items and calls onResult once per item", async () => {
  ...
  const terminalCalls: { status: string; count: number }[] = [];
  ...
  const dispatchPacket = async (
    packet: RollingDispatchPacket<string>,
    _slot: { providerName: string; hostModel: string | null; poolId: string },
  ): Promise<RollingDispatchResult<string>> => {
    dispatchCount++;
...
test("runRollingDispatch: one packet outcome:failed is included in results with outcome failed", async () => {
  const onResultCalls: { id: string; outcome: string }[] = [];
  ...
  const dispatchPacket = async (
    ...
    if (packet.id === "p-fail") {
      // @ts-expect-error deliberate contract-violation probe (TST-30655614): "failed"
      // is not a member of RollingDispatchResult["outcome"] — this exercises the
      // engine's generic pass-through for a dispatcher-supplied outcome outside the
      // known enum, which must still be recorded rather than silently stranded.
...
test("both shared APIs compose end-to-end without throwing", async () => {
  ...
  const onResultIds: string[] = [];
  const contract: Partial<RollingDispatchEngineContract<{ lens: string }>> = {
    livelockGuard: 3,
