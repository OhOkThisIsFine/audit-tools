...
import type { AnalyzerOutput } from "../../src/audit/extractors/analyzers/types.js";

const { typescriptAnalyzer } = await import("../../src/audit/extractors/analyzers/typescript.js");
const { buildPathLookup } = await import("../../src/audit/extractors/graph.js");
...
function repoManifest(): RepoManifest {
  return {
    repository: { name: "ts-analyzer-fixture" },
...
test("typescript analyzer returns no edges for an empty file set", async () => {
  ...
  try {
    const output = await typescriptAnalyzer.analyze([], {
      root,
      repoManifest: { repository: { name: "empty" }, generated_at: "2026-01-01T00:00:00Z", files: [] },
      includedFiles: [],
      pathLookup: new Map(),
    });
...
async function analyzeFixture(files: Record<string, string>): Promise<AnalyzerOutput> {
  ...
  try {
    ...
    const repo: RepoManifest = {
      repository: { name: "ts-cfe-fixture" },
      generated_at: "2026-01-01T00:00:00Z",
      files: Object.keys(files).map((path) => ({
        path,
        size_bytes: files[path].length,
