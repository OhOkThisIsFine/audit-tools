...
const { renderRollingDispatchPrompt } = await import("../../src/audit/cli/prompts.js");
const { buildAnalyzerSignalAnchorIndex } = await import(
  "../../src/audit/orchestrator/fileAnchors.js"
);
...
function makeSessionConfig(
  overrides: NonNullable<SessionConfig["dispatch"]> = {},
): SessionConfig {
...
function makeLensDefs(): Record<string, { description: string; do_not_report: string }> {
  return {
...
test("buildTaskSections — renders external analyzer signal detail for tagged tasks, not just the generic tag", () => {
  ...
  const lensDefs = makeLensDefs();
  const lineIndex = { "src/t1.ts": 50 };
  const externalAnalyzerResults: ExternalAnalyzerResults[] = [
    {
      tool: "knip",
      results: [
        {
...
test("buildTaskSections — caps analyzer signal lines at 24 with an omitted-count footer (N4)", () => {
  ...
  const results: ExternalAnalyzerResults["results"] = Array.from({ length: 30 }, (_, i) => ({
    id: `knip-${i}`,
    category: "unused-export",
    severity: "low",
    path: "src/t1.ts",
  ...
  const externalAnalyzerResults: ExternalAnalyzerResults[] = [{ tool: "knip", results }];
...
test("buildTaskSections — renders the knip↔graph cross-check tag inline for a knip lead (CP-NODE-2)", async () => {
  const { buildKnipGraphIndex } = await import(
    "../../src/audit/orchestrator/knipGraphCrosscheck.js"
  );
  const task = { ...makeAuditTask("t1"), tags: ["external_analyzer_signal"] };
  const lensDefs = makeLensDefs();
  const lineIndex = { "src/t1.ts": 50 };
...
function makeWaveSchedule(
  confidence: string,
  contextTokens = 10000,
  outputTokens = 2000,
): Parameters<typeof collectOversizedWarnings>[1] {
  return {
    confidence,
...
): Parameters<typeof collectOversizedWarnings>[0][number] {
  return {
    packet_id: packetId,
...
test("resolveTierBudgets — direct entries survive unchanged", () => {
  const perRank = new Map<DispatchModelTier, number>([["small", 1000], ["standard", 2000], ["deep", 4000]]);
...
test("resolveTierBudgets — missing tier falls back to lower rank (not higher) when both equidistant", () => {
  ...
  const perRank = new Map<DispatchModelTier, number>([["small", 1000], ["deep", 4000]]);
  const out = resolveTierBudgets(perRank);
...
test("resolveTierBudgets — missing 'small' takes the nearest lower rank and does not inherit deep (COR-0e031ac0)", () => {
  ...
  const perRank = new Map<DispatchModelTier, number>([["standard", 2000], ["deep", 4000]]);
  const out = resolveTierBudgets(perRank);
...
test("resolveTierBudgets — missing 'deep' takes the nearest higher rank (standard, not small)", () => {
  ...
  const perRank = new Map<DispatchModelTier, number>([["small", 1000], ["standard", 2000]]);
  const out = resolveTierBudgets(perRank);
