...
test("all EXECUTOR_REGISTRY entries have a valid kind field", () => {
  ...
  expect(hostEntries.map((e) => e.id).sort(), "Exactly 'agent', 'charter_extraction_executor', 'design_review_contract', 'design_review_conceptual', 'intent_checkpoint_executor', 'provider_confirmation_executor', 'rolling_dispatch_executor', and 'synthesis_narrative_executor' should have kind host_delegation").toEqual(["agent", "charter_extraction_executor", "design_review_conceptual", "design_review_contract", "intent_checkpoint_executor", "provider_confirmation_executor", "rolling_dispatch_executor", "synthesis_narrative_executor"]);
});
