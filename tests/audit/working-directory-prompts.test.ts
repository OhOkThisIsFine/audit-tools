...
const { renderWorkerPrompt } = await import("../../src/audit/prompts/renderWorkerPrompt.js");
const { renderDispatchReviewPrompt, nextStepCommand, mergeAndIngestCommand } =
  await import("../../src/audit/cli/prompts.js");

function makeAgentTask(overrides: Partial<WorkerTask> = {}): WorkerTask {
  return {
...
type ReviewRunParams = Parameters<typeof renderDispatchReviewPrompt>[0];

function makeReviewRunParams(
  overrides: Partial<ReviewRunParams> = {},
): ReviewRunParams {
  return {
    root: "/repo",
    ...
    activeReviewRun: {
      run_id: "RUN-TEST",
      task_path: "/repo/.audit-tools/audit/current-task.json",
      prompt_path: "/repo/.audit-tools/audit/current-prompt.md",
      audit_results_path: "/repo/.audit-tools/audit/task-results/results.json",
      worker_command: ["node", "audit-code.mjs", "merge-and-ingest"],
    },
