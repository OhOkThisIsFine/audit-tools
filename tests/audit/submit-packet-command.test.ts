...
import { cmdSubmitPacket } from "../../src/audit/cli/submitPacketCommand.js";
import type { AuditResult, AuditTask, Finding } from "../../src/audit/types.js";
 ...
 */
function makeTask(taskId: string, unitId: string = "unit-1"): AuditTask {
  return {
    task_id: taskId,
    unit_id: unitId,
...
function makeTaskCovering(taskId: string, path: string, lens: string = "security", unitId: string = "unit-1"): AuditTask {
  return {
 ...
 */
function makeResult(task: AuditTask, findings: Finding[] = []): AuditResult {
  return {
...
interface OtherPacketResult {
  packetId: string;
  taskId: string;
  result: AuditResult | AuditResult[];
}

interface MakeArtifactsDirOptions {
  tasks: AuditTask[];
  packetId?: string;
  packetTaskIds?: string[];
  otherPacketResults?: OtherPacketResult[];
}
...
async function makeArtifactsDir({
  ...
  packetTaskIds,
  otherPacketResults = [],
}: MakeArtifactsDirOptions): Promise<{ artifactsDir: string; runDir: string }> {
 ...
 */
async function runSubmit(argv: string[]): Promise<{ stdout: string; stderr: string; error: any }> {
  let stdout = "";
  let stderr = "";
 ...
 */
function makeArgv(artifactsDir: string, packetId: string, payload: unknown): string[] {
...
function makeLensVerificationTask(taskId: string, path: string, unitId: string = "unit-1"): AuditTask {
  return {
