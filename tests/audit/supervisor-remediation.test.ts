...
const { buildAuditCodeHandoff, writeAuditCodeHandoffArtifacts } = await import("../../src/audit/supervisor/operatorHandoff.js");
const { loadRunLedger } = await import("../../src/audit/supervisor/runLedger.js");
const { getSessionConfigPath, loadSessionConfig, persistAnalyzerSettings } = await import("../../src/audit/supervisor/sessionConfig.js");
const { frictionCapturePath } = await import("../../src/shared/io/frictionCapture.js");
 ...
 */
function makeHandoffArtifactPaths(
  artifactsDir: string,
  incomingDir?: string,
): AuditCodeHandoffArtifactPaths {
...
test("writeAuditCodeHandoffArtifacts preserves original error as cause when write fails", async () => {
  await withTempDir("audit-code-handoff-cause-", async (artifactsDir) => {
    ...
    if (!isErrorWithCode(caughtError.cause)) throw caughtError.cause;
...
test("non-blocked statuses keep dispatch paths null and emit no blocked-mode outputs", () => {
  const artifactsDir = join("tmp", "audit artifacts");
  const statuses: AuditTopLevelStatus[] = [
    "not_started",
    "active",
    "complete",
  ];
    ...
    const handoff = buildAuditCodeHandoff({
      ...
      bundle: {
        audit_tasks: [{
          ...
          pass_id: "pass-1",
          lens: "correctness",
          rationale: "Exercise handoff rendering with an assigned audit task.",
          file_paths: [],
        }],
