...
import { normalizeCoverageSummary } from "../../src/audit/adapters/coverageSummary.js";
...
test("normalizeCoverageSummary keeps only below-threshold files and preserves severity boundaries", () => {
  ...
  expect(normalized.results.map((result: any) => ({
      id: result.id,
  ...
  const normalized = normalizeEslintJson([
    {
      ...
      messages: [
        ...
        // @ts-expect-error Deliberate contract-violation probe: test null severity handling
        { severity: null, message: "Null severity." },
        // @ts-expect-error Deliberate contract-violation probe: test string severity handling
...
test("normalizeCoverageSummary pins the lines_pct=50 boundary to medium and lines_pct=49.9 to high", () => {
  ...
  const highEntry = normalized.results.find((r: any) => r.path === "src/boundary-high.ts");
  const mediumEntry = normalized.results.find((r: any) => r.path === "src/boundary-medium.ts");
...
test("normalizeClippyJson maps clippy severities and validates through the generic seam", () => {
  ...
  expect(normalized.results.map((r: any) => ({ severity: r.severity, path: r.path, rule: r.rule }))).toEqual([
...
test("normalizeRubocopJson maps rubocop severities (fatal→high, convention→low) through the generic seam", () => {
  ...
  expect(normalized.results.map((r: any) => ({ severity: r.severity, path: r.path, rule: r.rule, line_start: r.line_start }))).toEqual([
