...
const { normalizeAstGrepJson } = await import(
  "../../src/audit/adapters/astGrep.js"
);
const { normalizeCodeqlSarif } = await import(
  "../../src/audit/adapters/codeql.js"
);
...
test("normalizeSemgrepDataflowJson maps source→sink trace to an edge", () => {
  ...
  if (!out.graph_edges) throw new Error("semgrep adapter omitted graph_edges");
...
test("normalizeSemgrepDataflowJson degrades to empty on malformed input", () => {
  ...
  // @ts-expect-error — deliberate malformed result exercises runtime degradation.
...
test("normalizeAstGrepJson maps file→captured target", () => {
  ...
  if (!out.graph_edges) throw new Error("ast-grep adapter omitted graph_edges");
...
test("normalizeAstGrepJson degrades to empty on malformed input", () => {
  ...
  // @ts-expect-error — deliberate malformed matches exercise runtime degradation.
...
test("normalizeCodeqlSarif maps first→last threadFlow location", () => {
  ...
  if (!out.graph_edges) throw new Error("CodeQL adapter omitted graph_edges");
