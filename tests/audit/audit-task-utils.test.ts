...
const { LENS_ORDER, priorityRank, sortLenses, computeRiskEstimate } =
  await import("../../src/audit/orchestrator/auditTaskUtils.js");
const { LENS_REGISTRY, ALL_LENSES, isLens } = await import("../../src/audit/types.js");
...
test("priorityRank returns 1 for unknown/default priority values", () => {
  // @ts-expect-error Deliberate runtime fallback probe for a non-contract priority.
...
const baseTask = (over: Partial<AuditTask> = {}): AuditTask => ({
  task_id: "t",
  unit_id: "u",
  pass_id: "p",
...
test("LENS_REGISTRY contains an entry for 'architecture'", () => {
  ...
  if (!entry) throw new Error("architecture lens is missing");
...
test("LENS_ORDER is sorted ascending by order_weight", () => {
  const weights = LENS_ORDER.map(
    (id) => {
      const definition = LENS_REGISTRY.find((d) => d.id === id);
      if (!definition) throw new Error(`lens definition is missing for ${id}`);
      return definition.order_weight;
    },
  );
