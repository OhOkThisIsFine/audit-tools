...
// Resolve the hook path relative to the repo root (two levels up from
// tests/audit/). The canonical hook is the tracked `.claude/hooks/session-start.sh`
...
const SKIP_REASON = BASH
  ? undefined
  : "no POSIX-compatible bash available (e.g. only the WSL launcher is on PATH)";
const SKIP = SKIP_REASON !== undefined;

/** Narrow `BASH` to `string` at test-body entry. Bodies only ever run when
 * `SKIP` is false (vitest's declarative `{ skip }` option never invokes a
 * skipped test's callback), so `BASH` is always non-null here — this just
 * gives that runtime invariant a static type instead of a cast. */
function requireBash(bash: string | null): string {
  if (!bash) {
    throw new Error("POSIX bash unexpectedly unavailable (test should have been skipped)");
  }
  return bash;
}

function spawnHook(env: Record<string, string | undefined>) {
  const bash = requireBash(BASH);
...
await test("session-start.sh runs npm install and the build command in order when CLAUDE_CODE_REMOTE=true", { skip: SKIP }, () => {
  const bash = requireBash(BASH);
