...
const { prepareDispatchArtifacts, resolveTierBudgets } = await import("../../src/audit/cli/dispatch.js");
...
const VALID_ROSTER: HostModelRosterEntry[] = [
...
test("resolveTierBudgets maps reported ranks directly", () => {
  const budgets = resolveTierBudgets(
    new Map<DispatchModelTier, number>([
      ["small", 1000],
      ["standard", 2000],
      ["deep", 3000],
    ]),
...
test("resolveTierBudgets fills missing ranks from the nearest reported rank", () => {
  ...
  expect(resolveTierBudgets(new Map<DispatchModelTier, number>([["standard", 2000], ["deep", 3000]]))).toEqual({ small: 2000, standard: 2000, deep: 3000 });
  ...
  expect(resolveTierBudgets(new Map<DispatchModelTier, number>([["small", 1000], ["deep", 3000]]))).toEqual({ small: 1000, standard: 1000, deep: 3000 });
  // single reported rank serves every tier
  expect(resolveTierBudgets(new Map<DispatchModelTier, number>([["deep", 3000]]))).toEqual({
    small: 3000,
    standard: 3000,
    deep: 3000,
  });
...
test("resolveTierBudgets throws when given an empty roster Map", () => {
  assert.throws(
    () => resolveTierBudgets(new Map<DispatchModelTier, number>()),
...
function linkedTasks(riskEstimate: number): AuditTask[] {
...
async function makeArtifactsDir(tasks: AuditTask[]) {
...
const SPLIT_ROSTER: HostModelRosterEntry[] = [
...
await test("per-rank model_id keys each roster pool's quota independently", async (t) => {
  ...
  const quota = await readJson(join(runDir, "dispatch-quota.json"));
  const poolIds = quota.capacity_pools.map((pool: any) => pool.pool_id);
