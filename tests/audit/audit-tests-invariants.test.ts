...
const DIST_IMPORT_ALLOWLIST = new Set([
  // audit-code-wrapper.test.ts imports dist/cli.js for the spawn wrapper test
  "audit-code-wrapper.test.ts",
  // cli-dispatcher.test.ts tests the CLI entry point dispatching
  "cli-dispatcher.test.ts",
  // validate-command.test.ts tests the validate CLI command end-to-end
  "validate-command.test.ts",
  // review-packets.test.ts uses CLI test helpers that need the compiled bin
  "review-packets.test.ts",
]);
...
function pythonLookup(...paths: string[]): Map<string, string> {
...
// schema-contracts.test.ts reads three files from examples/ inside async test
...
// schema-contracts.test.ts statically imports six src/*.ts modules at the top
...
test("FND-REL-0838abd9-2: schema-contracts statically-imported src modules exist on disk", () => {
  ...
  expect(missing, `These src modules are statically imported by schema-contracts.test.ts but are missing:\n  ${missing.join("\n  ")}`).toEqual([]);
});
