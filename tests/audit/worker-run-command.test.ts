 ...
 */
function buildTask(overrides: Partial<WorkerTask> = {}): WorkerTask {
  return {
...
test.concurrent("cmdWorkerRun: valid audit results reach runAuditStep (partition: errors fatal, warnings not)", async () => {
  ...
  try {
    ...
    const fakeRunAuditStep: WorkerRunDeps["runAuditStep"] = async () => {
    ...
    // Use real JSON helpers for the temp fixtures and inject the step seam below.
    const realReadJsonFile = (await import("audit-tools/shared")).readJsonFile;
    await writeFile(auditResultsPath, JSON.stringify([result]), "utf8");
