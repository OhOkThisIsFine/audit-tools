...
function readNumericConst(src: string, name: string): number {
  ...
  if (!m) throw new Error(`Could not find numeric constant ${name} in fileLock.ts`);
...
test("CLAUDE.md file-lock description matches the shared withFileLock constants", async () => {
  ...
  if (lockLine === undefined) throw new Error("Could not find file-lock description");
