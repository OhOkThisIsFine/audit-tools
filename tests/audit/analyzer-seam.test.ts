...
const { resolveAnalyzerPlan, needsInstallDecision } = await import("../../src/audit/extractors/analyzers/registry.js");
const { typescriptAnalyzer } = await import("../../src/audit/extractors/analyzers/typescript.js");

function floorGraph(imports: GraphEdge[] = []) {
  return { graphs: { imports, calls: [], references: [], routes: [] } };
}

function tsBundle(extra: Partial<ArtifactBundle> = {}): ArtifactBundle {
  return {
    repo_manifest: {
...
test("runSingleAnalyzer (via executor): returns ok:false with structured note when analyze() throws", async () => {
  const throwingAnalyzer: LanguageAnalyzer = {
    id: "throwing",
    supports: (file) => file.endsWith(".ts"),
...
test("buildEnrichedGraph (via executor): merges edges into correct buckets and deduplicates routes", async () => {
  const routeAnalyzer: LanguageAnalyzer = {
    id: "route-emit",
    supports: () => true,
    analyze: () => ({
      edges: [
...
test("runGraphEnrichmentExecutor loop: not_applicable analyzers record capability entry and skip analyze()", async () => {
  ...
  const neverSupportedAnalyzer: LanguageAnalyzer = {
    id: "never-supported",
    supports: () => false,
