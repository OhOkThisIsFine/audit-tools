...
import type {
  ...
  Scorecard,
  UnmatchedReason,
} from "../../src/audit/reporting/scoreAudit.js";
...
} = await import("../../src/audit/reporting/scoreAudit.js");
...
const FINDINGS_DOC = await readJson<{ findings: Finding[] }>("scoring-fixture.findings.json");
const LABELS = await readJson<CorpusLabels>("scoring-fixture.labels.json");
const FINDINGS = FINDINGS_DOC.findings;

function score(): Scorecard {
...
test("a reworded re-emission of a labeled finding still matches its label", () => {
  ...
  if (!auth) throw new Error("fixture missing SEC-001");
...
test("unmatched accounting is explicit — nothing is silently scored", () => {
  ...
  const byReason = (reason: UnmatchedReason) =>
