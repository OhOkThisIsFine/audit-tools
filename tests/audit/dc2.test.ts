...
import {
  ...
  type SessionConfig,
  type SharedProviderConfirmation,
...
await test("G3: a built confirmation carries NO roster snapshot (reach is never persisted)", () => {
  const built = buildSharedProviderConfirmation({}, CLEAN_ENV);
  expect((built as Partial<{ roster?: unknown }>).roster, "the writing auditor's reach must not be persisted").toBe(undefined);
});
...
await test("cross-tool honor: a confirmation written by audit is read + honored verbatim", async () => {
  await withTempRoot(async (root: string) => {
    const built = buildSharedProviderConfirmation({}, CLEAN_ENV);
...
await test("G3: confirmed keys span provider_pool + source_pool_cost_order + host_model_cost_order", () => {
  const keys = confirmedBackendKeys({
    ...
    provider_pool: [
      { name: "worker-command" },
      { name: "openai-compatible", model_id: "cfg-model" },
    ],
    source_pool_cost_order: [
      { source_id: "s1", transport: "codex", model_id: "src-model", blended_price_usd_per_mtok: null, price_declared: false, cost_order: 0 },
    ],
...
await test("G3: a CONFIRMED backend that is no longer reachable is silent (subset case)", () => {
  const built = buildSharedProviderConfirmation({}, CLEAN_ENV);
  const withGhost: SharedProviderConfirmation = {
    ...built,
    source_pool_cost_order: [
      { source_id: "ghost", transport: "codex", model_id: "vanished-model", blended_price_usd_per_mtok: null, price_declared: false, cost_order: 0 },
    ],
  };
...
await test("CE-003: a lockless read never observes a torn file under a concurrent writer", async () => {
  await withTempRoot(async (root: string) => {
    ...
    const confFor = (i: number): SharedProviderConfirmation => ({
      ...base,
      confirmed_at: new Date(Date.now() + i).toISOString(),
      provider_pool: [
        ...base.provider_pool,
        ...Array.from({ length: i }, (_, k) => ({
          name: "worker-command" as const,
          model_id: `pad-${k}-${"x".repeat(k * 8)}`,
        })),
      ],
    });
    ...
    const readers: Promise<SharedProviderConfirmation | null>[] = [];
    for (let i = 0; i < 60; i++) {
