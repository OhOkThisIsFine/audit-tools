/**
 * seam-dispatch-concurrency-model-agnostic-routing.test.ts
 *
...
import type { DispatchModelTier, CapacityPool } from "audit-tools/shared";
...
import {
  ...
  TIER_RANK,
  TIER_ORDER,
} from "../../src/audit/cli/dispatch/tierRouting.js";
import type { DispatchComplexity } from "../../src/audit/cli/dispatch/types.js";
 ...
 */
function openPool(id: string, rank?: DispatchModelTier): CapacityPool {
  return {
    id,
    accountKey: id,
...
function complexity(overrides: Partial<DispatchComplexity> = {}): DispatchComplexity {
  return {
    priority: "high",
    task_count: 1,
    file_count: 1,
...
function packet(id: string, complexityScore: number, estimatedTokens = 100) {
...
test("C1: all three tiers reported — each gets its own budget", () => {
  const input = new Map<DispatchModelTier, number>([
    ["small", 100],
    ["standard", 200],
    ["deep", 300],
  ]);
...
test("C2: only 'deep' reported — standard and small fall back to deep (nearest is deep, no lower available)", () => {
  const input = new Map<DispatchModelTier, number>([["deep", 300]]);
  const out = resolveTierBudgets(input);
  expect(out.deep).toBe(300);
...
test("C3: only 'small' reported — standard and deep get small (nearest lower wins — COR-eebbabf7)", () => {
  const input = new Map<DispatchModelTier, number>([["small", 100]]);
  const out = resolveTierBudgets(input);
  expect(out.small).toBe(100);
...
test("F1: createRollingDispatcher routes packets and returns all results", async () => {
  ...
  const resultIds = results.map((r: any) => r.packet.id).sort();
  expect(resultIds).toEqual(["a", "b", "c"]);
  expect(results.every((r: any) => r.outcome === "success")).toBeTruthy();
});
