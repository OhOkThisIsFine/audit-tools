...
function makeParams(
  overrides: Partial<DesignAssessmentParams> = {},
): DesignAssessmentParams {
  return {
    unitManifest: { units: [] },
...
test("detectUnitSprawl does not throw RangeError for large unit manifests (COR-3eaa834c)", () => {
  ...
  let result!: ReturnType<typeof buildDesignAssessment>;
  assert.doesNotThrow(() => {
...
test("hidden coupling: no co_change bucket (git-history not mined) → no findings, nothing renumbered", () => {
  ...
  if (cycle === undefined) throw new Error("expected a dependency cycle finding");
