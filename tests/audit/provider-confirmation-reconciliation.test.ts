...
import {
  ...
  selectCapabilityAnchors as pickAnchors,
...
import { renderProviderConfirmationPrompt } from "../../src/audit/cli/providerConfirmationStep.js";
...
async function withTempRoot<T>(
  fn: (dir: string, artifactsDir: string) => Promise<T>,
): Promise<T> {
...
async function writeInput(artifactsDir: string, body: Record<string, unknown>): Promise<void> {
  await writeFile(
...
/**
 * A fully-shaped `AuditNextStepCtx`-compatible fixture for the `def.execute`
 * calls below. `nextStepHelpers.ts`'s ctx type is internal (unexported) and
 * every field is required, but the `provider_confirmation` obligation's EMIT
 * branches only ever read `ctx.params.artifactsDir` — so the rest of the shape
 * is inert scaffolding, filled with harmless values rather than cast away.
 */
function buildThinNextStepCtx(artifactsDir: string) {
  return {
    params: {
      root: "",
      artifactsDir,
    ...
    analyzersRef: { value: undefined },
    lastSummaryRef: { value: "" },
    iterationRef: { value: 0 },
    dispatchedSignatures: new Set<string>(),
    seenStateSignatures: new Set<string>(),
    obligationTrail: [] as string[],
  };
}

/**
 * Thin wrapper over `renderProviderConfirmationPrompt` supplying the two fields
 * every call site here is indifferent to (`inputPath`/`continueCommand` — no
 * assertion in this file inspects either), so each test's `opts` literal only
 * has to state what it actually varies.
 */
function renderPrompt(
  opts: Omit<Parameters<typeof renderProviderConfirmationPrompt>[0], "inputPath" | "continueCommand">,
): string {
  return renderProviderConfirmationPrompt({
    inputPath: "/tmp/.audit-tools/audit/provider-confirmation.input.json",
...
describe("autonomous fail-closed exclusion", () => {
  ...
  const autonomousGate = (): ProviderConfirmationGateState => ({
    newlyReachable: [
      {
  ...
  test("fail-closed write emits service: axis pattern to close multi-transport residue durably", async () => {
    await withTempRoot(async (root, artifactsDir) => {
      const gate: ProviderConfirmationGateState = {
        newlyReachable: [
          {
...
describe("gate → obligation → executor selection", () => {
  ...
  const confirmedBundle = (): ArtifactBundle => ({
  ...
  test("attended + delta + no submission ⇒ EMITS the prompt (does not promote)", async () => {
    await withTempRoot(async (_root, artifactsDir) => {
      ...
      const outcome = await def.execute(confirmedBundle(), buildThinNextStepCtx(artifactsDir));
      expect(outcome.kind, "attended ⇒ ask the operator").toBe("emit");
      if (outcome.kind !== "emit") throw new Error("unreachable: asserted emit above");
...
describe("capability-evidence gate → obligation", () => {
  ...
  const providerObligation = (state: ReturnType<typeof deriveAuditState>) =>
  ...
  test("autonomous + reach-only delta (no capability delta) ⇒ FOLDS to the executor, never emits", async () => {
    await withTempRoot(async (_root, artifactsDir) => {
      ...
      const thinCtx = buildThinNextStepCtx(artifactsDir);
      await expect(
...
describe("BL-1 prompt: anchored insertion makes the delta-scoped ask answerable", () => {
  const POOL = [{ name: "worker-command" as const, capability_tier: "unknown" as const, excluded: false }];
