...
interface PostinstallRunResult {
  stdout: string;
  stderr: string;
}

function runPostinstall(homeDir: string): Promise<PostinstallRunResult> {
...
async function seedOpenCodeConfig(homeDir: string, config: Record<string, unknown>): Promise<string> {
