...
import { writeCoreArtifacts, loadArtifactBundle } from "../../src/audit/io/artifacts.js";
import { mergeAndIngest } from "../../src/audit/cli/mergeAndIngestCommand.js";
import { taskResultPath } from "../../src/audit/cli/args.js";
import { mergeOwnerTokens } from "../../src/audit/cli/ownerTokens.js";
import { ClaimRegistry, taskClaimsPath } from "audit-tools/shared";
...
async function scaffoldRun(
  artifactsDir: string,
  root: string,
  tasksAndResults: Array<[AuditTask, AuditResult | null]>,
) {
...
test("mergeAndIngest ownership gate: a task reclaimed by a peer since dispatch is excluded from ingest, its claim left alone, and surfaced", async () => {
  ...
  try {
    ...
    expect(unowned.map((u: { task_id: string }) => u.task_id)).toEqual([reclaimedTask.task_id]);
...
test("mergeAndIngest ownership gate: a fresh different-token claim is judged LIVE under the 20-min task lease even when older than the 30s default window", async () => {
  ...
  try {
    ...
    const unowned = JSON.parse(await readFile(join(runDir, "unowned-tasks.json"), "utf8"));
    expect(unowned.map((u: { task_id: string }) => u.task_id)).toEqual([victim.task_id]);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
import { recordAttemptedPackets } from "../../src/audit/cli/dispatchAttempted.js";
...
test("mergeAndIngest partial wave: an ATTEMPTED packet that produced no result is STILL a failure (deferral needs positive evidence)", async () => {
  ...
  try {
    ...
    expect(failed.map((f: { task_id: string }) => f.task_id)).toEqual([deadWorkerTask.task_id]);
