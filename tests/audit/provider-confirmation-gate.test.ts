...
import type { ConfirmedPoolEntry, SessionConfig } from "audit-tools/shared";

const { renderProviderConfirmationPrompt } = await import(
  "../../src/audit/cli/providerConfirmationStep.js"
);
const { runProviderConfirmationAutoComplete } = await import(
  "../../src/audit/orchestrator/intakeExecutors.js"
);
const { readConfirmedCostPositions, readSharedProviderConfirmation } = await import(
  "../../src/shared/providers/sharedProviderConfirmation.js"
);
const { deriveSourcePoolDisplay } = await import(
  "../../src/shared/providers/providerConfirmation.js"
);
...
describe("renderProviderConfirmationPrompt — dropped sources reach the operator at Gate-0", () => {
  const pool: ConfirmedPoolEntry[] = [{ name: "claude-code", capability_tier: "frontier", excluded: false, cost_order: 0 }];
  ...
  test("a reason containing a pipe cannot break the markdown table", () => {
    ...
    if (row === undefined) throw new Error("expected the 'weird' row to render");
...
describe("executor consumes seeded operator input (b/c — reorder + host roster)", () => {
  const NIM_CONFIG: SessionConfig = {
  ...
  test("promotes an operator input into the shared confirmation + dispatch map", async () => {
    ...
    try {
      ...
      if (!read) throw new Error("expected the shared provider confirmation to be readable");
