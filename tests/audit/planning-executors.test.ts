...
function makeFullBundle(): ArtifactBundle {
  return {
    repo_manifest: {
...
test("runPlanningExecutor includes delta scope summary in progress_summary", async () => {
  const scope: AuditScopeManifest = {
    mode: "delta",
    since: "abc123",
    seed_files: ["src/a.ts"],
 ...
 */
function makeBundleWithFiles(
  fileEntries: Array<{ path: string; status?: FileDispositionStatus; reason?: string }>,
): ArtifactBundle {
  ...
  return {
    repo_manifest: {
      repository: { name: "fixture" },
      generated_at: "2026-01-01T00:00:00.000Z",
      files: fileEntries.map(({ path }) => ({ path, language: "ts", size_bytes: 100 })),
    },
    file_disposition: { files },
    unit_manifest: {
      units: [
...
test("disposition_overrides exact-path match excludes the file from audit tasks", async () => {
  const fileEntries = [
    { path: "src/gen.ts", status: "included" as const },
    { path: "src/real.ts", status: "included" as const },
  ];
...
test("disposition_overrides prefix match excludes all files under the prefix", async () => {
  const fileEntries = [
    { path: "vendor/a.ts", status: "included" as const },
    { path: "vendor/b.ts", status: "included" as const },
    { path: "src/main.ts", status: "included" as const },
  ];
  const bundle = makeBundleWithFiles(fileEntries);
  bundle.intent_checkpoint = {
    schema_version: "intent-checkpoint/v1",
...
test("disposition_overrides absent: initializeCoverageFromPlan receives original file_disposition", async () => {
  const fileEntries = [{ path: "src/a.ts", status: "included" as const }];
  const bundle = makeBundleWithFiles(fileEntries);
...
test("free_form_intent='security audit' promotes security lens tasks in runPlanningExecutor", async () => {
  const fileEntries = [{ path: "src/auth.ts", status: "included" as const }];
  const bundle = makeBundleWithFiles(fileEntries);
  bundle.intent_checkpoint = {
    schema_version: "intent-checkpoint/v1",
    confirmed_at: "2026-01-01T00:00:00.000Z",
...
function makeTwoFileBundle(): ArtifactBundle {
  return {
    repo_manifest: {
      repository: { name: "fixture" },
      generated_at: "2026-01-01T00:00:00.000Z",
      files: [
        { path: "src/a.ts", language: "ts", size_bytes: 100 },
        { path: "src/b.ts", language: "ts", size_bytes: 120 },
      ],
    },
    file_disposition: {
      files: [
    ...
    unit_manifest: {
      units: [{ unit_id: "u1", name: "u1", files: ["src/a.ts", "src/b.ts"], required_lenses: ["security"] }],
    },
    surface_manifest: { surfaces: [] },
    critical_flows: { flows: [] },
    risk_register: { items: [] },
