...
} = await import("../../src/audit/cli/workerResult.js");

// ── buildWorkerResult ─────────────────────────────────────────────────────────

await test("buildWorkerResult maps all params to WorkerResult contract fields", () => {
  const params: Parameters<typeof buildWorkerResult>[0] = {
    runId: "run-abc",
...
await test("formatAuditResultValidationError includes error count and formatted issues", () => {
  const issues: Parameters<typeof formatAuditResultValidationError>[0] = [
    {
      result_index: 0,
      task_id: "T-1",
      field: "task_id",
