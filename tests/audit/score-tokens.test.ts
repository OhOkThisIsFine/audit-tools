...
import type { TokenUsageEntry, TokenScorecard } from "../../src/audit/reporting/scoreTokens.js";
...
const { scoreTokens, cacheHitRatioRegressed, renderTokenScorecardMarkdown, packetPromptPrefixHash } =
  await import("../../src/audit/reporting/scoreTokens.js");
const { cmdScoreTokens } = await import("../../src/audit/cli/scoreTokensCommand.js");
const { buildPacketPrompt } = await import("../../src/audit/cli/dispatch.js");

const here = dirname(fileURLToPath(import.meta.url));
const fixturesDir = join(here, "fixtures", "tokens");

async function readLedgerEntries(): Promise<TokenUsageEntry[]> {
...
test("cacheHitRatioRegressed: the sole gate is a cache-hit-ratio DECREASE vs baseline", async () => {
  ...
  const worse: TokenScorecard = { ...baseline, cache_hit_ratio_overall: 0.1 };
  expect(cacheHitRatioRegressed(worse, baseline)).toBe(true);

  // Higher current ratio → improvement, not a regression.
  const better: TokenScorecard = { ...baseline, cache_hit_ratio_overall: 0.9 };
  ...
  const nullCurrent: TokenScorecard = { ...baseline, cache_hit_ratio_overall: null };
  ...
  const nullBaseline: TokenScorecard = { ...baseline, cache_hit_ratio_overall: null };
  ...
  const totalsBallooned: TokenScorecard = {
    ...baseline,
...
function makeAuditTask(id: string): AuditTask {
  return {
    task_id: `task-${id}`,
    unit_id: `unit-${id}`,
...
function renderPromptFor(id: string): string {
...
const { cmdScoreAudit } = await import("../../src/audit/cli/scoreAuditCommand.js");

async function withScoreAuditFixture<T>(
  fn: (ctx: { dir: string; findingsPath: string; labelsPath: string }) => Promise<T>,
): Promise<T> {
...
test("score-audit: a non-.json --out must NOT be overwritten by the markdown summary (sibling path is derived safely)", async () => {
  await withScoreAuditFixture(async ({ dir, findingsPath, labelsPath }) => {
    ...
    let scorecard!: { schema_version: string };
