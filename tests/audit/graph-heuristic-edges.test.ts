...
/** `graphs.heuristics` is a catchall (open-ended) category on GraphBundle —
 * widen the type locally so tests can access it without an `unknown` cast at
 * every call site. */
type GraphsWithHeuristics = GraphBundle["graphs"] & { heuristics?: GraphEdge[] };
...
test("heuristic container edges land in heuristics graph, not imports", () => {
  ...
  const graphs = (bundle.graphs ?? {}) as GraphsWithHeuristics;
...
test("extractHeuristicContainerEdges produces a container edge for a backslash-separated path", () => {
  ...
  const bundle = buildGraphBundle(m, undefined, { fileContents: {} });
  const heuristics = (bundle.graphs as GraphsWithHeuristics)?.heuristics ?? [];
