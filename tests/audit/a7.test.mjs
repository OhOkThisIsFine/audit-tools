...
test(
  ...
  async () => {
    // Deploy the Codex host surface into an isolated temp $HOME, then drive a real
    // headless Codex dispatch through `audit-code next-step` and assert a bounded
    // audit step round-trips (a result artifact lands). The live wiring reuses the
    ...
    try {
      ...
      const outcome = await runCodexHeadlessAuditDispatch({ root: report.repo_root });
      assert.ok(
