...
// Loose test-double signatures for options.spawn / options.createWriteStream on
// spawnLoggedCommand — node:child_process.spawn / node:fs.createWriteStream are
// heavily overloaded; test doubles only need to satisfy the narrow surface
// spawnLoggedCommand actually calls (mirrors tests/shared/spawnLoggedCommand.test.ts).
type SpawnDouble = (
  command: string,
  args?: readonly string[] | SpawnOptions,
  options?: SpawnOptions,
) => any;
type CreateWriteStreamDouble = (...args: any[]) => any;

// Loose test-double for a reassigned process.stderr.write — the real property
// type is heavily overloaded (chunk[, encoding][, callback]); the diagnostics
// tests below only ever exercise the single-chunk form.
type StderrWriteDouble = (...args: any[]) => boolean;

/** Shape recorded by the `launches.push({ command, args, passedInput })` launcher stubs below. */
interface RecordedLaunch {
  command: string;
  args: string[];
  passedInput: LaunchFreshSessionInput;
}
...
test("spawnLoggedCommand waits for close before ending logs", async () => {
  await withTempDir("audit-code-spawn-close-", async (root) => {
    ...
    const stdoutLog: any = new PassThrough();
    const stderrLog: any = new PassThrough();
    const writes: string[] = [];
    stdoutLog.write = (chunk: any, callback: (error?: Error | null) => void) => {
    ...
    const promise = spawnLoggedCommand("fake", [], input, undefined, {
      ...
      createWriteStream: ((path: string) =>
        path === input.stdoutPath ? stdoutLog : stderrLog) as CreateWriteStreamDouble,
    });
    ...
    await assert.rejects(
      () =>
        spawnLoggedCommand("fake", [], input, undefined, {
          spawn: (() => child) as SpawnDouble,
          createWriteStream: (() => new PassThrough()) as CreateWriteStreamDouble,
          killGraceMs: 10,
        }),
  ...
  await withEnv("CLAUDECODE", undefined, async () => {
    await withTempDir("audit-code-claude-provider-stderr-", async (root) => {
      ...
      process.stderr.write = ((chunk: any, ...rest: any[]) => {
        ...
        return (originalWrite as StderrWriteDouble)(chunk, ...rest);
      }) as StderrWriteDouble;
  ...
  await withTempDir("audit-code-spawn-log-error-", async (root) => {
    ...
    await assert.rejects(
      () =>
        spawnLoggedCommand("fake", [], input, undefined, {
          spawn: (() => child) as SpawnDouble,
          createWriteStream: (() => streams.shift() ?? new PassThrough()) as CreateWriteStreamDouble,
          killGraceMs: 10,
        }),
      /disk full/i,
    );
