...
import type { DispatchResultMapEntry } from "../../src/audit/types/activeDispatch.js";
import type { FailingTask } from "../../src/audit/cli/mergeAndIngestCommand.js";

const { cmdMergeAndIngest, validateAndCollectResults } = await import("../../src/audit/cli/mergeAndIngestCommand.js");
...
async function scaffoldRun(
  artifactsDir: string,
  repoRoot: string,
  { entries, pendingTasks }: { entries: DispatchResultMapEntry[]; pendingTasks: AuditTask[] },
): Promise<{
  runDir: string;
...
test("cmdMergeAndIngest blocks (throws + writes failed-tasks.json) when every assigned result is missing", async () => {
  await withTempDir("merge-ingest-cmd-", async (artifactsDir) => {
    ...
    const { failedTasksPath } = await scaffoldRun(artifactsDir, repoRoot, {
      ...
      pendingTasks: [
        { task_id: "u1:security", unit_id: "u1", pass_id: "p", lens: "security", file_paths: ["src/a.ts"], rationale: "test" },
        { task_id: "u2:correctness", unit_id: "u2", pass_id: "p", lens: "correctness", file_paths: ["src/b.ts"], rationale: "test" },
      ],
    });
    ...
    const failed: FailingTask[] = JSON.parse(await readFile(failedTasksPath, "utf8"));
...
test("cmdMergeAndIngest discards a STALE completion marker when a pending task has an un-ingested on-disk result (staleness self-heal), and re-processes", async () => {
  await withTempDir("merge-ingest-cmd-", async (artifactsDir) => {
    ...
    const { taskResultsDir, mergeCompletePath, failedTasksPath } = await scaffoldRun(artifactsDir, repoRoot, {
      ...
      pendingTasks: [
        { task_id: "u9:security", unit_id: "u9", pass_id: "p", lens: "security", file_paths: ["src/x.ts"], rationale: "test" },
      ],
    });
...
test("validateAndCollectResults rejects a duplicate task_id at ingest (dedup via seenTaskIds) instead of double-ingesting", async () => {
  await withTempDir("merge-ingest-cmd-", async (dir) => {
    const t1: AuditTask = { task_id: "t1", unit_id: "u", pass_id: "p", lens: "security", file_paths: ["src/a.ts"], rationale: "test" };
    const t2: AuditTask = { task_id: "t2", unit_id: "u2", pass_id: "p", lens: "security", file_paths: ["src/b.ts"], rationale: "test" };
    const mk = (tid: string, unit: string, path: string) => ({
      task_id: tid,
      unit_id: unit,
      pass_id: "p",
      lens: "security",
