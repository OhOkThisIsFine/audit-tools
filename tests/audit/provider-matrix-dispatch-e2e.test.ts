...
function availability(candidate: Candidate): { available: boolean; reason: string } {
  ...
  return {
    ...
    // DiscoveredProvider carries no `reason` field — this always fell back to
    // the same literal at runtime even pre-conversion; kept byte-identical.
