...
const { updateRuntimeValidationReport } = await import("../../src/audit/orchestrator/runtimeValidationUpdate.js");
 ...
 */
function captureWarn(fn: () => void): unknown[][] {
 ...
 */
function makeManifest(...ids: string[]): RuntimeValidationTaskManifest {
  return {
    tasks: ids.map((id) => ({
      id,
...
function makeReport(...results: RuntimeValidationResult[]): RuntimeValidationReport {
  return { results };
}
...
test("merges update into existing result, deduplicating evidence and notes, update wins for status/summary", () => {
  ...
  if (!result) throw new Error("Expected merged result for task-a");
...
test("normalizes duplicate evidence/notes in an existing result that has no matching update", () => {
  ...
  if (!result) throw new Error("Expected result for task-a");
...
test("warns when existing results contain stale task IDs", () => {
  ...
  let report: RuntimeValidationReport | undefined;
  const warnMessages = captureWarn(() => {
    report = updateRuntimeValidationReport(manifest, existing, updates);
  });
  if (!report) throw new Error("Expected runtime validation report");
  const ids = report.results.map((r) => r.task_id);
