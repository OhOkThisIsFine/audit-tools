...
function priorFinding(id: string, title: string, severity: Finding["severity"]): Finding {
  return {
    id,
    title,
...
test("renderDesignReReviewSection: carries prior verdict + diff, instructs diff-scoped re-review", () => {
  const base = bundle();
  const priorFindings = [priorFinding("DR-001", "old finding", "high")];
...
test("state.ts: completed pass with a fresh snapshot is satisfied", () => {
  ...
  if (!obl) throw new Error("expected design_review_contract_completed obligation");
...
test("capture → load roundtrip + buildDesignReReviewSection emits the re-review on a real change", async () => {
  ...
  try {
    ...
    if (!loaded.contract) throw new Error("expected the contract snapshot to load");
