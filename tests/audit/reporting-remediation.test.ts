...
test("renderAuditReportMarkdown renders the standardized per-finding block", () => {
  const report: RenderableAuditReport = {
    summary: {
      finding_count: 2,
      work_block_count: 0,
...
// Thin typed + runtime-validated wrapper over the untyped fixture (mirrors
// tests/audit/reporting-invariants.test.ts) — zod.parse both proves the fixture
// output actually satisfies the live contract and gives callers a real `Finding`
// / `AuditResult` type instead of the fixture's widened inferred shape.
