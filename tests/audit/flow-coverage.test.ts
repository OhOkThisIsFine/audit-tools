...
import type { CoverageMatrix } from "../../src/audit/types.js";

const { buildFlowCoverage } = await import("../../src/audit/orchestrator/flowCoverage.js");
...
function makeFlow({
  ...
  paths = [],
  concerns = [],
}: { id?: string; paths?: string[]; concerns?: string[] } = {}): CriticalFlowManifest {
  return {
    flows: [
      {
        id,
 ...
 */
function makeMatrix(
  files: Array<{ path: string; audit_status?: string; completed_lenses?: string[] }>,
): CoverageMatrix {
  return {
