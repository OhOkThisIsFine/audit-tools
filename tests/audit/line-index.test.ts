...
function makeManifest(paths: string[]): RepoManifest {
  return {
    repository: { name: "line-index-fixture" },
...
test("buildLineIndex returns a line-count record keyed by file path", async (t: TestContext) => {
  ...
  try {
    ...
    const manifest = makeManifest(["a.ts", "b.ts"]);
...
test("addFileLineCountHints annotates each task with per-file line counts", async (t: TestContext) => {
  ...
  try {
    ...
    const tasks: AuditTask[] = [
      { task_id: "t1", unit_id: "u1", pass_id: "p1", lens: "correctness", file_paths: ["src/a.ts", "src/b.ts"], rationale: "Fixture task" },
    ];
    ...
    if (!annotated?.file_line_counts) {
      throw new Error("addFileLineCountHints must annotate every returned task");
    }
...
test("addFileLineCountHints preserves original task fields (no mutation of input)", async (t: TestContext) => {
  ...
  try {
    ...
    const annotated = result[0];
    if (!annotated?.file_line_counts || !("custom_field" in annotated)) {
      throw new Error("addFileLineCountHints must preserve and annotate the task");
    }
...
test("buildLineIndexForPaths emits 'file not found' diagnostic for ENOENT, returns 0 (COR-c868f53d)", async (t: TestContext) => {
  ...
  process.stderr.write = (chunk: string | Uint8Array) => {
    ...
    return origWrite.call(process.stderr, chunk);
  };
  try {
...
test("addFileLineCountHints keys in file_line_counts match task file_paths array", async (t: TestContext) => {
  ...
  try {
    ...
    const tasks: AuditTask[] = [
      { task_id: "t1", unit_id: "u1", pass_id: "p1", lens: "maintainability", file_paths: ["p1.ts", "p2.ts"], rationale: "Fixture task" },
      { task_id: "t2", unit_id: "u2", pass_id: "p1", lens: "maintainability", file_paths: ["p2.ts"], rationale: "Fixture task" },
    ];
