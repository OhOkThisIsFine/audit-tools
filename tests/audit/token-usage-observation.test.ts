...
import type { QuotaWindowPctReading } from "../../src/shared/quota/state.js";
import type { WindowSlopeKey } from "../../src/shared/quota/quotaSource.js";
import { windowSlopeKey } from "../../src/shared/quota/quotaSource.js";

const { AuditResultSchema } = await import("../../src/audit/types.js");
...
} = await import("../../src/audit/cli/dispatch/tokenUsageObservation.js");
const {
  setQuotaStateDir,
  readQuotaState,
  foldSlopeObservationFromPctMaps,
} = await import("../../src/shared/quota/state.js");
...
function poolSummary(overrides: Partial<DispatchCapacityPoolSummary> = {}): DispatchCapacityPoolSummary {
  return {
...
async function writeDispatchQuota(runDir: string, capacityPools: DispatchCapacityPoolSummary[]) {
  await writeFile(
...
test("foldSlopeObservationFromPctMaps: a swallowed recordTokensPerPctObservation failure is NOT reported as folded (C2)", async () => {
  ...
  try {
    ...
    const prior = new Map<WindowSlopeKey, QuotaWindowPctReading>([[windowSlopeKey("account", "default"), { remainingPct: 0.5, resetAt: null }]]);
    const current = new Map<WindowSlopeKey, QuotaWindowPctReading>([[windowSlopeKey("account", "default"), { remainingPct: 0.4, resetAt: null }]]);
