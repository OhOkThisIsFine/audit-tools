...
const { deriveDataStateCoupling } = await import(
  "../../src/audit/extractors/dataStateCoupling.js"
);
...
const { detectNonColocalization } = await import(
  "../../src/audit/decompose/findings.js"
);
const { buildStructureDecomposition } = await import(
  "../../src/audit/decompose/buildStructureDecomposition.js"
);
...
function readerFrom(contents: Readonly<Record<string, string>>) {
  return async (abs: string): Promise<string | undefined> => {
...
describe("buildStructureDecomposition + executor — end to end", () => {
  ...
  it("produces a scaffold + findings artifact (no root → intent from dir only)", async () => {
    ...
    if (!payNode) throw new Error("Expected payment decomposition node");
  ...
  it("executor returns the uniform run result and updates the bundle", async () => {
    ...
    if (!result.updated.structure_decomposition) {
      throw new Error("Expected structure decomposition artifact");
    }
