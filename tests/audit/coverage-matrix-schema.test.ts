...
import { CoverageMatrixSchema, ClassificationStatusSchema } from "../../src/audit/types.js";
...
function coverageMatrixWith(classificationStatus: string) {
  return {
    files: [
      {
        path: "src/example.ts",
...
describe("coverage_matrix schema accepts all classification_status values written by code", () => {
  it("classification_status enum includes the statuses written by scope and trivial-audit code", () => {
    ...
    for (const value of ["out_of_scope_delta", "excluded_trivial", "out_of_scope_intent"] as const) {
