...
const { normalizeCheckpointForms, computeGateVersion, normalFormHash } =
  await import("../../src/audit/orchestrator/intentCheckpointGate.js");
...
function checkpoint(over: Partial<IntentCheckpoint> = {}): IntentCheckpoint {
  return {
...
function baselineFor(cp: IntentCheckpoint, revision = 3): IntentBaseline {
...
test("status: stale gate version / structured delta / prose delta discriminate correctly", () => {
  ...
  if (status.kind !== "prose_judgment_pending") {
    throw new Error("expected a pending prose judgment");
  }
...
test("stamp arm: baseline stamped from current, revision mirrors the entry (min 1)", async () => {
  ...
  if (!run.updated.artifact_metadata?.intent_baseline) {
    throw new Error("intent baseline was not written");
  }
  ...
  if (!fresh.updated.artifact_metadata?.intent_baseline) {
    throw new Error("intent baseline was not written");
  }
...
test("a verdict naming a stale pair is discarded; the obligation stays pending", () => {
  ...
  if (!run.updated.artifact_metadata?.intent_baseline) {
    throw new Error("intent baseline was not preserved");
  }
...
test("metadata stamper mirrors the baseline revision for the intent entry", () => {
  ...
  const previous: ArtifactMetadataManifest = {
    metadata_schema_version: METADATA_SCHEMA_VERSION,
    artifacts: {
      "intent_checkpoint.json": {
        revision: 3,
  ...
  if (!after.intent_baseline) throw new Error("intent baseline was not written");
...
test("the revision mirror never REWINDS below the previous entry revision (gate-flap hardening)", () => {
  ...
  const bundle: ArtifactBundle = { intent_checkpoint: cp, artifact_metadata: previous };
