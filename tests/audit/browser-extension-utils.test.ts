...
import type { FileDisposition } from "../../src/shared/types/disposition.js";

const {
  isBrowserExtensionManifestPath,
  hasBrowserExtensionManifestFile: hasBrowserExtensionManifestFileImpl,
  deriveBrowserExtensionLensesForPath,
  inferBrowserExtensionUnitKind,
  buildBrowserExtensionSurfacesFromGraph,
} = await import("../../src/audit/extractors/browserExtension.js");

function hasBrowserExtensionManifestFile(
  fixture: { files: Array<{ path: string; size_bytes: number }> },
): boolean {
  const repoManifest: RepoManifest = {
    repository: { name: "browser-extension-fixture" },
    generated_at: "2026-01-01T00:00:00.000Z",
    files: fixture.files.map((file) => ({ ...file, language: "unknown" })),
  };
  return hasBrowserExtensionManifestFileImpl(repoManifest);
}
...
test("file with excluded disposition status is skipped", () => {
  ...
  const disposition: FileDisposition = { files: [{ path: "vendor/bg.js", status: "vendor" }] };
