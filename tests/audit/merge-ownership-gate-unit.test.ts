...
import type { ClaimResult } from "../../src/shared/quota/claimRegistry.js";

const { partitionByOwnership, partitionUnattemptedMissing } = await import(
  "../../src/audit/cli/mergeAndIngestCommand.js"
);
...
const MISSING = (task_id: string): FailingTask => ({
  task_id,
  errors: ["Missing audit result for assigned task."],
  kind: "missing",
});
const UNPARSEABLE = (task_id: string): FailingTask => ({
  task_id,
  errors: ["Invalid JSON: boom"],
  kind: "unparseable",
});
const CONTRACT_MISMATCH = (task_id: string): FailingTask => ({
  task_id,
...
test("recordAttemptedPackets: a later round's grant UNIONS with earlier rounds, so an earlier attempt is never erased", async () => {
  const { recordAttemptedPackets, readAttemptedPackets } = await import(
    "../../src/audit/cli/dispatchAttempted.js"
  );
  ...
  try {
    ...
    if (attempted === null) {
      throw new Error("Expected cumulative attempted-packets sidecar");
    }
    expect(
      [...attempted].sort(),
...
function acquiredOwnerToken(result: ClaimResult): string {
  if (!result.acquired) {
    throw new Error(`Expected claim acquisition; held by ${result.heldBy}`);
  }
...
test("partitionByOwnership: a STALE different-token claim (crashed peer's ghost, heartbeat older than the lease) fails OPEN, not unowned", async () => {
  ...
  try {
    ...
    const aOwnerToken = acquiredOwnerToken(a);
    ...
    const { owned, unowned } = partitionByOwnership(
      [{ task_id: "t1" }],
      { t1: aOwnerToken }, // A's original persisted token
