...
import { mineGitHistory, isGitRepo } from "../../src/shared/git.js";
import type { GitHistory, GraphEdge } from "audit-tools/shared";
import {
  mineGitHistoryArtifact,
  gitHistoryGraphEdges,
  gitHistoryRiskSignals,
  GIT_CO_CHANGE_CATEGORY,
} from "../../src/audit/extractors/gitHistory.js";
import { mergeAnalyzerGraphContribution } from "../../src/audit/extractors/graph.js";
import {
  mergeAnalyzerRiskSignals,
  deriveRiskConcentration,
} from "../../src/audit/extractors/risk.js";
import { allGraphEdges, deriveGraphSignals } from "../../src/audit/extractors/graphSignals.js";
import { runStructureExecutor } from "../../src/audit/orchestrator/structureExecutors.js";
import { buildRepoManifestFromFs } from "../../src/audit/extractors/fsIntake.js";
import { ARTIFACT_DEPENDS_ON_MAP } from "../../src/audit/orchestrator/dependencyMap.js";
import type { RepoManifest, UnitManifest } from "../../src/audit/types.js";
...
const manifest = (paths: string[]): RepoManifest => ({
...
test("gitHistoryRiskSignals flags hotspots and broad authorship by unit", () => {
  const units: UnitManifest = {
    units: [
      { unit_id: "u1", name: "u1", files: ["a.ts"], required_lenses: [] },
      { unit_id: "u2", name: "u2", files: ["b.ts"], required_lenses: [] },
    ],
  };
...
test("F6 inv-9: an end-to-end co-commit append composes producers with the seam (the only land-order-safe path)", () => {
  ...
  const units: UnitManifest = { units: [{ unit_id: "u1", name: "u1", files: ["a.ts"], required_lenses: [] }] };
...
test("runStructureExecutor wires git-history mining: git_history + co_change bucket + provenance + risk signals", async () => {
  ...
  try {
    ...
    expect((git_history!.churn.find((c) => c.path === "a.ts")?.commits ?? 0) >= 8, "a.ts mined as a churn hotspot").toBeTruthy();

    // 2) Co-change landed in the co_change bucket (NOT references) with provenance.
    const graphsRecord = graph_bundle!.graphs as Record<string, unknown>;
    const coChangeRaw = graphsRecord[GIT_CO_CHANGE_CATEGORY];
    const coChange: GraphEdge[] = Array.isArray(coChangeRaw) ? coChangeRaw : [];
