...
describe("line-count mismatch policy is single-sourced", () => {
  it("predicate matches only file_coverage total_lines warnings", () => {
    expect(isCoverageTotalLinesMismatch({ severity: "warning", field: "file_coverage[0].total_lines", message: "m" })).toBe(true);
    expect(isCoverageTotalLinesMismatch({ severity: "error", field: "file_coverage[0].total_lines", message: "m" })).toBe(false);
    expect(isCoverageTotalLinesMismatch({ severity: "warning", field: "findings[0].title", message: "m" })).toBe(false);
  });
