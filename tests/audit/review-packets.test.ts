...
} = await import("../../src/audit/orchestrator/reviewPackets.js");
...
interface OwnershipRootReferenceEdge {
  from: string;
  to: string;
  kind: string;
  confidence: number;
...
interface OwnershipRootKeyEdge {
  from: string;
  to: string;
  kind: string;
  confidence: number;
...
interface OwnershipRootCase {
  name: string;
  taskPrefix: string;
  apiPath: string;
  storePath: string;
  referenceEdges: OwnershipRootReferenceEdge[];
  expectedTaskIds: string[];
  expectedKeyEdge: OwnershipRootKeyEdge;
...
const ownershipRootCases: OwnershipRootCase[] = [
  {
...
test.skip("prepare-dispatch writes one packet prompt for multiple task outputs", async () => {
  await withTempDir(async (artifactsDir) => {
    ...
    expect(resultMap.entries.map((entry: any) => entry.task_id).sort()).toEqual(["src-auth:correctness", "src-auth:security"]);
    expect(new Set(resultMap.entries.map((entry: any) => entry.result_path)).size).toBe(2);
...
test.skip("large single-file packets stay isolated and get mechanical anchors", async () => {
  ...
  await withTempDir(async (artifactsDir) => {
    ...
    expect(anchors.anchors.some(
        (anchor: any) => anchor.kind === "symbol" && anchor.name === "authenticate",
      )).toBeTruthy();
    expect(anchors.anchors.some(
        (anchor: any) => anchor.kind === "keyword" && anchor.name.toLowerCase() === "password",
      )).toBeTruthy();
    const prompt = await readFile(plan[0].prompt_path, "utf8");
