...
import type { UnitManifest } from "../../src/audit/types.js";

const { buildAuditTasks } = await import("../../src/audit/orchestrator.js");
...
test("buildAuditTasks rejects malformed unit manifests with path-aware errors", () => {
        ...
        units: [
          {
            ...
            required_lenses: [
              // @ts-expect-error -- deliberate wrong-typed lens exercises the runtime guard.
              42,
            ],
          },
        ],
  ...
  assert.throws(
    // @ts-expect-error -- deliberate incomplete unit exercises required-field validation.
  ...
  assert.throws(
    () =>
      buildAuditTasks(createUnitManifest(), {
        limit_lenses: [
          // @ts-expect-error -- deliberate wrong-typed option exercises the runtime guard.
          123,
        ],
      }),
