...
import type { DispatchRoutingTiers } from "audit-tools/shared";
import type { AuditTask } from "../../src/audit/types.js";
import type { DispatchComplexity } from "../../src/audit/cli/dispatch.js";
...
const { buildReviewPacketsFromPartition } = await import("../../src/audit/orchestrator/reviewPackets.js");
...
const tier = (
  ...
  complexityOverrides: Partial<DispatchComplexity> = {},
  routingTiers?: DispatchRoutingTiers,
) =>
  resolveDispatchTier({
    routingRisk,
...
test("partition packets carry routing_risk (max member risk)", () => {
  ...
  if (!merged || !lone) throw new Error("expected merged and lone packets");
