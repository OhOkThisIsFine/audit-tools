...
import {
  ...
  resolveAuditScope,
  fullAuditScope,
} from "../../src/audit/orchestrator/scope.js";
...
} from "../../src/audit/extractors/disposition.js";
import type { GitSpawn } from "../../src/audit/extractors/disposition.js";
import type { CoverageMatrix, RepoManifest } from "../../src/audit/types.js";
...
function bundleWithEdges(edges: Array<{ from: string; to: string; confidence: number }>) {
  return {
    graphs: {
      imports: edges.map((e) => ({
        from: e.from,
        to: e.to,
        kind: "ts-import" as const,
...
function coverage(
  paths: string[],
  { status = "pending", lenses = ["correctness"] }: { status?: string; lenses?: string[] } = {},
): CoverageMatrix {
  return {
...
test("resolveAuditScope: real git repo — changed file + graph neighbour, mistyped ref → full", async (t: { skip: (msg?: string) => void }) => {
  ...
  try {
    ...
    const bundle: ArtifactBundle = {
      repo_manifest: manifest(["a.ts", "b.ts", "c.ts"]),
      file_disposition: {
        files: [
          { path: "a.ts", status: "included" },
          { path: "b.ts", status: "included" },
          { path: "c.ts", status: "included" },
        ],
      },
      graph_bundle: {
        graphs: {
...
async function makeGitRoot(t: { skip: (msg?: string) => void }, gitignoreContent?: string) {
  let root: string | undefined;
  try {
...
test("disposition: batched check-ignore classifies ignored files as vcs_ignored (single spawn, slash-normalized)", async (t: { skip: (msg?: string) => void }) => {
...
test("disposition: check-ignore exit 1 (nothing ignored) is success; targeted exclusions unchanged", async (t: { skip: (msg?: string) => void }) => {
...
test("disposition: not a git work tree (exit 128) falls back cleanly and records why", async (t: { skip: (msg?: string) => void }) => {
  let root: string | undefined;
  try {
...
test("disposition: every ignored file keeps a per-file excluded record", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t, "alpha/\n");
  if (!root) return;
  try {
...
test("disposition: large ignored sets keep per-file records — no record is ever dropped (exclusions stay downstream-effective)", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t, "alpha/\nbeta/\n");
  if (!root) return;
  try {
...
test("disposition: root-ignored guard skips the rule and records root_ignored", async (t: { skip: (msg?: string) => void }) => {
...
test("disposition: share guard skips the rule and records share_exceeded", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t, "alpha/\n");
  if (!root) return;
  try {
...
test("disposition: untracked scratch is excluded; tracked + gitignored files keep their classifications (single batched ls-files, slash-normalized)", async (t: { skip: (msg?: string) => void }) => {
...
test("disposition: repository with no tracked files skips the untracked rule (root_untracked)", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t);
  if (!root) return;
  try {
    const disposition = await buildFileDisposition(
...
test("disposition: untracked share guard skips the rule and records share_exceeded", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t);
  if (!root) return;
  try {
...
test("disposition: large untracked sets keep per-file records — no record is ever dropped", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t);
  if (!root) return;
  try {
...
test("disposition: tracked files whose on-disk case drifted from the index are NOT excluded as untracked", async (t: { skip: (msg?: string) => void }) => {
  const root = await makeGitRoot(t);
  if (!root) return;
  try {
