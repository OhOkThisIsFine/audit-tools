...
test(
  ...
  // vitest's `skip` is boolean-only (no node:test-style string reason) — the
  // requirement is documented in the module comment above: set RUN_NIM_E2E=1
  // and a NIM key (LLM_BACKEND_API_KEY/NVIDIA_API_KEY) to run.
  { skip: !RUN },
  async () => {
    await withTempDir("hybrid-nim-audit-e2e-", async (root) => {
      ...
      // Widened to the one field this test reads (`kind`) — the assertions below
      // deliberately compare against "transition", which is an internal engine-fold
      // signal never actually returned by runDeterministicForNextStep; a literal
      // NextStepResult['kind'] union would reject that comparison outright.
      ...
      if (result?.kind === "semantic_review") {
        ...
        const hostTaskIds = existsSync(hostTasksPath)
          ? JSON.parse(readFileSync(hostTasksPath, "utf8")).map((t: { task_id: string }) => t.task_id)
          : [];
