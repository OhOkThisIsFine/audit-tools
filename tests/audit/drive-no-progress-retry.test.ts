...
const { driveWithNoProgressRetry } = await import("../../src/audit/cli/nextStepHelpers.js");

interface DriveResult {
  status: "complete" | "paused";
  ingest: { ingested: number } | null;
  stranded_ids: string[];
  packet_count: number;
}
...
const noProgress: DriveResult = { status: "complete", ingest: null, stranded_ids: [], packet_count: 3 };
const progress: DriveResult = { status: "complete", ingest: { ingested: 3 }, stranded_ids: [], packet_count: 3 };
const isNoProgress = (d: DriveResult): boolean => d.status !== "paused" && !d.ingest && d.stranded_ids.length === 0;
...
test("D1: a paused pass is NOT treated as no-progress (resumable, handled separately)", async () => {
  const { sleep, waits } = fakeSleep();
  let calls = 0;
  const paused: DriveResult = { status: "paused", ingest: null, stranded_ids: [], packet_count: 3 };
  const result = await driveWithNoProgressRetry(
