...
function runBridge(taskPath: string): Promise<BridgeResult> {
...
test("provider-assisted bridge writes synthetic results for valid agent tasks", async () => {
  await withTempDir(async (dir) => {
    ...
    expect(written[0].notes.some((note: string) => /priority: high/i.test(note))).toBeTruthy();
  });
});

interface ValidTaskFixture {
  repoRoot: string;
  tasksPath: string;
  resultPath: string;
...
async function buildValidTaskFixture(dir: string): Promise<ValidTaskFixture> {
  const repoRoot = join(dir, "repo");
  await mkdir(repoRoot, { recursive: true });
  await writeFile(join(repoRoot, "src.ts"), "export const x = 1;\n", "utf8");
