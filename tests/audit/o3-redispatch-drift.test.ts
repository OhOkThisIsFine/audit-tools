...
const {
  ...
  emitSourceFor,
} = await import("../../src/audit/orchestrator/ledger.js");
const {
...
/** A minimal but contract-complete Finding, distinguished only by its id. */
function finding(id: string): Finding {
  return {
    ...
    affected_files: [{ path: "a.ts" }],
  };
}

function baseResult(findings: Finding[], over: Partial<AuditResult> = {}): AuditResult {
  return {
    task_id: "u1:security",
    unit_id: "u1",
    pass_id: "p1",
...
const tasksByTaskId = (t: AuditTask): Map<string, AuditTask> => new Map([[t.task_id, t]]);

test("first ingest establishes baseline; unchanged content never drifts", () => {
  const t = task();
  const tasks = tasksByTaskId(t);
  const r = baseResult([finding("F1")]);
...
test("content drift → re-dispatch appends fresh findings, supersedes, converges", () => {
  ...
  const r1 = baseResult([finding("F_old")]);
  ...
  const r2 = baseResult([finding("F_new")]);
...
test("supersession keys on task_id, never identity_key — distinct tasks never collapse", () => {
  ...
  const a = baseResult([finding("A")], { task_id: "u1:security:part-1" });
  const b = baseResult([finding("B")], { task_id: "u2:security:part-2", unit_id: "u2" });
  ...
  const base = baseResult([finding("OLD")], { task_id: "u1:security" });
  const redis = baseResult([finding("NEW")], {
    task_id: "u1:security",
