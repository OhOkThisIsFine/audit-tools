 ...
 *     npx vitest run tests/audit/a9.test.ts
 */
...
import type { RemediationStep } from '../../src/remediate/steps/types.js';
...
const { statusToDisposition, isTerminalStatus } = await import(
  '../../src/remediate/state/itemStatus.js'
);
 ...
 */
async function runAuditHalf(root: string): Promise<void> {
  ...
  const nextStep = () =>
    runDeterministicForNextStep({
      ...
      timeoutMs: SESSION_CONFIG.timeout_ms ?? 180_000,
 ...
 */
async function runRemediateHalf(root: string, inputPath: string): Promise<RemediationStep> {
  ...
  let step: RemediationStep | undefined;
 ...
 */
async function satisfySynthesizeIntake(step: RemediationStep): Promise<void> {
 ...
 */
async function satisfyHostGate(ctx: { step: RemediationStep; artifactsDir: string }): Promise<void> {
  ...
  const write = async (target: string, body: string) => {
  ...
  switch (step.step_kind) {
    ...
    case 'collect_triage': {
      ...
      const parsed = JSON.parse(readFileSync(join(artifactsDir, 'state.json'), 'utf8')) as {
        items?: Record<string, { finding_id: string; status: string; failure_reason?: string }>;
      };
      ...
      try {
        for await (const p of glob('**/accept-outcome-*.json', { cwd: artifactsDir })) {
          const a = JSON.parse(readFileSync(join(artifactsDir, p), 'utf8')) as {
            outcome?: string;
 ...
 */
async function seedCompleteContractDag(artifactsDir: string): Promise<void> {
  const shared = await import('audit-tools/shared');
  const cp = await import('../../src/remediate/validation/contractPipeline.js');
  const { writeContractArtifact, CP_ARTIFACT_NAMES } = await import(
    '../../src/remediate/contractPipeline/artifactStore.js'
  );
  type ContractPipelineArtifactName = (typeof CP_ARTIFACT_NAMES)[number];
  const created_at = new Date(0).toISOString();
  const goal_id = 'AUTONOMY-G1';

  const PHASE_BUILDERS: Partial<Record<ContractPipelineArtifactName, () => unknown>> = {
    goal_spec: () => ({
...
test.skipIf(!RUN)(
  ...
  async () => {
    await withTempDir('a9-autonomy-e2e-', async (root) => {
      ...
      const completeState = JSON.parse(await readFile(completeStatePath, 'utf8')) as {
        items?: Record<string, { finding_id: string; status: string }>;
      };
