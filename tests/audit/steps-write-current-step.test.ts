...
import type { StepArtifact } from "../../src/audit/cli/steps.js";
...
type WriteCurrentStepParams = Parameters<typeof writeCurrentStep>[0];
type CanonicalStepArtifact = StepArtifact & {
  artifact_paths: StepArtifact["artifact_paths"] & {
    current_step: string;
    current_prompt: string;
  };
};

function requireCanonicalPaths(
  step: StepArtifact,
): asserts step is CanonicalStepArtifact {
  if (
    step.artifact_paths.current_step === null ||
    step.artifact_paths.current_prompt === null
  ) {
    throw new Error("writeCurrentStep must return its canonical artifact paths");
  }
}
...
function baseParams(artifactsDir: string): WriteCurrentStepParams {
  return {
    artifactsDir,
...
describe("writeCurrentStep writes prompt to current-prompt.md and returns correct StepArtifact", () => {
  ...
  let params: WriteCurrentStepParams;
  let step: CanonicalStepArtifact;
  ...
  beforeAll(async () => {
    ...
    const writtenStep = await writeCurrentStep(params);
    requireCanonicalPaths(writtenStep);
    step = writtenStep;
  });
...
test("writeCurrentStep includes progress when supplied", async () => {
  ...
  try {
    const artifactsDir = join(dir, "artifacts");
    await mkdir(artifactsDir, { recursive: true });
    const progress = { summary: "12 tasks remaining", pending_tasks: 12, agent_count: 4 };
...
test("writeCurrentStep artifact_paths merges current_step and current_prompt with caller-supplied paths", async () => {
  ...
  try {
    ...
    requireCanonicalPaths(clobberStep);
