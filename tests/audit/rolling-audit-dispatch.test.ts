...
import type {
  ...
  AuditResultIngestor,
  DriveRollingAuditDispatchResult,
} from "../../src/audit/cli/rollingAuditDispatch.js";
import type {
  CapacityPool,
  SessionConfig,
...
const { ACTIVE_DISPATCH_FILENAME, AUDIT_TASK_CLAIM_LEASE_MS } = await import("../../src/audit/cli/dispatch.js");
const { primaryInProcessSource } = await import("../../src/shared/quota/apiPool.js");
const { ClaimRegistry } = await import("../../src/shared/quota/claimRegistry.js");
const { taskClaimsPath } = await import("../../src/shared/index.js");
...
async function makeRun(taskListOverride?: AuditTask[]) {
...
function makeWritingDispatcher(runDir: string, taskList: AuditTask[]): AuditPacketDispatcher {
  ...
  return async (packet, _slot) => {
    ...
    const taskIds = resultMap.entries
      .filter((e: { packet_id: string }) => e.packet_id === packet.id)
      .map((e: { task_id: string }) => e.task_id);
    const results: AuditResult[] = taskIds.map((tid: string) => {
...
// A minimal, contract-complete DispatchPlanEntry fixture builder: only the
// fields each test actually varies are parameters; the rest are fixed sensible
// defaults matching the real (non-optional) DispatchPlanEntry shape.
function packetPayload(fields: {
  packet_id: string;
  prompt_path: string;
  result_path: string;
}): DispatchPlanEntry {
  return {
    ...
    result_path: fields.result_path,
    ...
    model_hint: { tier: "standard", reasons: [] },
    file_paths: [],
  };
}
...
test("A8a: makeAuditProviderPacketDispatcher launches read-only against the repo root and returns success when the worker wrote a result", async (t) => {
  ...
  const captured: LaunchFreshSessionInput[] = [];
  const fakeProvider: FreshSessionProvider = {
    name: "fake-provider",
    async launch(input) {
      captured.push(input);
  ...
  const packet: RollingDispatchPacket<DispatchPlanEntry> = {
    id: "pkt-1",
...
test("A8a: driveRollingAuditDispatch drives every packet, writes results, and folds them in via the terminal ingestor", async (t) => {
  ...
  let ingestCalls = 0;
  const ingestStub: AuditResultIngestor = async ({ runId }) => {
    ingestCalls++;
    ...
    const plan: DispatchPlanEntry[] = JSON.parse(
...
test("A8a: driveRollingAuditDispatch pauses resumably (waiting_for_provider) when packets strand (DC-4)", async (t) => {
  ...
  const ingestStub: AuditResultIngestor = async () => {
...
test("write-scope: in a git repo, spawned review workers launch against a disposable snapshot worktree, never the real checkout", async () => {
  ...
  const { removeReviewSnapshot } = await import("../../src/shared/providers/reviewSnapshot.js");
...
test("A8a: a same-packet account wall escalates through the retained host-session source and captures a quota_escalation friction (parity with remediate)", async (t) => {
  ...
  const pools: CapacityPool[] = ["pa", "pb", "pc", "pd"].map((id) => ({
    id,
    accountKey: id,
  ...
  const stranding: AuditPacketDispatcher = async (packet, slot) => {
  ...
  const escalation = friction.frictions.find((f: { id: string }) => f.id.startsWith("quota_escalation:"));
...
test("A8a: driveRollingAuditDispatch degrades to no-progress (does not crash) when every accepted result is ingestion-invalid", async (t) => {
  ...
  let result: DriveRollingAuditDispatchResult | undefined;
...
test("F4: driveRollingAuditDispatch never dispatches a floor-carrying packet to an incapable pool", async (t) => {
  ...
  const capablePool: CapacityPool = { ...poolBase, id: "src-deep", accountKey: "src-deep", rank: "deep" };
  const incapablePool: CapacityPool = { ...poolBase, id: "src-small", accountKey: "src-small", rank: "small" };
