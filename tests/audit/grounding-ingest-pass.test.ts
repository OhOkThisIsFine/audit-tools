...
const { groundPassingFindings } = await import("../../src/audit/cli/mergeAndIngestCommand.js");
...
test("groundPassingFindings grounds every finding and returns the ungrounded ones in input order", async () => {
  ...
  try {
    ...
    if (
      !passing[0].findings[0].grounding ||
      !passing[0].findings[1].grounding ||
      !passing[1].findings[0].grounding
    ) {
      throw new Error("groundPassingFindings did not annotate every finding");
    }
