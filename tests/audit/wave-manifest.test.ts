...
import type { WaveManifest } from "../../src/audit/cli/waveManifest.js";
...
const {
  ...
  removeWaveManifest,
  waveManifestPath,
} = await import("../../src/audit/cli/waveManifest.js");
...
await test("writeWaveManifest stamps contract_version and persists all manifest fields", async (t) => {
  ...
  const written = await readJsonFile<WaveManifest>(waveManifestPath(dir));
  ...
  if (roundTripped === null) {
    throw new TypeError("written wave manifest could not be read back");
  }
