...
function run(scriptPath: string, args: string[] = [], cwd: string = process.cwd()) {
...
function withTempDir(prefix: string, fn: (dir: string) => any): any {
...
function minimalValidResult(taskId: string) {
  return {
    task_id: taskId,
...
function manifestTask(taskId: string) {
  return {
    task_id: taskId,
    unit_id: "src/utils/helper.ts",
    pass_id: "pass-1",
...
function writeManifest(artifactsDir: string, runId: string, tasks: unknown[]) {
...
test("NEGATIVE: merge-results.mjs rejects a manifest task with NO on-disk result as missing (completeness judged against the manifest)", () => {
  withTempDir("dispatch-scripts-missing-", (artifactsDir) => {
    ...
    expect(failedTasks.map((f: { task_id: string }) => f.task_id)).toEqual(["task-b"]);
    expect(failedTasks[0].errors.some((e: string) => /missing audit result/i.test(e))).toBeTruthy();
    expect(result.stdout).toMatch(/1\/2 tasks valid/i);
  });
});
...
test("NEGATIVE: merge-results.mjs rejects a result whose task_id is not in the pending manifest (identity is the tool's authority)", () => {
  withTempDir("dispatch-scripts-unknown-", (artifactsDir) => {
    ...
    expect(runResults.map((r: { task_id: string }) => r.task_id)).toEqual(["task-known"]);

    const failedTasks = JSON.parse(
      readFileSync(join(artifactsDir, "runs", runId, "failed-tasks.json"), "utf8"),
    );
    expect(failedTasks.some((f: { task_id: string; errors: string[] }) => f.task_id === "task-unknown" && f.errors.some((e: string) => /unknown task_id/i.test(e)))).toBeTruthy();
  });
});
...
test("NEGATIVE: merge-results.mjs rejects a duplicate task_id instead of double-merging (dedup-by-task_id, CP-NODE-2)", () => {
  withTempDir("dispatch-scripts-dup-", (artifactsDir) => {
    ...
    expect(failedTasks.some((f: { errors: string[] }) => f.errors.some((e: string) => /duplicate audit result/i.test(e)))).toBeTruthy();
  });
});
...
test("INV-01: merge-results.mjs expands a top-level AuditResult[] array from a single file", () => {
  withTempDir("dispatch-scripts-array-", (artifactsDir) => {
    ...
    const ids = runResults.map((r: { task_id: string }) => r.task_id).sort();
