...
test("computeArtifactStateSignature ignores revision churn but tracks content", () => {
  const sigOf = (artifacts: Record<string, ArtifactMetadataEntry>) =>
...
test("runtime_validation_current obligation: stale when report exists but tasks incomplete", () => {
  ...
  const bundleWithPendingResult: ArtifactBundle = {
    ...
    runtime_validation_report: {
      results: [{ task_id: "rv-task-1", status: "pending", summary: "Not yet run." }],
    },
  };
  ...
  const bundleNoReport: ArtifactBundle = {
    runtime_validation_tasks: {
      tasks: [
        {
          id: "rv-task-1",
  ...
  const bundleComplete: ArtifactBundle = {
    runtime_validation_tasks: {
      tasks: [
        {
          id: "rv-task-1",
    ...
    runtime_validation_report: {
      results: [{ task_id: "rv-task-1", status: "confirmed", summary: "Mitigation confirmed." }],
    },
  };
...
test("absent dependency with recordedRevision > 0 marks artifact stale", async () => {
  ...
  const initialBundle: ArtifactBundle = {
    repo_manifest: {
      repository: { name: "fixture" },
      generated_at: "2026-03-23T00:00:00Z",
      files: [{ path: "src/api/auth.ts", language: "ts", size_bytes: 100 }],
...
test("F1 metadata-migration fail-safe: old-shape (pre-F1) manifest → ALL present artifacts stale, no throw", () => {
  ...
  const preF1Manifest: ArtifactMetadataManifest = {
...
test("F1 single-adjacency: ARTIFACT_DEPENDENTS_MAP is the derived inversion of the one canonical table", () => {
  ...
  for (const [artifact, ups] of Object.entries(ARTIFACT_DEPENDS_ON_MAP) as Array<[string, readonly string[] | undefined]>) {
    if (!ups) continue;
    for (const up of ups) {
      (rebuilt[up] ??= []).push(artifact);
    }
  }
  const norm = (m: Record<string, readonly string[] | undefined>) =>
    Object.fromEntries(
      Object.entries(m)
        .map(([k, v]) => [k, [...(v ?? [])].sort()])
...
test("F1 inv-4 [CP-NODE-5]: old-shape manifest => all-stale, no throw", () => {
  ...
  const olderShapeManifest: ArtifactMetadataManifest = {
...
test("F1 inv-6 [CP-NODE-7]: dep-map.md literal parity incl. git_history.json upstream edges", async () => {
  ...
  const mdDependsOn: Record<string, Set<string>> = {};
...
test("F1 fail-2 [CP-NODE-13]: per-element verdict never keyed on the bare grouping coordinate (distinct result_content_discriminator never collapses)", () => {
  ...
  const redispatchInput = { ...grouping, source: "redispatch" as const, attempt: 2, stage: "O3" };
  const redispatch = deriveLiveResultKeys(redispatchInput);
