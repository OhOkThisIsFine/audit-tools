...
import type { DesignReviewSnapshot } from "../../src/audit/orchestrator/designReviewSnapshot.js";
...
function bundle(overrides: Record<string, any> = {}): ArtifactBundle & DesignReviewBundle {
  return {
    repo_manifest: {
    ...
    surface_manifest: {
      surfaces: [
        { id: "S1", kind: "interface", entrypoint: "src/a.ts", exposure: "local", methods: ["GET"], notes: ["n"] },
      ],
    },
    critical_flows: {
      flows: [
        { id: "F1", name: "login", entrypoints: ["src/a.ts"], paths: ["src/a.ts"], concerns: ["auth"], confidence: "high" },
      ],
    },
    risk_register: {
...
function checkpoint(overrides: Record<string, any> = {}): IntentCheckpoint {
  return {
...
function projStr(b: DesignReviewBundle): string {
...
test("excluded field: surface.notes churn projects byte-identically", () => {
  ...
  const after = projStr(
    bundle({
      surface_manifest: {
        surfaces: [
          { id: "S1", kind: "interface", entrypoint: "src/a.ts", exposure: "local", methods: ["GET"], notes: ["ENTIRELY REWORDED NOTE"] },
        ],
      },
    }),
  );
...
test("excluded field: the scope bracket-tag REASON text is cosmetic (reword projects byte-identically)", () => {
  ...
  const unitProj = projectDesignReviewInput("unit_manifest", wordingA) as Array<{ unit_id: string; scope: string }>;
...
test("negative-complement: a surface method (rendered) re-stales", () => {
  ...
  const after = projStr(
    bundle({
      surface_manifest: {
        surfaces: [
          { id: "S1", kind: "interface", entrypoint: "src/a.ts", exposure: "local", methods: ["GET", "POST"], notes: ["n"] },
        ],
      },
    }),
  );
